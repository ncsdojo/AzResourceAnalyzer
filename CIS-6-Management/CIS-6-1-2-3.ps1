# CIS 6.1.2.3 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "6.1.2.3"
        Title       = "Activity log alert: Create or Update NSG"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $alerts = $D.ActivityLogAlerts
        ...
        foreach ($a in @($alerts | Where-Object { $_.enabled })) {
            if ($a.conditions -match [regex]::Escape('Microsoft.Network/networkSecurityGroups/write')) { $found = $true; break }
        }
        if (-not $found) {
            return @(@{ AffectedResources=@("Microsoft.Network/networkSecurityGroups/write"); Description="No enabled activity log alert for Create or Update NSG." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBfKmDpzYk58xQr
# OQREMYxp664qlYRiYUILzzoeyKUvFaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIAqAQN2EDpgQzVvXthjSzVPk8vDb0Ag62lm8F6ve
# vpFSMA0GCSqGSIb3DQEBAQUABIIBgD7u5PfxJXPINJkwcYDkTjNSGVC/Dghe4h4t
# Gyl8RI9b75RV6dPFJrUcI+my5T2rsog7o2/GAmFgDYtsCo7yQGgx3DGvsc8XvcsO
# h/hDtkuxb/9+Cf9VHoGU6zpQErIxjqySeSib/txHSfJthz4hvCx84FTIfa4BM4bk
# 5G+1s0SrVtGB82kbxtaYOvMqPpBu7lzISk9voTtZtYE9ELeQtwPIHz3r7lXejdR5
# vJNjatxP/cLDHRJ1QRcOqKvMGwuCep5pWITKcmNCouCOMF6bmfqHY1aIeec3EAQO
# 23Y7yHrC7o90UjxBlFiXvQ159JRw3HP/xrlfJACrct96qTTEZrlxbY5aOCymrVcf
# auJEeD3knqN72mruJwBZqTW+4hnWCxbZOCYqOoDQRHl8CcBfKAFjLHj4W2uU3+2x
# iWU2SObUGJFlHRs0EGa1nTtTpDzmzQEBnh8wG84PlhK1LwQZC2JMz+oyF3fjICDK
# Ua8X7iYPNrMyK/8n00YD3h5UO7fbkqGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCCm411/BTFfKeYvaO6qzrHT7Kx76fBEG3NzjPA0EP5wBQIGaeiBPKaJ
# GBMyMDI2MDUyMjE0NDQyNy40MjJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NDQyN1owLwYJKoZI
# hvcNAQkEMSIEIElWEuQd72lp+Xz4WjBuz2raq5fPzyCHiJbtaUGZaje9MIG5Bgsq
...
# 9w0BAQEFAASCAgA3b4kJpO1b1wib2hnsMRngeNE57U9H8eFMhddkreJvRO1YW3wC
# TQPhVxtNoflNpIE9Ka+vXbhb3gx1gj7cHTcF4a5kQ6i6F13Pdq+15VMJS27C7AGw
# OO+BqpbuzF+POwYodj+t7D54B4VH+c4sr+caeioPbOVggfKQvDPwiBihwtpoCXO9
# xIJl28/n/61TPx8xxhRv2UPkt8Qdek2y+zReq4hx7D3Z788Me50dKatH6wE3maCr
# +9YwlsIQoWzVbbdr2QDaQARqf3XeR2fwodDq5sroZWIJ6lSkZeKCMxyL0RFHMyVU
# bKx/yJNH8iV16g00QNz6rELgqogKEX+vPCl4lOG4jfPWGhv/KRxDtJWOe1+PzTaJ
# IQVE7hR0W+nw1b4s4Qy4MX1eqxjtjgI0B45NmhipyNjAA419iPZ4LWeYiQd2j6ov
# ki0Kt/UdiFg7VPH1Z3qFfidU+p4agNYFKecwqaWM+CjssLgcrmAndGeplq6mGhep
# t8V7BLugNDvZo2K9gmd/QRSXYqrGV5zwLtVOdILspnSsoAepEV7m2PIKsZOZsIV8
# FQpojnI9Rj5WZrLD3J5faaP/aLELjhBqzGxb+w+S9ZxAg+fTKzohzxYBuA4WLZeD
# /SaSl0hsF/dlZJ7SB/2vbuVVxj+ap4dOzXIDbYidEtmNnu2PENGUidnf6w==
