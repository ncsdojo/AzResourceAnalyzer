function Connect-ArSession {
    <#
    .SYNOPSIS
        Authenticates to Azure for CIS resource collection.
    .PARAMETER TenantId
        Tenant domain or GUID.
    .PARAMETER AuthTimeout
        Device code sign-in timeout in seconds. Default: 45.
    .EXAMPLE
    ...
    param(
        ...
        [ValidatePattern('^[a-zA-Z0-9][a-zA-Z0-9._-]*\.[a-zA-Z]{2,}$|^[0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}$',
            ErrorMessage = 'TenantId must be a valid domain or GUID.')]
        [string]$TenantId,

        [int]$AuthTimeout = 45
    )

    if ($script:_ArSession) { Write-Verbose 'Disposing previous session'; $script:_ArSession.Dispose() }

    [ArTokenStore]::TimeoutSeconds = $AuthTimeout
    $session = [ArTokenStore]::new($TenantId)
    ...
    try {
        ...
        $session.Authenticate([ArTokenStore]::ArmScope, [ArTokenStore]::ArmClientId)

        Write-Information '  Acquiring Graph token...' -InformationAction Continue
        $session.Authenticate([ArTokenStore]::GraphScope, [ArTokenStore]::GraphClientId)

        Write-Information '  Acquiring Vault token...' -InformationAction Continue
        $session.Authenticate([ArTokenStore]::VaultScope, [ArTokenStore]::ArmClientId)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBGSfJl7hB6g1er
# 2mk33jfi7N9YHkQtPC9lOYoKYtkG36CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIKgicpyykdym+RQ1+lT7rTR+9IKDn2reUfkNWD0U
# IDN3MA0GCSqGSIb3DQEBAQUABIIBgJhVgi0XgL4eOZKR74USpN5JDRbiYbyC3RqZ
# geTjSr4vRAaUgmWY6gCrkfCuIsLg/uIV8/oPjyfNvS6l3m9xrsl0MN60XrnQ5QTM
# 50ZB/Syxll3ikenyWbkXApXLDJWd9DAsfhpsf5C7gp/BDDzwrX53Pq3yAsxGYWvY
# r8T2iPXfF9ZuQDq+naaJEJQnPSnSKfDFtebQsxywav++4+hZB6BkXH1adWwNDPtf
# MIPxGEQaE60qmqf0PLKKgfKy3/wKyQHfUD7uoAbf/abURc1s+/XCvJ0Jgi13dkJi
# JSlBSueIZ+0TUAEU+Km0wyIQ+pLbo6bj2TtsYvuckigvCs3j8FKt45brtv/KZiEi
# R+aIjNUHCD8D4UXRAEFQmDD9a0ZdhiHzJ4idNWUr+hU6sPSjxWgA8vIpVI6rqwx5
# Rmm28V2IMRSLlK1nZkpwo0JYp9j+lDXs65CPtJs6GSx+rHUu4xNkd5y/vye/0Bdv
# KzDpx5HGJBHyGdbkOxsXne61HLlKqKGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCDU+0ALCXa09bkKlEhZ0hHNfpjESYeqgCv9ahzXe2bvXQIGaeiBPO7X
# GBMyMDI2MDUyMjE1NDgwMS4yNzNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1NDgwMVowLwYJKoZI
# hvcNAQkEMSIEIPY7NbNHCmDho0hS7WlhtMrawLzi1UnJlv8gZLBY7nkuMIG5Bgsq
...
# 9w0BAQEFAASCAgBoMSg5BzGOIKxXTLz4qRR6r2IeWIQUvRicQu7VNNU8W2e4awc+
# eEmrz5UgIL6+dWwncEoxN8rYMu7quamUKI06ITDxzDvNgXOYWWOjol49oTVfQNXa
# fLBw53c+UywJ2F0HTMPw2BHNnWv43ALOinHBUgtmrtOwXmm2mVsDgHASqk/upqjn
# U6/q3SBa153fX534Oba66bRIlTbGVAWDT/UBga547TtDrpt1g+HegMQg7vfPVKVL
# NDHyYm2IkqxAi9n0SmmYYQNlzkZZV4q4bRfAlDmWK2vKISJV+iTvWQgPqmvG+znm
# ngh029TVDuPjZdvNdzNipKLMZNo7o5r8b+VUsexQy6PNvIBmnu/Kt++0WWd1JMZl
# eIHFQOUvAiWW0kJf7Qd26YibsKtH2ADpx0S6cZrc2NUfiCxJFqs0RfbGV1oomAum
# etPUoXEXmwfGHZwQnD6I6TosPxTpB7Ox2XqNHib3q2CrRruGEnWdcb5heBD3GuVg
# FqA8McRimG7yZm1BtbjXt1nOR3wPl2+UssrtJ1KfZ6Md1opwPqhoVbfwBu4VsIWy
# uH3SxELyK2fONqqxe9oUBBAAsbCnsatmrpwE5a4rFRfDdxRlBaB+gNoQUKRN8J7b
# O5DZufFvDB6FdR/YECSCdhCJNoWidUa+MWHqPqzL7ruoQblNvttIQ5jUjg==
