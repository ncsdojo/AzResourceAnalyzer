# Key Vault data plane collector — Copyright (c) NCS Dojo. All Rights Reserved.

function Invoke-KeyVaultCollection {
    param(
    ...
    # Need vault URIs from a prior KeyVault RG collection
    $kvData = @()
    if ($CollectorStatus.Contains('KeyVault') -and $null -ne $CollectorStatus['KeyVault']['_results']) {
        $kvList = $CollectorStatus['KeyVault']['_results']
        if ($kvList.Count -gt 0) { $kvData = @($kvList) }
    }
    if ($kvData.Count -eq 0) {
        $kvFile = Get-ChildItem $OutputPath -Filter "KeyVaults_*.json" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($kvFile) {
            $kvJson = Get-Content $kvFile.FullName -Raw | ConvertFrom-Json
            if ($kvJson.PSObject.Properties['data'] -and $null -ne $kvJson.data) {
                $kvData = @($kvJson.data)
            }
        }
    }
    ...
    if ($kvData.Count -eq 0) {
        Write-Log "No vault data found, skipping Key Vault collectors" -Level WARN
        return
    }
    ...
    foreach ($coll in $Collectors) {
        $CollectorStatus[$coll.Name] = @{
            ...
            status = "Pending"; warnings = @(); errors = @(); _results = [System.Collections.Generic.List[object]]::new()
        }
        $sw = [System.Diagnostics.Stopwatch]::StartNew()
        $apiVer = if ($coll.ContainsKey('ApiVersion')) { $coll.ApiVersion } else { "7.4" }
        $vaultErrors = 0

        foreach ($kv in $kvData) {
            $vaultUri = $kv.vaultUri
            if (-not $vaultUri) {
                if ($kv.PSObject.Properties['vaultName']) {
                    $vaultUri = "https://$($kv.vaultName).vault.azure.net/"
                } else { continue }
            }
            $vaultUri = $vaultUri.TrimEnd('/')

            try {
                $kvHeaders = @{ Authorization = "Bearer $vaultToken" }
                $uri = "$vaultUri/$($coll.SubPath)?api-version=$apiVer"

                do {
                    $resp = Invoke-RestMethod -Uri $uri -Headers $kvHeaders -Method GET
                    if ($resp.PSObject.Properties['value']) {
                        foreach ($item in $resp.value) {
                            $item | Add-Member -NotePropertyName 'vaultUri' -NotePropertyValue $vaultUri -ErrorAction SilentlyContinue
                            $item | Add-Member -NotePropertyName 'vaultName' -NotePropertyValue ($vaultUri -replace 'https://|\.vault\.azure\.net','') -ErrorAction SilentlyContinue
                            $CollectorStatus[$coll.Name]._results.Add($item)
                        }
                    }
                    $uri = if ($resp.PSObject.Properties['nextLink']) { $resp.nextLink } else { $null }
                } while ($uri)
            }
            catch {
                $vaultErrors++
                ...
                $reason = switch ($true) {
                    ($statusCode -eq 403) { 'access denied (RBAC or firewall)' }
                    ($statusCode -eq 401) { 'unauthorized' }
                    ($statusCode -eq 404) { 'not found' }
                    ($_.Exception.Message -match 'Unable to connect') { 'network blocked (private endpoint?)' }
                    default { ($_.Exception.Message -replace '\r?\n',' ').Substring(0, [Math]::Min(100, $_.Exception.Message.Length)) }
                }
                $vaultName = $vaultUri -replace 'https://|\.vault\.azure\.net',''
                $CollectorStatus[$coll.Name]['warnings'] += "$vaultName : $reason"
            }
        }

        $sw.Stop()
        $CollectorStatus[$coll.Name]['rowCount'] = $CollectorStatus[$coll.Name]['_results'].Count
        $CollectorStatus[$coll.Name]['durationSeconds'] = [Math]::Round($sw.Elapsed.TotalSeconds, 2)
        $CollectorStatus[$coll.Name]['status'] = if ($vaultErrors -eq $kvData.Count) { "Error" } else { "Success" }
        Write-Log "  $($coll.Name): $($CollectorStatus[$coll.Name].rowCount)$(if ($vaultErrors -gt 0) { " ($vaultErrors denied)" })" -Level $(if ($vaultErrors -gt 0) { "WARN" } else { "SUCCESS" })
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC