...
function Invoke-GraphCollection {
    ...
    foreach ($coll in $Collectors) {
        ...
        try {
            ...
            $CollectorStatus[$coll.Name]['rowCount'] = $results.Count
            $CollectorStatus[$coll.Name]['status'] = "Success"
            Write-Log "  $($coll.Name): $($results.Count) $($coll.Label)" -Level SUCCESS
        }
        catch {
            $CollectorStatus[$coll.Name]['status'] = "Error"
            $CollectorStatus[$coll.Name]['errors'] += $_.Exception.Message
            Write-Log "  $($coll.Name): error — $($_.Exception.Message)" -Level ERROR
        }

        $CollectorStatus[$coll.Name]['_results'] = $results
        $sw.Stop()
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAPHYTCxcnEk9Uq
# vjh7/13e4Jesye5HML4SETP+nbWDbqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEILAERq3GNFbTn9+iZWNKsmZgM1LC+CLh58Vy1fjW
# w619MA0GCSqGSIb3DQEBAQUABIIBgEnUqcQHXlbUwoSq3b4tjZSUAW4CrjO9y38H
# ZZhOoRGSABSNAoHHGpOegBURjKMLBkfH80h4zcYzHC7vRVsFpr7v4V14BihRx0ME
# aoIOW9vvvkanOA2RO++VRYIIg57i/C1kFtfEeBHqYdXkTp56yqkXBR6S8I7Rc54z
# FPnCssKRXvgO7BK2BlCb/B15aargbZE65a1tFR35QAGMr7E3v1e45QXNfndUCsfI
# q2pTClBsihFn4EUBDYuX7BQSUR48IFEIMYsdEjyMkmidWQKCJf3guwdM69XPFXNF
# +umb2ziQNK6LiJeaGldvO5OFfsQ8I8ifBm/74O4RpHxsBhGVKba7ybMGFSzAc2EH
# +tLPrQbtFeEBCdBP++es98PhyorQQKBmxS9VYJ2cmXqAEH1YSCXYHOC0mO36xVGe
# xFM3m4raJnqEkoeNLOehMI9VX8bIXjHqRkt/H7/cFzMH0R6SIaiFjZAWE2bxnIJD
# DZV4k/WZUDZh9c4/qii4+eMQuzoVsqGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCCS6Fiuyj0dxUDegs3m/4G2bVAmAOTRaUu4Yp0oreIFegIGaeiBPwp+
# GBMyMDI2MDUyMzAzMDYxMi4zODRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAzMDYxMlowLwYJKoZI
# hvcNAQkEMSIEILL2Jh1oOyG+Vu1TBMDx33maETTJRy36Y3gh7iKc4yr5MIG5Bgsq
...
# 9w0BAQEFAASCAgCPOC9Hyz0/JfyJfyF1hiK+P8C/76spZQBWpTdEtNyftBHAz9T+
# lxOuItbv8axlKFf1h4a8nhS33mNbu6OxE24DnYZiXJHGn65hXk64MYpEO350C99r
# i2jVv56Ecmptb6N/Ik0Z4BhgQZ+SU0HmBgxKGqaLkTXduHzwVkMxneVHeLQJkB3z
# 2YAOP0vrLmunpblBshgrbZVKnIB1ePEONU+45AXFI1zAhUPTyaicgKRVLHx4SBdh
# XDsMYD+IkJTqvFGGOI6z1vXQ4+QgPcw5olr/b9ZipTEphIGKguqALTDa4AqL/+SV
# qm64ZWDrLQMAQoHGUf+oiX/7hQAjgIrC4hHlnv0cSE5ZsK0/UMAdfexaf5Cqc+ib
# mealCYNZThMQPgdF7TCpxJDjky0gCEa5qNwZFu+scHgJmkSlqiXYvvJw0b04XelX
# mmU4njtfoRPWaMfKzmaCea+69fD5hMwNeiNSBC0z10cRP6qWqF6Owowq/XnYk5KV
# ydnevqh+sKlBYsLqvL6jtJImroFjK45KOfpHdWkCsyv9qoL4gI91DTYCE2vGoWAi
# EooWVnyJgYHmUOSI7ovEAHb1CY2ZjwXRds0rpqHHrD2RfeZGQQMFl/5xoslHxY0A
# 2A/QEWrEvpM0cpETkJ79ntcA3WMT9fV1tXrSH6yN9rGS2d6iJmhWLFjSyw==
