# Microsoft Graph collector — Copyright (c) NCS Dojo. All Rights Reserved.
...
function Invoke-GraphCollection {
    ...
    foreach ($coll in $Collectors) {
        ...
        try {
            ...
            $uri = "https://graph.microsoft.com$($coll.Endpoint)"

            $params = @()
            if ($coll.ContainsKey('Select') -and $coll.Select) { $params += "`$select=$($coll.Select)" }
            if ($coll.ContainsKey('Filter') -and $coll.Filter) { $params += "`$filter=$($coll.Filter)" }
            if ($params.Count -gt 0) { $uri += "?" + ($params -join "&") }

            $paginated = if ($coll.ContainsKey('Paginated')) { $coll.Paginated } else { $true }
            ...
            do {
                ...
                if ($resp.PSObject.Properties['value']) {
                    ...
                    $CollectorStatus[$coll.Name]._results.Add($resp)
                }
                $uri = if ($paginated -and $resp.PSObject.Properties['@odata.nextLink']) { $resp.'@odata.nextLink' } else { $null }
            } while ($uri)

            $CollectorStatus[$coll.Name]['rowCount'] = $CollectorStatus[$coll.Name]['_results'].Count
            $CollectorStatus[$coll.Name]['status'] = "Success"
            Write-Log "  $($coll.Name): $($CollectorStatus[$coll.Name].rowCount) $($coll.Label)" -Level SUCCESS
        }
        catch {
            $CollectorStatus[$coll.Name]['status'] = "Error"
            $CollectorStatus[$coll.Name]['errors'] += $_.ToString()
            Write-Log "  $($coll.Name): error — $_" -Level ERROR
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDiCtDd7NDDFgp6
# 09hK0Eb3AakozpGbRTzKlSzVEavV3KCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIL/L7g7sthyQ0eEzh3HxbIyygektLGYiygqNrTL/
# IHOiMA0GCSqGSIb3DQEBAQUABIIBgDGDBFptZdj7+ptguzWqriUe6QsK5jbbDXQz
# 3C+Z7LbmPuTOcTZMmP63pJw63l1x8if7wp2eiltP9TfMVe1P6L7v7QboU+cgx732
# FtTEe3C4VMoKZTxcVkagT1Q/7BSilWQ0Q/AD78FDeRepVACJPHnAoXcVHXzKulZw
# oOkThkMO4psiCZL9Y4WKCS6qQQzN4kCsXUy2SyJIoBwexAt04wzeH5c5I6w194fp
# 5GhouKNBbMV+JzU3Xz3uuj0vNNh0+VwQVVsvzqzaqr1XSMrCQBsX0EFgMG7B/J+I
# wpy6x2yBp4fLvh9wTRLoSF9C2BRDj1XYsu83k7tREAhix87pBYIbQqnhMC1NF8mU
# KUxlFhghS067fa6iDWFFRRvI1n8Yi8wVorMSR+/22hJTASSQSSut4/zVNQ2lyZVd
# E/EWecv+3JX5SgC2GE4nhCIx1OTviusbh+J1fz+3Fc5+wfLNC7tMlJXCCnPgSyTh
# QbaG5ujNjKl1Q60XxF0gCEYN6VFfkaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCB12Ae7FZ/qs7qieG9c1Y8SuDxk2SPljmLpAMMK+iPO5QIGagxE151j
# GBMyMDI2MDUyMjE1NDY1My4wMDRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1NDY1M1owLwYJKoZI
# hvcNAQkEMSIEIKbmK11M1m+y3c6V54BZbiKNVR33GXQoyEm2k6wM8pbnMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgB77A4rUncHi5eY7lkz9hGyGRQRtVNTjSBMD82X5nuOYyAG
# i5e6KkWceD0JbJAAJvi3iiCJ4gEStR3NrMDT/QwwE7+MA8OirPo0SeFY+F8jqYqo
# KVrCIOJEdefnIgiIUm56nuJgJYN0/WE9ZsM+mWBJOZs4L/exklog+Ppn3lBa/qvJ
# FEvLLAhIa/+o/88CDNxVPKjJ3U2gfu0aUAtC8VX3y9ggOh2NhnQB3ENX0QnNiTqr
# I9juAQasIuw2+Sr4GYXMUVaptbuckMLIYVYDO8YGHIofpHfXSgVTaIQG5BoTGqf7
# p0ORE68vmeEWOD2WS8VLroyuHJlp9hQywoHkwxS1yXgc9MZMezYa9tkTZZQQqNQq
# tQ17xNbcddtonY7hkQ36MeUFO85XgXb4w2XQzVZQ7E6qYPf3Ps1uTK7l/GsCqZTk
# UNQRyK2nnuPr2yNyE9Q0FGAIHNegg35xSOlTyD5X/kmksIg9KOpSPpSU6mM0yZ7Z
# aqjkc8hZt/NPklUhuqTvwQptv5iChTWtaNg2j9S24zIM+c+CRL8rzLuEf1rTxY54
# /ZpZGrRBtcAfjtzdOyZhbALhQmLFeVXWFUeTShuSj9Agu2Y8eqCkmqep7suOaPI+
# Y2TQeq4+pM4c90Uf61RSEZwcfLXT4OcOc4UkEcLfg9+yXSyqahpiFMN3h7GIDg==
