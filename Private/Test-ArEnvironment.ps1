...
function Test-ArPermissions {
    ...
    try {
        ...
        $me = Invoke-RestMethod -Uri 'https://graph.microsoft.com/v1.0/me?$select=id,userPrincipalName,displayName' -Headers $headers -Method GET
        $results.User = $me
    }
    catch {
    ...
    # Check Reader role on each subscription
    if ($armToken -and $results.Subscriptions.Count -gt 0) {
        $readerRoleId = 'acdd72a7-3385-48ef-bd42-f606fba81ae7'
        $readSubs     = [System.Collections.Generic.List[object]]::new()
        $noReadSubs   = [System.Collections.Generic.List[object]]::new()

        foreach ($sub in $results.Subscriptions) {
            try {
                $permHeaders = @{ Authorization = "Bearer $armToken"; 'Content-Type' = 'application/json' }
                $permResp = Invoke-RestMethod -Method GET `
                    -Uri "https://management.azure.com/subscriptions/$($sub.subscriptionId)/providers/Microsoft.Authorization/permissions?api-version=2022-04-01" `
                    -Headers $permHeaders

                $hasRead = $false
                if ($permResp.PSObject.Properties['value']) {
                    foreach ($perm in $permResp.value) {
                        $actions = @($perm.actions)
                        if ($actions -contains '*' -or $actions -contains '*/read') {
                            $hasRead = $true
                            break
                        }
                    }
                }
                if ($hasRead) { $readSubs.Add($sub) } else { $noReadSubs.Add($sub) }
            }
            catch {
                $noReadSubs.Add($sub)
            }
        ...
        $results.SubsWithReader    = @($readSubs)
        $results.SubsWithoutReader = @($noReadSubs)

        if ($noReadSubs.Count -gt 0) {
            $results.Warnings += "$($noReadSubs.Count) of $($results.Subscriptions.Count) subscription(s) missing Reader role. Assign Reader on all target subscriptions."
        }
    }
    ...
    if ($armToken) {
        try {
            $mgHeaders = @{ Authorization = "Bearer $armToken"; 'Content-Type' = 'application/json' }
            $mgResp = Invoke-RestMethod -Method GET `
                -Uri 'https://management.azure.com/providers/Microsoft.Management/managementGroups?api-version=2021-04-01' `
                -Headers $mgHeaders
            if ($mgResp.PSObject.Properties['value']) {
                $results.ManagementGroups = @($mgResp.value | ForEach-Object {
                    [PSCustomObject]@{
                        id          = $_.name
            ...
            $results.Warnings += 'Could not list management groups. The account may lack management group Reader access.'
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC1NnuD+iu2Z+58
# ATFS2cuqJeoBpJCARQYIc5JDGpgWaKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIHQWL0ajxjjcXeA+4pC7ABEivnWpeu8dJk7g6ZMz
# fN6QMA0GCSqGSIb3DQEBAQUABIIBgLAJy5cJ1C85uphrY9nKGwLnHpTctttnH9oi
# oiC273EhyS24n8OUbwimEMVgyVQzoCq0AVfDyvfj1Qmcru6RfEWZuRGGxYY2WPSQ
