# CIS 6.4 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "6.4"
        Title       = "Azure Policy non-compliance reviewed"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $policies = $D.AzurePolicies
        if ($null -eq $policies -or @($policies).Count -eq 0) { return @() }
        $totalNonCompliant = 0
        foreach ($p in @($policies)) {
            if ($p.PSObject.Properties['nonCompliantCount']) { $totalNonCompliant += [int]$p.nonCompliantCount }
        }
        if ($totalNonCompliant -gt 0) {
            return @(@{ AffectedResources=@("$totalNonCompliant total non-compliant resource(s) across $(@($policies).Count) policy assignments"); Description="Azure Policy reports $totalNonCompliant non-compliant resource(s). Review and remediate policy violations." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBp5CjFzpE1AoJ2
# gayu0hF9LUGLyJs9XXWai8jCaAEO/qCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIAcU+0OKpoGy9lHkUtjiMlsCMjhkIMWG7SQPcvW7
# KtujMA0GCSqGSIb3DQEBAQUABIIBgJiX5tP6Z965tdBRzJNcW9DMXVNkwGXMtNWg
# MZI8CLV3VXP3VRQSuLoJkrgDvLtdo+4//n8YE/TcYqr2n0OrbIGZxCmqGZwcV/AZ
# vzECyPy+63Aa9+cwV2pS35ZzrrX+azQAytyT33SAyZ0It9SGUW8fUDr0ysQvEdMT
# 4/jMXwQrTL1ROESqB27aF07B8LG6qXhR6yZUDM9M2bHr1ZXC3GDpJ9MhYcOGEqFh
# Ft1TWLDvM3VE/eJNwRFjVqK8LFGneGztQ3xnpSOIIdPwEK0GTTjRDW7MS/KmO5Sy
# iH9ucPE5NRF7/pTS0XJ1+J+lErrUbrF46NXJTriyf/PzwJqNnnDtIwCciE9y/Mui
# Rasxb20PElO5xDxPWS+J9kI+9x98iiZks9PQ9M2JNKdAkrHB3u7PtCM8UI+txoIx
# 4iR7H9fXCK0c13hqr/8Lcaqj8DP+of5xb9AEBSrn8CFcTt4hxyJpXE511kz/84lU
# pUfbKo0sL1FYNbMQU0irw3rw9cv9RaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDnt+oKocp6jlB0Z/Q5buFre575keulFHGfCrM80x7cUAIGagxE11Yz
# GBMyMDI2MDUyMjE0NDc1Ny42NTNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NDc1N1owLwYJKoZI
# hvcNAQkEMSIEIKZPO6GElfnPNQgodLFcCRDZt3zZ5exw/Ippwdi5X0QDMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAa0I7jIc/QG9Htq/8WmNxMtCEu/FDAeXREWPX1zTEGofVd
# FZmiqK8sEjCFsmFUznOqZ2KAgZeeTr6quEjelNYqNx9ClDulcsTjLJ5L+ve7TNTF
# cczEN3Bwcg56+mjXx0+fHuKK06f7g9/ynp8sSR/r2JoSbrIpeFCcm9vV2m/Vs6ZT
# inifJ+U10akom+JtETbXtsC9GJH5DP4rPAMp9HWk2jim0LJCkX60c5BX465v/VTf
# EqlWM1/Z2gqRSPdKCCzzfLd4Es2EQb+U0GvS3HXbNq8ZnrLm7m7P+t0EQoBwpQE9
# Gvhg3AXbcY5GkqgoTu9sIb/L7IRFhtlhfQG1QAvWSydUsQyF6VrL7sc2qRpetf61
# ad1yl012+JG0uHkpvIr+jpcvws6Q3JrMJ9j1fVyWBDKZtJRNgm3nH11tcw5HT4tU
# T4s6eGnDLCCstrPbiR/KmTETTy/MkjC6HO3nFDsfKZDiI0ieYnxGN32+ztR/dtSm
# mlANUa5xbBOWwfgICGJEx9nU/tPQ59mjb7w6Enr4ULlG4lW2eb6EuIBo3k+MyKQq
# /VVDy2SSZxjhwP0QrwHaUdyJE86kUPfz1cFwN77U//Tv8sCl7R+EWhRyrwfeotve
# rKWbF/+I9vUldoXa9AnbbBHCRh9nGK687EVELpstrijBIaifFwAVqJg+L6HRVQ==
