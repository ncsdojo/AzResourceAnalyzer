# CIS 6.1.1.4 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "6.1.1.4"
        ...
        Check       = {
            param($D)
    $kvs = $D.KeyVaults
        if ($null -eq $kvs -or @($kvs).Count -eq 0) { return @() }
        # Use SecurityAssessments as the data source for diagnostic log status
        $sa = $D.SecurityAssessments
        if ($null -eq $sa) { return @() }
        $kvLogAssessment = @($sa | Where-Object {
            $_.PSObject.Properties['displayName'] -and $_.displayName -match 'Diagnostic logs.*Key Vault' -and
            $_.PSObject.Properties['statusCode'] -and $_.statusCode -eq 'Unhealthy'
        })
        if ($kvLogAssessment.Count -gt 0) {
            return @(@{ AffectedResources=@($kvs | ForEach-Object { $_.vaultName } | Select-Object -First 500); Description="Security assessment indicates Key Vault diagnostic logging is not enabled." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDhM79lGaZWzUW7
# 2PE5+qa1yjvK5DGuPHc51gv5yq6MX6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIDERK+3MXkYdv3fhbZTqy97cT3FYB3QdWdgWyxAt
# chvzMA0GCSqGSIb3DQEBAQUABIIBgHm+DrHTRk4sYXsDLC0hrp8W4WY4w+ST1VVm
# 5C8MERtEURAQBi08h/us5RSWsatTHF2TCNlgevXVnm86M59+KN6h5N0OtRBMP43m
# 1+/S94xN46W9rGgsIDp2o7PFAwcNLnae4nmLlCn9RJ0B8Nmbvvx3ZnbhLWMK9L83
# BNnDIFRcoNG6kIUNi0CRNcouu6kCaWxC8BJlLrQUkCw6GxiWrv8/BwGHkCdY5OBI
# 14Cgjq8fGIGDHEiAV0FDnVnF9cxNdMmBwUhNAwa6Md0V1Tu/KV+fZPD+42eAKdrr
# 1+YKOW2LenfUwcF15RI1YXmki2FbGrBmweCBwxpcy9tPNdqYVrT2hv4mJ48XJHv4
# RiDxoqJVqmIwSG5rReJqmu+M7+rsWhXSJZkKOaWkmExCzmromc4brXkOwJqaaDwk
# urebsHCnGzu++rz2rCMrb0sjDAApe6L63dRROVUuRYHKqmm+PxP0SWLd8KlsQW8i
# gHS15P9IQh3vJ9j38555sv83Y7uW7qGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCCgcbzSLMI3uP3i7iZcf1JW1D1vWIX3HA52uqwr9nsS7QIGaeiBPuse
# GBIyMDI2MDUyMzAxNTkxNy40M1owBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDE1OTE3WjAvBgkqhkiG
# 9w0BCQQxIgQgcoUGqbiRMQLJIPvZ2/a3vif5U+HKJnQYqLKbhwbqvtQwgbkGCyqG
...
# DQEBAQUABIICAJ8WsllAJPTnUIyONJcd3U4Mo6/O14mx7cTIfgBwUVKvP7+DoWlC
# CBe1MU9V+929NhL2TtNVH3EPlC3VuWhdHDmi0N2yHkDvfrYtv027XgHddWTQm5sw
# RSYPp/k2vUxa9BjlkPXAoBP4YsFbKC/PblcsrfJmnV4FYVWjNPD8EFbAdwuuPI8D
# 4UZrZqQUeLaFxK1N1lnnKdNzoGflGhAyJCLGERNUhae6/M9TfApHWD6+3viBta2Q
# KsRtjrk4Dnih+GmFXFk9fEVfEXYWLS+Z8d1YuxFASIXivVGjIAjGlJDi+I6vLmT2
# iT7sHO/ZwxfYSsFFwBAsqhAMhcf235wqmeVs+FUeTt/uVIHEY2D0ulCMlWeKaGYd
# 4hyXvFH8+TgXIM++RFfdtWUHQv8Jdkdv/lDScnA28VbqiJKawn2IM93eTBLGK4NH
# HC52FBdSeIVzwqbl4Mt5czlD7kh0ld9xhrZMtVDY0ciKDWmnLqA/eWEGvIIBHDGH
# RJiTq7paXsuihNPg4safUV6C4zCQ1K7FdTb8bTHwrxR4NxK12FO6bNv0wi7VLMW0
# 51ELDj2p/4nBDBHmBRE6zvyCgequ1SYfEWf7nOzDvBKPc36Mosve1JphmsavMbRT
# BTYNoobTW27Ih72zQXBVcm0QX8Y1hgVCqFlIK4Aydf+Qd/eX60poEGh7
