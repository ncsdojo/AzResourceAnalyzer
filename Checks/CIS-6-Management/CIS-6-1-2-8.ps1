# CIS 6.1.2.8 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "6.1.2.8"
        Title       = "Activity log alert: Delete SQL Firewall Rule"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $alerts = $D.ActivityLogAlerts
        ...
        foreach ($a in @($alerts | Where-Object { $_.enabled })) {
            if ($a.conditions -match [regex]::Escape('Microsoft.Sql/servers/firewallRules/delete')) { $found = $true; break }
        }
        if (-not $found) {
            return @(@{ AffectedResources=@("Microsoft.Sql/servers/firewallRules/delete"); Description="No enabled activity log alert for Delete SQL Firewall Rule." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBNwqiH/U67ULB3
# x9NSrl1/ZD5rXpgQQPFot/lxQieakaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEID6b4u1VJVMyoWJA+O3YpXUaLOeVzDuI76EbY/4S
# Pj+5MA0GCSqGSIb3DQEBAQUABIIBgEWK3MEZM64wqmmQ50F5Rn6VL1K0+AC3nWWU
# XUcmN+7JLEvWz0gG3G1j5Sj2g8V/nQcIU/WFdcgaozO6Pi5kdsp/sXKcr19whh6l
# IwBgRQcvGG2GZsaAiwbCa9bW7CXz71/70D/rkjpgsvWZ7Np5wMokjD8dTz/ZlEYb
# sfAmVRwDtsfCtnRBZRQ5zgaluUlwoD/2LBWxMoqsV0kSnUHSjE4R4PEK69pYjV4d
# spxVxGluHxt3s455HIJQ0nab3jFxPt1gBldaGtk+9so4IsJ5d2sFZh0AC9OPm04v
# U5ziYmYvCqJAc32fF4yS0weosevVQzqSPikUgF8PKEyfy/kAT0sHVh/9dN5kAmsR
# kLebIQZekEaB5yeK4q1lKeWLo03fWsoXnd9LLPP7PqgU+u3Left1qbC7JaaC9dG5
# 9FZCrg9x/jg4GVwfxhMVtFKClPqC9Ku8NLFBv5mGmvU1mSTP4kkeabu+AwPkH+u8
# zSUf57lvzRNbwSJemTWXV32XQa+uiqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDzqNuKHeVd/8XU8HzVlg+17paVwhBTm4YtGle4GNobzAIGagxE2ZXe
# GBMyMDI2MDUyMzAyMDI0NS4zMzdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDI0NVowLwYJKoZI
# hvcNAQkEMSIEIIG2nQ3N8e0it6BA1zb0nwSZtsf/ml5PBlsS5kpmOIWcMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBRfVpgfbfXbBqvYIzbSF4uNEwUknQdkcSeRIFX7MneG1bZ
# lNbC76Van23HEcMtaVmjne9+A+nj04hxevtz0Yc375RXuuDdmaSewn6ODJ7VI3Ko
# QrotV/WSK/dbmyeeqzhnHDRAf9a9kHBCKsPlZCNo+wKbf3wkb2pZesrGG7f4hr7B
# tR8zU7MJhLN/0LDhKUjH2NmaYDptQ4fmcj6pkoJi9ZRh+cB/FVXXkEPKvUN9wAkw
# ogtlxNa7pNW0UWgHhLrrIjDk4veSWz00A4pf9uDuTc35RU8IHrBbm0n6ZKnKERE/
# zAXRomLxK2U+6hXBNVTY8WECOajRNs6pUO2JglvlP56v6WZH1Ipl2en/AsPeyBy8
# w2G0uyXNrCNJucIWLyE2/KVeHxZmqj3JGV/CYeuiiDYuJysysR6MBR7+N3oA8oZ2
# mn1ZEu93EZ8ejQZW9PqIXkMVY+OAckYkDeU6bIbtTvRRSKUFGAewVSuosK6d4tLV
# aWGkuOKiroSKTP2SAM4sh6EE8W1sVZ5TVs/npr1oBZeGMKo3pjjXZV4yNOUbLG51
# 6HjL0pCs+N1m99UCpYzGV+dZiS29KiumyKa4x+CN1n+uFrhk2PR4UMe1zwWCZAYY
# bSgA2UfAmzVZDg/eY/3Rz0xC1fMuWw8XUtDJhycHvt39r5ANUAicL8w3NSoiGQ==
