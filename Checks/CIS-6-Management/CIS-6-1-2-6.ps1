# CIS 6.1.2.6 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "6.1.2.6"
        Title       = "Activity log alert: Delete Security Solution"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $alerts = $D.ActivityLogAlerts
        ...
        foreach ($a in @($alerts | Where-Object { $_.enabled })) {
            if ($a.conditions -match [regex]::Escape('Microsoft.Security/securitySolutions/delete')) { $found = $true; break }
        }
        if (-not $found) {
            return @(@{ AffectedResources=@("Microsoft.Security/securitySolutions/delete"); Description="No enabled activity log alert for Delete Security Solution." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAivP7QF8SJ0Eau
# Q2V7qhRvdXpWeiQv6HRno2s9udVIzaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIJ7Q56pn+ll6EKerN8RviTU7ogAxBrOJHps+jYbI
# cuXDMA0GCSqGSIb3DQEBAQUABIIBgJ3Nv8NlDXmXmgOLKY5yZm5bumUaEUEvtXL6
# sHXImKcHckczzjzalL+QZpHTCUUMm4G1DTrAN1p+MPvo10paa9m5tf5pajO4pOKo
# 2hcV3kck4As3ZqiEtOsANrH/diVcKppdmqnQCTnTQC9Y/1wG7gJlReeKKwjeITUC
# hVcRVyhO+A/N0WvFHDMR8f6wDe4t7oZVDkhWO7/oPO4+mMB64f+dWQuNYqUD+T7K
# OhWLQW75niB/briucCy8+UBz+5yQ3J9rn3OxHWu9byyTfE+5gtp4jab4Z07auSpT
# 4G4gX5Cnb3jBNmE1otKl4+nJ4KiRFqBtLX1dyJqCKanJj+RUcSTjNDaiznInaCxx
# roQp7Pceb3H1pSxMIVMkAfSbzxxhMWaaHPeU9QnGAV1Qx/nA+txXuUgxdx0UB1jj
# Ly0o1Gyu/uCbhDKlDANZp0H3OHI3to5EBuaCt1luDB1H8yEkZrLAgLcHPvTRZDcl
# /X1xX378kBcEiSFxpMkH2pwobSzXEKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBUIrSm1S8mX2w42F9VRanY+LPMlvdvKSWp9cX6erkguwIGagxE2ZWh
# GBMyMDI2MDUyMzAyMDIxNS4yMTJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDIxNVowLwYJKoZI
# hvcNAQkEMSIEIKMCWiqr/lKnkAw3+LLEmVVJfZENqnbXqMBMOOqwTYQuMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBy1hp9wQFngqWREwGt2/peUy2N8fwoEPi9MCtku6vfjvLA
# UOXkmA3RsJdOqAwnpNKH3X0DMIgT5TEIysoQ1JVC0jDsyS5yBe49y6Cy1D2DzV4r
# ozAYl76kdAFKQZ/75Sb9clESMJ2gzKG6O+ybIpEOPAky4HSVkMmeBycCHwqAdh6O
# nng+GeR/XXFSQ4jCyIezVOHNUGrIm43QMsCVBPlV2NuJZ7Nz32EkVs1XgL2RLLjb
# UUinPvKyueinNN9NWU6/lDdbR2oYFGbBOnZjCNkfYevaIrpRzc+4M3fWRWDP0Yna
# J6d+W2r3zjMl7sxZ3AULGjbqsWNS4eDxBCi338ledyWK4e5L9fikLrX8au/vqXMf
# SaafWJj+MjLAc21TKupwPoDgBO+62pnvl34KQ3Nmwn9Y3DnZPEk6PwfqnhobLU30
# ad1E3T/zdGciagd62at2v7k/IC8xWfFZx1KBMcL6Oqn3aHpgHEe8zui/vi/khqq0
# VeIoXDeiEkm+dkkXlA6TAbqG1gYNgfaTuqB75zKHtQNQtaEthlsOWpQBUotkgr3J
# JbIfKMGctNnqbVCrQ0n1AVpcKgeRU4QqjsKjUIkhgK4dZS8wJMHc/fL4OaxT4Vqj
# 7UvsshOOiaT8w3x9zEYCpeu1njecVr3pyVpA6smwlr889NSZA5N1kZ327AnMjA==
