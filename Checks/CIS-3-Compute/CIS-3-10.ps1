# CIS 3.10 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "3.10"
        Title       = "VM OS updates check configured"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $vms = $D.VirtualMachines
        ...
        $bad = @($sa | Where-Object { $_.PSObject.Properties['displayName'] -and $_.displayName -match 'system updates' -and $_.statusCode -eq 'Unhealthy' })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($vms | ForEach-Object { $_.vmName } | Select-Object -First 100); Description="Machines should be configured to periodically check for missing system updates." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDgQKHdbaFEpQqq
# k1lyCaNH5AtIiAHIUnpmGjbFTQ35eKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIEOV4eE/VBh8h5/bbVbON/s8Rpzq4l4JzQfOWMK5
# BiEgMA0GCSqGSIb3DQEBAQUABIIBgKYqdZFORXYOaMVzWAXUpV3uTddmvRmQpCMe
# K/xpkshRWnjbUWXxQc9ZmyOO4Ap0fpW8HCwhaXucbxUL3lCfOdKBjqbkVFcB8G4q
# RSSrMieSSrZi+VO/y9LeOeoLuXj4+0gi/9mAoOmlU7fZgCd0E0iJM79DU7CuiVQt
# oL+3IgQLgR0M4W4Im3n0pr3wj2WNKncO+LdO/8MXO8t2wnZU4+fR5iWxaWeTyeGi
# ypcV8XtPmFa8P/0W626zAiNNOtxrPGWSGgnnN79uMNxu7lJqlYVfa5SlGTalf8mY
# RSRYodG0Hn/F99pv9GJk20f+DHQ84zC7ZxL/jO50DSj9THxCdXXmQsk06X3nb3T+
# j5JG8mztvtWw3mp/4B4duacDF91MkDEEElh8S4fwGt+f0Inxp2Tk6tLACuofSYJw
# p849ado2wp6oMN/i8AHrjOmBL+HLobWadH8PmieLKWWTDHR8xNoE0z7Goep08gIi
# Zrxqlq2iJPv5aFuJtOo94OuTFCqGh6GCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCCEtdDmuZP0qUJK8sX6qa8+AfviEYRFYMZe3kiaOhCn7wIGaeiBPI82
# GBMyMDI2MDUyMjE0MjEzMC4xNzVaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0MjEzMFowLwYJKoZI
# hvcNAQkEMSIEIPl0/esKkLTiXaKKoPoQadTJ3yIOdmHT7IjhYF+aL+lsMIG5Bgsq
...
# 9w0BAQEFAASCAgCnZCReMIdKTSGSTF+YfTb+5b3RPJQd9MH5e0DZ6UOO7F1STJ4t
# 0O7HxqOPpwlFkJTi6RJcG1N4wWwnoFyHooAXLfoLhtkIKKJKMiyfguy7Bzp8Q+DN
# myIz1zRL8qkCBRPwkqHl52YhPdVvXzWxmTkA0byr9TCciY+cgi8Ao/NziYgsRFwU
# nI+gc3Is9FuPkbU4DrwaRoKWMRAtfAQIbzxamC7vr188XganI+qidAD1yoL3QkeQ
# FRj3Vlo6BlGp3QT4p+qI+4Mjvu4EcUCciTDxjs5oCM+OIMUiREOlgRI5MmPtN/N5
# H7Qhn+W6tQ48+yNkSxepSF5HFdtnnHKh78v9Inojo0vXL9137NazNo6zuuqM1i9b
# NRARTrNVE8pOKdvPwY+0h+Pp1M6kxCYmWIsAaLCZ+Z7l1gcSCjVQ9BIRxdZio5Bx
# IJEpBQe+ZcoiTTg2D92b6RKqMWJSU94uSiTMeYbYLLcokmOTFWFQrVpiR+mYwCi6
# JabjJKVbNZWTOa9TiQR8uQPitvVqzM0Y8URguomFC0WcDtVzvsFhqwOL/txF45dO
# BbIB0ATB6PPGcyQBPhKARSiXqMeypN4unKP1m4Wcllko7S2IRhTRA7WI/D7D8/eD
# tLuUK3suSc2e0N/REDvz3iO9bY65YXCdLVDJM/liEvCVPS7K6AV7B6nX2Q==
