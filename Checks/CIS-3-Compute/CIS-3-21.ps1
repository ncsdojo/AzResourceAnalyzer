# CIS 3.21 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "3.21"
        Title       = "Disk public network access disabled"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $disks = $D.Disks
        if ($null -eq $disks -or @($disks).Count -eq 0) { return @() }
        $bad = @($disks | Where-Object { $_.PSObject.Properties['publicNetworkAccess'] -and $_.publicNetworkAccess -eq 'Enabled' })
        if ($bad.Count -gt 0) { return @(@{ AffectedResources=@($bad | ForEach-Object { $_.diskName } | Select-Object -First 500); Description="$($bad.Count) managed disk(s) with public network access enabled." }) }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCh5jK9iNTg5v0H
# GcnZE21pTfFm7Y/5Ud6Fjhy+6MAU1KCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIKuSlprNIfsKExT+DkRo1QPbKrCt7wWKjCk+eKWT
# KbYFMA0GCSqGSIb3DQEBAQUABIIBgA59SgeTfDWG0XDT2J5dzI3iAk/vIdlDkCL+
# iFYIZTxCGgl030qW0jcUfFVLIKVPDsFwtu+vVBKA8h3Dn+hb+fW6UNHvXHyqoHdZ
# jZ0a65dRWFj93lJex3JcOA11VQaZv3kZ8nbpYnf2i5rfbmjJQzkbFOTM1ZZmv/Gl
# tlJUjZSF+WKRjeXO6nfHGZlQuHQwAqLxQkBDlTpgDMu6lCCSNtT6Xv9/vl/UnvNC
# jZpYWqCupRzajt/x9wJrYxwCApTCSlDe2mxGM/2Ubs2oFM6as9E/OEmZtBlo8oAb
# U4wXTqD8UEvV3m/0Np//inG+iqTIee5905kpdHjFF7vx6QXOIcv4mfy+i04lL7vq
# x91uIqgs4Btw4PydfJareSpV2NI8XzxWUULboTaDPL0NsQgU7DPinLwBYposVYhV
# wJSXAunSnDZ5IN81oHT/IOtzqC4/1Giyvq8CAKTZD7VOhC3ZgqhDfp5CMQRGAyFA
# ujGNKaurV0UWR/LTfla+/vAQuF5BIqGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCBeY6UQ1J/nNEvae8HCrnNCxq95wF8L6PzpTtVsDSg/CQIGaeiBPJQV
# GBIyMDI2MDUyMjE0MjQ0Ni40MVowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIyMTQyNDQ2WjAvBgkqhkiG
# 9w0BCQQxIgQga1A5DCJU2p85l4K/Y8+iwbSgUH1nTLLeZErKgIxFPpwwgbkGCyqG
...
# DQEBAQUABIICAFtbovoUqfozkPEvOZz69gSztDEBT7hmRIFf4U+XL2LU9UDAkAOu
# kAjB7hr3qJeckDHYhGMGZ/HbbW3zJ/O7gPEuzMr7OuXDxfjm382e+1ZJ96Z//nyn
# e6yTz79+XXlas2BTC/4DzuN9RX63XJ4Hxe9Dy9QTWtC9718bBkSWOlcblaYANxlw
# jtFYrpNNX9KDKNPtqLr6BGwqmOSsww03bHztg8NaxtxgOKEuqlesPXZ2Wv4kSvrf
# PbH+D7UXFxJx06GjPDsSjk/ONHcA2+pal89W2pHQvdXXdb1rrzJ7ELBnCdCWnown
# CiQC8QAKjC4McWDP2bNi6wuZ9jRuBcIfZ/Dr4v87OJtqTFjFbqNiZgMxiojr/ajE
# 2FqwKqaayvOG/Xe1iX0F4MbFAYSoyVzUa3L1e4N/ijIZGKChDjT4dttRlojDuQxa
# OL4tioebqP/DOPoCZjoU4hZ4cY8AmxNFUvK60MWH9Tx1axEXv7HrewVgr1orhris
# e7c2X+D+rTm7+fKAA8dMU+btT+zTZsOq/RmQfVUalSyxetqoQRiR+6FFOiZkigHr
# VObCT+jXicLCMREoenEK193oCAYGi9MyCL4crD4QgqYwZXaF3rh8KKIUw04wMKAn
# t9Eh5WCrbxI113Lf6xxC4MyMdRUhGdR7R0LXNrZW/7jyYX3sIOfQ7jZb
