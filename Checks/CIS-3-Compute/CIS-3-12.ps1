# CIS 3.12 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "3.12"
        Title       = "Secure communication protocols on Windows"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $sa = $D.SecurityAssessments
        ...
        $bad = @($sa | Where-Object { $_.PSObject.Properties['displayName'] -and $_.displayName -match 'secure communication protocols' -and $_.statusCode -eq 'Unhealthy' })
        if ($bad.Count -gt 0) {
            $vms = $D.VirtualMachines
            if ($null -eq $vms) { return @() }
            $winVms = @($vms | Where-Object { $_.PSObject.Properties['osType'] -and $_.osType -eq 'Windows' } | ForEach-Object { $_.vmName })
            if ($winVms.Count -gt 0) { return @(@{ AffectedResources=$winVms; Description="Windows servers should be configured to use secure communication protocols (TLS 1.2+)." }) }
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDnrO+XJWgFED3j
# opnAwiiLYRKSqMGU0mmGw7qNQmByWKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBUwki3VRvmghLcRVxi7C7/9m8KWQQljAvLcEwwa
# JaL4MA0GCSqGSIb3DQEBAQUABIIBgHmwUcf0xnIk0zyPYS8Ec820kzXx78edkpfb
# qSqZ1UGycNM/84wcKmcrdNzn5V6jnE4lqIMy5FdkhR7s3kTdL3cSpGnWfGf2IP0g
# JIdweWcK2iG1llyVRftth7XV2mMwQzMEZYcwc2ARdjkNTmyCGukwWoNyJGIt1dFJ
# /ULh3428DNltAmLwJuRX0edSgVfgzuogvVKJis/oTzxIQzHda1JqKiyseLPM3MOG
# XBgljvJZd4gKiMvUDnX0/dlDxC0pr5vvy0NWIkfrJCqFLL0B8932rFX3p2UGM5zY
# t1B+XAyOrm+Va6TXSRFNVo4No208ZC9LMdbnbhqejDVK/zmO4qBta7H7u9330kiD
# 69ZbPW6kA2koIrvI82GxINlIJERGOjL3QPsAuwEZekshFaaQ0TpY64422kGKv9vV
# L+EKeuJLjCgORI8gHRFc/HcZyvYtM+Ry4K088oWtBm0XLeKcxxuLp3RZTMWeWUPU
# +aJw7bVJLNa4BlyILJMCY99VW1iblqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCQXzvdPXlrXp1tEfdpK5R6HsTtJsfYBGXgJlKifrOL0gIGagxE2Ygx
# GBMyMDI2MDUyMzAxNDI0Ni41MjdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAxNDI0NlowLwYJKoZI
# hvcNAQkEMSIEIHIrgJw7pcHNMByH40WHBqkZdq33vFWfEgkEFAgUHhj0MIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAoNay0oGqNZARqA1aXtapf8p4O0FX1DJwROoDHnR/AzKaG
# s6oHdQFjgdSo7Ja3keLcQswip1uXqFO85IM3SCicH88eYvAa+JWu35LRBfvP1ef2
# 1ZglEYcxL4CbBXfQ3fpOtbwrYgZjTSGFuzD3HzytbeZTOOwNcGW2f91ojJwB/TuX
# 4Xnsq3OjSMYl9qzAb/U0C2RYsba8YQuCgz0HUctMXBw31hFKYQHeOjT0dFGfT5g4
# GVTe7gfoNfY2ISpEFNAz9DK19mFCATedrfM0FDFwcWYVB0Al+slCvTn0nnkqYHT3
# J79O23bRlozCH3gMpgmDjfSSL6wdPTvbAJIL09bOpej8523fxRPf5C+fbS1IqI+h
# SD/jsKTnLOVqUv3ZFqTuPP8du9LwQ10h466dy/YDGLSpNbhl+IQ/40HO4P1qHzbX
# d7KUjJ3xIdJLwq+IeWCAdzflp9YTmHg2URFJwWMy1ZY/DBMmnTTDeGRBCr/hxP2t
# ncf/E+22QwxNNg3jQlUVWUdrdDB3fnvMiDSRTxu7KPQP+G6MLwVVCLE9neeejwjw
# xLe0s2FTU5hRCKSUwmRXVWClkl/EdwJbajUC4QzHh1mj5VBLqas0CuTgKTVJJ+Dd
# +RNrO7uGEk76hQrdalIMQ9hZHTOGZ3Cwo6ely6Q4k4dCnmKaXd8ALE27x5mGrQ==
