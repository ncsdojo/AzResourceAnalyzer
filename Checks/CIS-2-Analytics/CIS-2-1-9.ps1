# CIS 2.1.9 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "2.1.9"
        Title       = "No Public IP enabled"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $dbx = $D.Databricks
        if (-not $dbx) { return @() }
        $bad = @($dbx | Where-Object { $_.enableNoPublicIp -ne 'True' -and $_.enableNoPublicIp -ne 'true' })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.workspaceName }); Description="$($bad.Count) workspace(s) have public IPs on cluster nodes." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCxty8KTSAqtzds
# vIaYYL4lXvV9WempyjbAId3cny3UnqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIDvVotWu5AlxA4j6OAa3l284RzvbQTeNUiayhrLa
# nnESMA0GCSqGSIb3DQEBAQUABIIBgHttjV2kD5H3KPKgVnj/Qf0KHW5vXsI6yB1h
# k0rge9wpBmg4Pbywv/NQTjaehWTNZB3M4hu7GD0sm6/5S7R/9qtkGjr2V4LcLY/U
# zRixnfLiBx8U3ZOrhwNUvZSmNHACQGs2ouKrskFF266USiOFc3KvT7FjAwuKuJoR
# 8qoBq3gag/TVIvJMuif9zzJ/zuHE4wyJ6q8kSM5zPMl/oSv63cszXzK9NjwFiJtq
# QDqjKGBjwP+K32FbgPw2AyOPPl6qCZDA4VBxc3hGFUoEfbz7i9Ltxh22m4YWeOKE
# 2ScrHpsVeLcerkpEVa8JVpUSFnLlR+Z4LmEX8DtyQYx41mHIuy3XZKJpi/PPkMEa
# 26P4IQY3e4QGa5B0Rk2gctLf9HAx3uG8MO72pyREiOraNG7NeXL3O1sCxo/5gBAU
# R+VuYpowv03m7IMbHmvXmo/B9PZv0ruJ84ndcNQ36ts0ib7fQYG36l+rx9oyTOKz
# sUd5Y1IbzS/1l7/ENZ+3bhZHxEa8k6GCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCC+NexMtH0rySB9Z/7geYq72s9og4aarRG/1frT0mMjlAIGaeiBPI7z
# GBMyMDI2MDUyMjE0MjEwMS42NjRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0MjEwMVowLwYJKoZI
# hvcNAQkEMSIEIPPAQMrv7YV36eSceNBJlbvsLAij0UZIspNDe7PD7Is1MIG5Bgsq
...
# 9w0BAQEFAASCAgCauNrryltNe7o/tUdSkpZ5H3Ra97ave7aZzzRMiOcFwyq3qKxA
# et52XV+TmOASI5JnbPXbl0mqClBpASrQW+htof6DTi4Ix4PdXDXDamEVP5OYrDVE
# Tcr55ym4nJ1JASiMfeaARYcYY3AmgllatIDpfH93X2F3nAVQFjzMYpEZ2mXVGHf+
# JOkYstvvDe8sJrTKC+NB0tzqtaeGiJj0uuCVcxbt4oJaGl1MtQdh3EfMotYZmGlT
# raFnE7dOKC6OJdkj9/T+jhkt9Dji1iNEURs15kB86205khgLWCRDljBWMyxT3z6u
# fzqqch0xmqImax8amEFxVPaIuLSYOD3xLfQJq6TYbY1plLmWwej4ZndZrAHUo/SB
# 624xQL0cKKOxhCtfi+bPOOVHwMlboqWkNyQx/hoUz/qIDtRv8d+vB2eH+YaKO67z
# rvh7zC13fYGuZlrxBbPTzl7ASUGXYbgPH2JZG6AnHw/9K2ABPi/lAFh2hc3uHA4y
# lyG6nrKy1vG5cv9DLISoUt4vCEa5TiPPJ5YJ3xb040GEvBg4yePnQhSYS69rX435
# sSigysrHw6Po2uZ8VUsx22QG3dpyyrZe9liuVxbK3MwvjZ8BNyeUJaWehtZ+z4Py
# ptN4O/VHk0sA/yeIc85iuZYlRl+f1f5vwMxQgXwjmdgLPqqA7wbTmYfhvA==
