# CIS 2.1.2 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "2.1.2"
        Title       = "NSGs configured for Databricks subnets"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $dbx = $D.Databricks
        if (-not $dbx -or @($dbx).Count -eq 0) { return @() }   # no Databricks — nothing to check
        $vnets = $D.VNets
        if (-not $vnets) { return @() }
        # Find subnets delegated to Databricks that lack NSGs
        $dbxSubnets = @($vnets | Where-Object {
            $_.PSObject.Properties['subnetDelegations'] -and $_.subnetDelegations -match 'Databricks' -and
            $_.PSObject.Properties['subnetNsgId'] -and (-not $_.subnetNsgId -or $_.subnetNsgId -eq '')
        })
        if ($dbxSubnets.Count -gt 0) {
            return @(@{ AffectedResources=@($dbxSubnets | ForEach-Object { "$($_.vnetName)/$($_.subnetName)" } | Select-Object -First 500); Description="$($dbxSubnets.Count) Databricks-delegated subnet(s) without NSGs." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCk8f6MpC5bLT9b
# S4QrYFw02UbvnzGekNYohi4UF9isRaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIN/BhWVYZ5NtaJ1scus24WnaKNebMt+g4leyQ6Tm
# 5sAOMA0GCSqGSIb3DQEBAQUABIIBgIhuLqRbQpF9HK5dwHjH5rt804IDd7qb1c+l
# ytkShQ8E4YLYOFbyoy/HMBmDrDpcGl77Wabd+eSDEQ97m75CA+YX8VrXx1qx5vGG
# OyrNAxx4+bUQoGdXYv8bBNeICgvA/YYUqWu+F57uLin8ffgFVHxtJ1nfKSHSx60G
# YkhrgsTbf9Sy8YwiOOyu86/f5cZniREkfTPRdmM6UGKoR21FRgXvovkJ3zZoOBby
# 4uoy9fSU3udx4CU1O4pZBSjZcUSQ3jj5ozL29t8qIA7a/Ex5L/veiNAN3ytwcHU9
# 5g5WoF+qZW54Sbk5P+55cn2RnY8S/Tn97+7UXL7q4glrLKxszwoPnxN3q5li2sHz
# HYsoP7i9of7+W8xXbI4kZc8TczSkS012+6hetIvkPAfCSs959KuLQ2j+FMR8Z7Jl
# BfQhC+JhzkhGC3YQjWgLmuff6M81bMOJuTF9BZEHhnGH/Ku7biPs8ohGbW6D7mXc
# TwcITNbrIdZHeDs4pew1UYnCr16sf6GCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCBVP8soUxp6+er1bRAqNrQu7pS+RIiPzORdewenVyYfLQIGaeiBPI6A
# GBMyMDI2MDUyMjE0MTkxNy44MjNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0MTkxN1owLwYJKoZI
# hvcNAQkEMSIEIGtcmAcsxujkqlrPd5Hybhoemka/7ipkxPT9nmZuKg7ZMIG5Bgsq
...
# 9w0BAQEFAASCAgB/QBh3LO0ZVAl7shbT+ZB4mQYMTa8Clt3tLdoLCU+kf8BCGJI6
# F49Z5UA56gga+zXQOccuOfuoCMBLnyhSenMpcT4kWmnB1e9xcETjccfVjOvDdPwN
# 9mfFKBJgXWj62OF5QFxnuRqFHWVqH5Od9HyPTHOKiLIuCDsaNEE0rtAqowbqFsTX
# fHVCG0/jtrj+W5MwP7Ce4INcgBfGja5tuxbFMbu4hL11F1Fb9tCXrWiz07xZKBUz
# MloF9YF7qjIywLivz471BKzaFwXULa75S6M+ycVD8gszS1k1pjojTn3sK63FbLYy
# pvGeXv1y4iSfcn4f1wqRtJPl9vvKlh+oPtJrFcWSLNO9G0WR11vwg7j99N5AIM/c
# rQCHzgFWfkO3I1TJyuGo6xBDUxxGFQ/TZfwwv1Fi0MMQGQyOXbo8DgRbNdKaihgJ
# BzncG313TkBIU541SDFhqXX0UxGAh8g/Cg9k7g/8hvMtc/ddDw2FRIS4ILFE7FTE
# T4F9m04nEZ+iD5rDFhc42J9oBLYByYU1BRQAeUXFubbG7Frg/l0dNyT0CtW5HVq8
# 4Dk4byVZ0jyxuLNHaD5rULP/txuECmGVlM+yG1ve0Q5Fd7BRK3J5ZyVdBa0qt38F
# pqmkoMpaL6wwjjc8XYFx/UUDU05GthBnKqgjwrjEYliM9vzVxrFQ2B2MRg==
