# CIS 5.9 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "5.9"
        Title       = "No broad Owner/Contributor for service principals"
        Severity    = "High"
        Automated   = $true
        Recommendation = "Scope service principal roles to specific resource groups. Avoid Owner/Contributor at subscription or management group level."
        Check       = {
            param($D)
    $rbac = $D.RBACAssignments
        ...
        $broadSPs = @($rbac | Where-Object {
            $_.principalType -eq 'ServicePrincipal' -and
            ($_.roleDefinitionId -match $ownerGuid -or $_.roleDefinitionId -match $contribGuid) -and
            ($_.roleScope -match '^/subscriptions/[^/]+$' -or
             $_.roleScope -match '/providers/Microsoft.Management/managementGroups/')
        })
        if ($broadSPs.Count -gt 0) {
            $roleName = { if ($args[0] -match $ownerGuid) { 'Owner' } else { 'Contributor' } }
            return @(@{ AffectedResources = @($broadSPs | ForEach-Object {
                "SP $($_.principalId): $(& $roleName $_.roleDefinitionId) at $($_.roleScope)"
            } | Select-Object -First 50); Description = "$($broadSPs.Count) service principal(s) with Owner or Contributor at subscription/management group scope." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBjowFsIraudndz
# uOpPqMo56/1QzadugszgtLUVPfY1aqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMLXW5b9ltn/vj5XHbNTbMfjmIxl1kGzBnvfDvIC
# RLydMA0GCSqGSIb3DQEBAQUABIIBgI4LZd4BwfPzZ/cwc/jFiKI6R2YLokhaVNAC
# KFAcVF1RFd9lnMKhqlNqoSWYFgv0l+YLKe0bByP9VwCIv9cKNrrADoPLFRnWtXZw
# 5r1Cpzy7eZTz6LZTNrQ9qVm+Pxad5VNYIAU/1om+RMIZ/FrcGAr+wteUc7XaHIK6
# E4HKw1X5/KdZ9RV2pAS8nH9XCSx88Jav6fJp6v2rw9Davz9G2ZIb0JxmJX5H7/lU
# jrNROrRyCWRvATz2ajttNZ9viXPZbAgfJL1mCvLTSOHeWYblfPwaUHPnoxndksxz
# UcPnO3qTu+UW+bQvM9pdJ+vAKh4IACKI4206zJ3wlpG3+LyDJ8llANIHpBDtv1Kt
# roGUOZ7eLDxfJfY8xYqC8xM4Y80TRGHyI3zv+td+vrOLbb3HuT2q40cWqDALtuvq
# J+bspJU9YLdmMpOn2s8XPeaXmONfQENvWNWdQpVGC0uXSy6adtzdRuakKcTIoAom
# 7bqi6lM/7dojwT8sUtKYw/8u7AAY+KGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCArx93VYc68N8n/Gi5BZtsoCxhuzA8uafQOPhU6m0q8CAIGagxE2FXt
# GBMyMDI2MDUyMjE4NDUzOS45MzlaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE4NDUzOVowLwYJKoZI
# hvcNAQkEMSIEIHYk1MQlyGkWORpTBpmEVEu4FTRhveo2ZqIB8NQF/xYxMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBOnxqSiymimoG0h+vcQgT2UiDKZLPsqNzvD8G/VfQ51r1Q
# IY1qeG0sNP2Hc+9HYp1WexXrlmAib4ZK2H2yw3UrLe2zakNqMSqGJ+xKfnkntjxC
# JRBHaXqcaQL3wzcxDe2XnlaiLylAD+ahLiLyvYc2ZRtjAkveeDlZX7F7U/XZR2fv
# jJwCHSycgNrMBmdGprVPQnyo6lA4ok5xKV114G4Q4/cnK3+TfnnKsczh6v6NUIrs
# Jrp7rBQr4+oLUNil7FVXCu/BHHsv8l7IZHA8gpf2IAsjbjibU5jOWRvybaz/EYoW
# WXTHP4odyMexHoeUoAGvUjYl22n9zhPgJWSzIHZpMjBzw7hIUcmxlIIcg2xH0qKd
# Kil4gRPsTpME2atNxdYBUyAvZSRTyjfqF+729bjo9PFabvvuYuelRfVUYwhQRodZ
# bvMk4qvwM3lFqxGYj/ULeC+IySpDXuHBffjdaN6lb05qNcrHDF3NeepntrMISVGG
# tw9zNO22uF4z98XyaA41Z7TTqp28ggCpYoZnx/YpbLWwq4RbR6tZDbvEKIm75o96
# Axyfbt4i3hyP/a8WkhZy+nOhUknOJS+0y4omQs/UQmKF3lylJ8l91XsZfpGZDBVs
# Exbb9MT3xPgtWB3P82G1saJXtRTx1WH+R6/Xs93+jXWOJZxbJKKsjq02Tr5uVA==
