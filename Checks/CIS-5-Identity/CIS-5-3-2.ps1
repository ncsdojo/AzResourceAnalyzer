# CIS 5.3.2 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "5.3.2"
        Title       = "Guest users reviewed"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $guests = $D.GuestUsers
        if (-not $guests) { return @(@{ Manual=$true; Recommendation="Collect GuestUsers via Graph API to identify stale guest accounts." }) }
        $stale = @($guests | Where-Object {
            $_.PSObject.Properties['signInActivity'] -and $_.signInActivity.PSObject.Properties['lastSignInDateTime'] -and
            $_.signInActivity.lastSignInDateTime -and
            ([DateTime]$_.signInActivity.lastSignInDateTime) -lt (Get-Date).AddDays(-90)
        })
        if ($stale.Count -gt 0) {
            return @(@{ AffectedResources=@($stale | ForEach-Object { $_.userPrincipalName } | Select-Object -First 500); Description="$($stale.Count) guest user(s) with no sign-in in 90+ days." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBNvYmXrPao+JaH
# pJz1GKErbGMLCrH8hsTDNCLJ5GwKkaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIKbGOvp/3PZvS7WcJvUvQDUpTvOQs1Jzrhc9wGmX
# X6k7MA0GCSqGSIb3DQEBAQUABIIBgFoZGWFIRy2oqBpzkXujOrTbjembT50ebX/t
# 4DEacA3FeUrkYVpUQ9MMdKaIAXONdXHtwAmvuKaEI5kBjRIZrg1OuwzzrBTCt3jH
# mdAgN0Qg5YhWk2vPlYgnabd+JLQPxxfMTOUsb05h1ccE6BDfX2eBCVAL3nRcPfq6
# eYvF4Zd72yZp94e9LjZlFNH9NpnvdN20/xHNc2DGzH1hegPfQCRRW3mNq1o5woZX
# xPq3SGdtVQmaVlSMKUmlAht0dNmTERv+Rl3E/h1zznswSRe+nIV0OQZVvzMbji5h
# pq+gSPyQB0DYihkgk2jX8nRMjMhl25/5GXGCFx+vQxh+y78NCbxfh1eCa3D7SWLa
# 6SxX/tyzT8PFVighEU145ssEZcgse2Y3+unLLyan1maiJQQVgBWiWGjX63wefOK1
# 4PA4mA09W1izLwsuwqGwjuSd7Vis21uar2uUPgl/74USaljWhFrUXQX0+MK93msW
# Z7t8o48lX1w5gmjdVIiuK7ZgdOP7XKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDYCjcKCzXecRH1m9DkI9snJ5jubw/rgZ+BZLwO6iYf9QIGagxE2ZIm
# GBMyMDI2MDUyMzAxNTU0Ni45MjJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAxNTU0NlowLwYJKoZI
# hvcNAQkEMSIEIMJvgqZ04juP5R8fQzvoMkVmBZb8i+ixm8tupedl5iXzMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgA3QBfP+E5mHbxt3K2uwfTOr+RljJdHGeLcNZWUQ/xHIxmB
# GwRiOngmZPEp1YppX/oQrgC1pVj6nKru9LuLjZ6WyuWY9evHzd15eBA5iOTbWHE8
# bga2pRvJIDbiKqPcl4JFR02zO7P8qC6dhD6hGT5TXYNfg/CdagGK7DE2Q+s120vv
# F8100bpW1NiqbRJfohusAoIPsS6srzMYXGfZBwQuOZD0SGpURBn7wfU+w9SdBciu
# DS3aIhTyZg+yza0xng36MOdTdGt38q1rVgrZIxdzJVWBoaks3ckBYWqpeojuIeoN
# AOD0VWlQqitOFJEBl0QCLdGKYAgeOqhAiNTmrw+Sxa8ILMt/TYtGLr4ZWzYkzaYK
# DCi+9kmo5xmjKdkr+DbdyyYqumz4zzIXpt0I+km2PElqZ4VytPQSFlkOZ56+jRda
# vHN+ThzWH5pmeRmEkrlBbwq9Mzs1bgMv569MCloAHT4WEU/2K/nJwT8hYJWbGx3h
# L+g0xfaHKAUk72juO/uZdMrd/gYuPscXkLV7oxM7zexsEit4/egSfQ5LgWOqWKNn
# ID2hPsScwpoIgHoElGFYSjhXvQEHpn2U42cMSt6ZlUqmQmA+l8IckW2vlAmakMCz
# 8AI/e1qeNT7cdCQQe9tutLws8LO3aeCKon5CsW4llPydCOqspu8/TiApMh8Btw==
