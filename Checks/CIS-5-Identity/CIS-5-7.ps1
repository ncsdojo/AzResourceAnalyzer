...
@{
    ...
    $rbac = $D.RBACAssignments
        if (-not $rbac) { return @() }
        $ownerRoleDef = '/providers/Microsoft.Authorization/roleDefinitions/8e3af657-a8ff-443c-a75c-2fe8c4bcb635'
        $subs = @($rbac | Where-Object { $_.roleDefinitionId -eq $ownerRoleDef -and $_.roleScope -match '^/subscriptions/[^/]+$' } | Group-Object roleScope)
        $bad = @($subs | Where-Object { $_.Count -lt 2 -or $_.Count -gt 3 })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.Name): $($_.Count) owners" }); Description="$($bad.Count) subscription(s) do not have 2-3 owners." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAAzJ1+iTa+zxXL
# 7eJiaCJ0zjYujl+SnIFxUfXGVPs+waCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBdypCGMnCm+WqylHgPdnlYcONVJ12zm2yXlLAOU
# ALNKMA0GCSqGSIb3DQEBAQUABIIBgJolYp7KMCW0DonZSjAYkWOEM4IBmDuhli04
# 1x2xgspcM+zCZQoQK5jJdEI2Ophy3gMiYJX2Rs+DlkTb6+NL5D18SjtRbWHv0QEc
# 5BO4pNuUd5VjrV0Lahkm2aHHKX6VI3PykYSdeinRv0DSnsZ3e7bGVPdMXPhhz8wF
# PKSZbme6b6pMqBursfBxxykfda7Urr0r3RShQL1JdsUXXOC/SNgYvkomUJlVMOi6
# JtWzHx74qdI3PNY1nFPO/uLrRuKdpYmMNREwkCM6+AJlUNbhahHDDSvwfS72tOTM
# 6m5Hs8lK5HWPp/wOg8INumkZk8eCV6sMlNt80GfrFilG7WH4TP/vOCC27rsN0Iec
# oGIOXb+qr+DioGNbBCPqd0mOFFI+usoWdArXuF0v7uJjL1BqavU4PDCZ53/ESRQY
# W4OU38b7i9ifqf0X/aHt7Nzu35GCP4ap6+n4pm07ksXzQzDSJ1UHc9QvJXct6ukZ
# lMyr5q3zV/cH/knfnjhleOeiW6+9MaGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCC5qz5PQ9p0dCVdWnhzgIe2JIgtnYfDQVrL7BYXVnC6KgIGaeiBPKRO
# GBMyMDI2MDUyMjE0NDAwNy42NzRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NDAwN1owLwYJKoZI
# hvcNAQkEMSIEIHt9ku4R7gvLnX/aejDuSAK1Z2I/flcNak+pFbSH4VvjMIG5Bgsq
...
# 9w0BAQEFAASCAgBQO17Bo77Z6KktXBgm9rUi10E2J/jH8fd00NJ7mazBTrwyBR5G
# 5b7ZDochNmLeUPVLTbK+8TdQBunTWgbDyF0MdWJ/nNvKy0csUBVowxVbaAFwKfxr
# 4qeNSQ7/iw0FbYr+ombOwoIaiBrvLsbkYSaAgXiIGhzvV9EOQ/vxeXB5xWXnh/Lg
# gslMBaMM4WFGUGKwc+oSjIP4/TVhSkAgPkLKRgFY5DyT5JSHZIn35n6N3mvFm/Ys
# WBtDhHUcK0b2Yg1/GMhfa54Xexz0OpnLPl/01xRvkSSsuDV8xF5lfoeaXphDf8Jz
# QoUe0aExz9LEHqf3Ji5gMuMnSS+yJqjKJvWydcKsuiPGbrYBqk7dlMDfKOsOssCo
# RWzV+TBvHL4Im8RcHkaNuouLGJ1f4dI3D3BR+sAJ9xz6MbSsj2aEW1f+R63NzheA
# SIWdmVGBAUM5mXYORMSPilvgIRBbfxhGkQoe+uKhn/bhRv5hk07+Wax+eO4DV9fo
# m1RBsbcR1ptzkU0oImu/SwU1HNL9vJgqhVfl/m6K6hbn1YCJnYFUtD/79bw/Qvqa
# XHREnYg8F3k48faAPYazFjbnzRIIgtyAFFmN3QPVj9p6W3zBNsWwsFalKVP/2ERY
# A4jn4NOYbrKW9XWLg3MbO2bAt/oBDzPvLqqAfl+Yg7liDfKW3rlX7MXgLA==
