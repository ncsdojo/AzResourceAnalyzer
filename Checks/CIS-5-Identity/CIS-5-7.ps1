# CIS 5.7 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "5.7"
        Title       = "2-3 subscription owners"
        Severity    = "High"
        Automated   = $true
        Recommendation = "Remove excessive Owner assignments. Owners at management group scope inherit to all child subscriptions."
        Check       = {
            param($D)
    $rbac = $D.RBACAssignments
        if (-not $rbac) { return @() }
        $ownerGuid = '8e3af657-a8ff-443c-a75c-2fe8c4bcb635'
        $results = @()

        # Flag Owner at management group scope (inherits to all subs below)
        $mgOwners = @($rbac | Where-Object {
            $_.roleDefinitionId -match $ownerGuid -and
            $_.roleScope -match '/providers/Microsoft.Management/managementGroups/'
        })
        if ($mgOwners.Count -gt 0) {
            $results += @{ AffectedResources = @($mgOwners | ForEach-Object {
                "$($_.principalType): $($_.principalId) at $($_.roleScope)"
            }); Description = "$($mgOwners.Count) Owner assignment(s) at management group scope (inherits to all child subscriptions)." }
        }

        # Flag subscriptions without 2-3 direct owners
        $subOwners = @($rbac | Where-Object {
            $_.roleDefinitionId -match $ownerGuid -and
            $_.roleScope -match '^/subscriptions/[^/]+$'
        } | Group-Object roleScope)
        $bad = @($subOwners | Where-Object { $_.Count -lt 2 -or $_.Count -gt 3 })
        if ($bad.Count -gt 0) {
            $results += @{ AffectedResources = @($bad | ForEach-Object {
                "$($_.Name): $($_.Count) owners"
            }); Description = "$($bad.Count) subscription(s) do not have 2-3 owners." }
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDcpQQwNkDLySIL
# efz+gX9oMHX01c2trQhpA4NGtnsV/KCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIP0dsM9yfYkvc4kwWy7IRmBS7iL7BkKjACn2DMuR
# AhNqMA0GCSqGSIb3DQEBAQUABIIBgEiNgTNs2Wu+Y7+v4YXIrqPbLxlI4SdmZXS5
# nvVF6jzZ+ixCB9v6TKYJc38aIZxq8cR0fV0TcxXGq3kZR/5bR1m7a9YtYYUA2lEi
# GVkQFuGo40wTHrBf9IZGkDhet02JyC4nTksCyzyVGT1KF2lfEDezswN9gHJerdA8
# GTNn9+o8LWVK8lxb2DzBNhf5h6nD0xU6BoX7p+5LA8ZJTDN3C28MmWvX114Pr3dd
# gNO0FgJDtrPWrhez58R77H172u2wPDlvpO5PofSdxXJuv+cFuZBP/x5NDSMaBBi+
# XyKFYss0be62bYaT3lUVbngfD3ORxBwhwNSBfwNnW/kOg4dcz+VzY/fGMBo1ZZp3
# eZMXWjDWjfZniqfb34FDpDA2UpXUIH4W7p2pJD2JSX6wQI6g/3oY07bwqMluD/t9
# 6Z65xmEC2fwJYcxSPJnO7Vl7ew+fQdTUEowbBIsQFe66xmUDSwKyiPk690iLVS4J
# YnRevbjk5KNqf82/tK7VTgsAEJoI8aGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCAN3HvuQtYDJgW7Nt81XwMOLx6I+If/HOT5wic5eAvBfgIGagxE2ZL/
# GBMyMDI2MDUyMzAxNTc1NS4yMDdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAxNTc1NVowLwYJKoZI
# hvcNAQkEMSIEILMm1S98WCmH0RG+hzhtFaBObbTnNOLwkDCyKHixEXi/MIG5Bgsq
...
# hkiG9w0BAQEFAASCAgB/h4Kk/TF/8dY3TilAG8R0YdCJqsaXVhLKaQPbvLuEQp8T
# lzdnCypkxwh8KPdvx7creWRtCZri9C/f2qa3q5Wg/+XJzNxIlwz67wA/A1Gfg3Za
# 6RrjvruiOd57xlUKRtCGSzYcKzqSYjXgXrTzhSEjlLEkdg8hY//oCVD5kHDz1O77
# 76L8P49oJbe6FNOFx+IuB80a6hXJNVkgHfmBtGyc/Ucd39G5Tp1fQOiNl+fw4V5H
# T87mjMXHozktDNHdI6jIfGW7+uJl4LZM4OoSxM2GJVjPMRf+NuB44CtpILIR49sv
# vnArLQSD4VXQhzVDzzwh93uG1T8SdeZYpEnDRKQonQIVdceJSA85KfmOicHmi6zO
# hSAZ7kxijkvshZUleoEkGnCNu2DAEcDI26ejl5LdXFknnQljUtYtCRh1FH3K8Aj7
# kkiTXb8Lg8VyqW19aB4XvaFC/Ed48Y/pqO8OSW2SQKBmIFKSlnJ9Si3dGMdT3KiP
# hF0Uxw5yRPQ3ORuUTdWX1NLL/NRWyg55jRq03TkhHJUR1428tfH43/DiVS2/AyIt
# qpE5Xkm10JTKR6gaJo/nAadfsUpcWh7LB4dV844vWO9HETOVX0+Io3ePFvFFLDb2
# A1/YCJGAhEKiGz3uQV+3iIz1cT6PjAxkm5YCoooAt9C8PLtNSvMDud+ejec1SQ==
