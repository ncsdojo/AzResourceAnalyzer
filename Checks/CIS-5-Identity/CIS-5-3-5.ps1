# CIS 5.3.5 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "5.3.5"
        Title       = "Disabled accounts have no permissions"
        Severity    = "High"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $disabled = $D.DisabledUsers
        $rbac = $D.RBACAssignments
        if (-not $disabled -or -not $rbac) { return @(@{ Manual=$true; Recommendation="Collect DisabledUsers via Graph API to cross-reference with RBAC assignments." }) }
        $disabledIds = @($disabled | ForEach-Object { $_.id })
        $withRoles = @($rbac | Where-Object { $_.principalId -in $disabledIds })
        if ($withRoles.Count -gt 0) {
            return @(@{ AffectedResources=@($withRoles | ForEach-Object { $_.principalId } | Select-Object -Unique -First 10); Description="$($withRoles.Count) RBAC assignment(s) for disabled user accounts." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDmcDfuaIj/SzzU
# O3QJ8l9SWduhdFSAQRIxy8CicJT7TqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMakimiKzoHbNB532OY+1BCrZlr4TyxAxFEBqD5+
# u4aNMA0GCSqGSIb3DQEBAQUABIIBgIrpOCYkOQ3pRYnShB/nSm0UcnUsdA0Xbdzj
# yRNwG3Q/MWWgfc0dsyGknmxJy7nA5Lptdhz5/X2ffW8v958tWYVuUb+bg4A+nxkE
# irFqh+k3swHiUZY0ZIR5vHRHUyYhcerjguIddo2t9mub+1+bdHDpexJpWFVjpLdA
# rcZWHNz+aOdl0fmK2Rc+ry0ThoU6J2DZ7n77+i4no5E88DMlfnUS6Yw9yHHRQr33
# vQHAwUTciY92y4memJjZhq3v0ZoE3Vz0wD4+OQVODQsdWX9Ui2yp1vkEw5lJcTSr
# cL0xGqvx5xfZvUOIQB8j7r+qVeITF7k4J3q5ShM203DaV8LOa5BFr4fJFtdEIHIO
# qSAqz6bxZyBVcouNoCUmC6e5wnaI348Y4Y41F2WDGVJDM3hEV2zfcriWSufwXWYP
# ur58QeJOXLMnG/JzCk5Ie5jMmO0LVvkXBnYo+BGqrzfM7QSbUWNbgmU9KHjIVqEn
# 5EU/OQhwLYxyrFtvmBsCajd98rzzm6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBYn5jYAqrGixrGeCSM3zZl3fZd/XuY68Lecm7RRfwJsQIGagxE1076
# GBMyMDI2MDUyMjE0MzgyNC44MDJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0MzgyNFowLwYJKoZI
# hvcNAQkEMSIEIF3ETsKgOnG8y+X49IuBkDEeWO84ReGpj0ykCQLgMqfiMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCWO0plT2d9ffXkWBMf07xQIwxr7FWhgBfjaF9f8CVaqPnz
# P9e+DTF19UCsrTo6LirN4XrZ4mHc96sYKSKKqCCqxnI4zARd9NWQHE3eu2Wjm0Nh
# c/ClCmUbKlc8/KrxZ7585jiZsr5Icl1DXbgI/qhuDA+hat1joqOPmJhQALKg3sWq
# 7MDyhsR4M9zP21NJRYixZqX8RAnB97XVT+MlGnuFQjRSN5s/0cWtgR1RwEBc4kch
# C0ZA8PXOMh5YVlYZoUiBdPmSLiM+baHr0HovTBxihbIw0W5PS7U8+3CRWhGjhaXz
# pDRPogRIDPGasKPKsSuLgzb5WwDxEaUeWIjsNnGx/qZ6R14IskA+c5vFfWbqOW4c
# BBn6Bt9HiWqNDhYO0Ugy1kMCykjcq/lESiTma5hQCoq0CEy/mrpBy5ha79iFtZAJ
# Ua8NZX5FqChjVf0HPOB4ZmZVOZcUObPzNVN85udDtn61JyhBrT9Ige1B6vkYuvt4
# ftnPeOKBxIcpBC0RDXVH1PUu/avwB+V1rKGbho3MWdreqAJLr5/9lF/x5oejAkLv
# dcrq5P8Zkn2eWC3KPV3cBgDxJfznuq8dstZVUB7V81XJ4OZrdwCWmJLz8KH5XNTd
# GTZzgBK0TLoyHZ9ajjpEf4/7gOnufiwtdjowLFVn4s9sdArcwYtp0rSKbtwDXQ==
