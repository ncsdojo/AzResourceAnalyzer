# CIS 5.1.1 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "5.1.1"
        ...
        Check       = {
            param($D)
    $sd = $D.SecurityDefaults
        if (-not $sd) { return @() }
        $policy = if ($sd -is [array]) { $sd | Select-Object -First 1 } else { $sd }
        if ($policy.PSObject.Properties['isEnabled'] -and -not $policy.isEnabled) {
            # CIS allows Conditional Access as an alternative to Security Defaults
            $ca = $D.ConditionalAccessPolicies
            if ($ca -and @($ca).Count -gt 0) {
                return @()   # CA policies in place — Security Defaults correctly disabled
            }
            return @(@{ AffectedResources=@("Security Defaults: Disabled"); Description="Security defaults are disabled and no Conditional Access policies are configured." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAMPi/GjW36qqH2
# vG0FuWEeq95Q48+hrouT46vlZC0whKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIPBmV7si72ToI8hGl0ym9yq1SCo9nxzTou6baTz6
# g1QpMA0GCSqGSIb3DQEBAQUABIIBgDUmbMQGwqfhZvJTEYPNniA7sz6bnE2yqDrO
# +DvOE0EFPEk8MIiwcjyg2Ahmw5tqYdA0ZnjRkMAssrNhZWvGbXiC2+V+1i3CPero
# 2Af21uHOVfB/NzQSM+JhybVJmKEiTBczKkdziwB1vpoDquRySVaDyvFZrtTPsgYI
# qiRbvZqXDsxjOyv1Ska/9E1RryvgHRMTV2DanBtwRhPGA/8o4titesnCd8URQPLU
# c/d9lfwnS3Nghp64W4nSaST/OVOPwz0G+zu0mS3+bUBGEFRSSdej5LAs8yn3mA2o
# +yobdNWHM26q/UV89DbPAQG0rnW5DIQcdltWtBLyEk5EEmT0stiHbvAyAoPBbW7L
# eBMQRZO9W+Nh8EtDGjnf420I29+pmLGhnHiYLymaaOsWseh1kgW4qVTNbG4GIUsk
# Z0mHXIAqea5niKTHzyizmQ7jxc1xWIarsglagksjs4aLsvmClhA/Yyf3GwgMqFgW
# RIcLZ96OEyQmHSBI1cQNQc7zb1V/AaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCAbG50MoWa10o5bfAGN7KTKueTYznbFrmioLnAj7Z7dmAIGagxE2ZE8
# GBMyMDI2MDUyMzAxNTQzNS44NzZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAxNTQzNVowLwYJKoZI
# hvcNAQkEMSIEIKbTfm0ivn9Zb6PXfInpOZqJWyscWyvreP4NecahgyIcMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBKZHrz61KIAOFDxqSellkabGgwO5Y4f4BgKhx1VuGNnml+
# w8VIfMvaDsLxRmpMNfoR9GIVb++E15sa8IOF/43FH0DwjFca00hsED7YDt4ahZpF
# Z2Rlr31Yc4ET7xg9y/mOQukzjfpI9e09bGeWH4b16lWITdo5RQ04X8rem6VW20U+
# FkwBPcdB+tU9nAD2nX4mzVf49OrA9RMg6P1gg6YBt+RsaU3tm8NrVpKKL0OLErhV
# oU9DQIHw2NXkAcdAjs8xqpzpDdxsLEKtO7XEBRNfltlUW7bD5f0OfLevf4dQFnrz
# lau2GT6N9f0ukHZvMLlVOwK7TUJTXTA2DIPHcxWd/PQBKQnR/0IbCtARfrhbRjAz
# BMlG8PEzCNr6onsdMnocON1a8DSg/0wjaxuUqVfT9ggBScnZeweQ0qJIpuVkzbGP
# I2LWarZE09rCSDTfYikqYV15uEQFPHkQdjTXMYnz07gde6CuyXuXZMVyFIAI1j1s
# xhiZZ678Iuj4CDGqSN3ZYHCmij4IlxXv9f/w3KFks6DXaA8AOu6sQIDR2MceZsPf
# uhLgOoxCfFlNbNH1D1MJ4O6QdMWMK6GeF4gdWV4O5s1M16WPpMK3EKR8ZGqA6Q+7
# E7yPjTOpOSKDSZwUaWgcIXFwDav2t39brcQVT4g3oE+MdcZBj3wNtEFdcGWj0g==
