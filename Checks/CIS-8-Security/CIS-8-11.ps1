# CIS 8.11 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.11"
        Title       = "SQL Server public network access restricted"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $sql = $D.SQLServers
        if ($null -eq $sql -or @($sql).Count -eq 0) { return @() }
        $bad = @($sql | Where-Object { $_.PSObject.Properties['publicNetworkAccess'] -and $_.publicNetworkAccess -eq 'Enabled' })
        if ($bad.Count -gt 0) { return @(@{ AffectedResources=@($bad | ForEach-Object { $_.serverName } | Select-Object -First 500); Description="$($bad.Count) SQL Server(s) with public network access enabled." }) }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBeQ7fPdYFodH3S
# iuzoSW4M20NpkCf5yhjI7Dx/6w07PaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIM9bcdzjVe1eR3ckHXp+3tvkDzpuL31/RqBZBcYZ
# ot+FMA0GCSqGSIb3DQEBAQUABIIBgJhHJk/96fg5oGThu1CTyjF+4q/qrpZYDxdX
# /B+Dw3o+jsYRErzmWXCpYnZwc5eCETOq5jPnOmouTJdFjAiotwvrhBqKSQtGy50Z
# O/UsqogB+uIRe/AUqaFz3T40izZsDzBy3xBAr4TkLu2ZFlAHVQ22WdJf05X0f+rp
# eeIhRbVJn8vf+ERiNDxEDlkklhbCciT4Z61tcgm7sEpr+xhCLWLwh7iD9JGgmVDl
# IKdIUn8g1bn9X/QSk8s8+0H4dGx9dIg83k5ytyvIfw5RAHGsjRpjJaz8TXXQf5Vf
# GYzm3R0YuG5oqyIU2TcZLVMuSrvnf2hP7vu8wZpJigSSyONzXoQdYRbfdXU1aohI
# TedEHThd6p7+jWXv04o/sy7LOQ/j6J2gAEgi5fyXngoac4YJvOxEX9K1+6dhjRUz
# mq3soXSM/zcGaVmuPE5+/VAYSCaYh6E5Pq5ct1ekfUNU+2GTs9Yk1yVcjiPIhGP4
# LZTFWSR7kzmdlygh7DeCnfDzFJp+0KGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDUofuguhjO3D5+Ey701/c9ylWWfkwOhXx97WHaIdKOlgIGagxE2Z/C
# GBMyMDI2MDUyMzAyMTg1Mi41MzVaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTg1MlowLwYJKoZI
# hvcNAQkEMSIEIHklTDaP+eUUcjKAN9zSzYH78aVPy3XdCR4LpsBy0ArCMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCvjU4P9/SFMpoIh3sV5fo1izA8VJFHYF2Hm+ER7w8ZHtI/
# iqe9mR2PtMUST8YiWF0bmS16p2/kAm6udje80MQCoDHQve1O3zqj7eWM7949bi/j
# 7pbFSwosZJfMkKRy0u2iYabJofIk3k8y5jZDlqkq0ZS0Ra7FiLBbXKnecdhRkvAR
# AF/jISfqh8msc9+Il0Y/drQ/+n8xvOXij1MC6K5baD8FS7WSOvfcZJ5nDQHLVhno
# GCKjL0le2SMCzrgC5YypH1Vwf439liaG3L8BpNjHblZlZX/vMztxypFlONYXfn1T
# 9Bbvu7/EqmjWD3NLTb69jXGpBr2y9lK5Gn/amFbZDNookDURgSdVb+yUNJIfHcKw
# OXQFtG8+1M7JNjvbOxOuHTxPE1RUuX6ufKxwZW+8ng9PMhp5Du4VUKog1makP/jC
# QToJIai66tz9hBwT54iZ0NpvOLgJrL5ffpM/3AV1hhQnZILnH66cCZQPEe3C9BbE
# y7AkDFI3czRxq9FA3UcxCSfKUjcpfpPqJCR0asC3WisX2vOKgwubbxa83p1L1jIY
# kv/8nqUC4xAECFrEbCwKC8+tS7LCG0Ov86epB0tntFLK0O1eoUeLWqW/bxr5bZL5
# D78uhlTnZwsI7i8O9RF8NYtRJ1S7d0TdIBdPYFrbWbPDliNy0pD17tcZUv4rjg==
