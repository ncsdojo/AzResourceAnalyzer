# CIS 8.3.11 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.3.11"
        Title       = "Certificate validity <= 12 months"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $certs = $D.KeyVaultCertificates
        if (-not $certs) { return @(@{ Manual=$true; Recommendation="Collect KeyVaultCertificates via data plane API to check certificate validity periods." }) }
        $bad = @($certs | Where-Object {
            $_.PSObject.Properties['policy'] -and $_.policy.PSObject.Properties['x509_props'] -and
            $_.policy.x509_props.PSObject.Properties['validity_months'] -and $_.policy.x509_props.validity_months -gt 12
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.vaultName)/$($_.id -replace '.*/certificates/','')" } | Select-Object -First 500); Description="$($bad.Count) certificate(s) with validity exceeding 12 months." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB7Wc9wbtXEdnDR
# 0J7B6jfcF3j3Wpw0Bx2Hbxy/JC33qqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIE+4ypv+veO7ON5miqKxFsU5NZEynx/su1PG/wSI
# vi1wMA0GCSqGSIb3DQEBAQUABIIBgKKpxiomLxkguxUY33+BdF6t9j5i/hD8cJZH
# umDluQyWAnneBs+4nvqdQoFkjY/idYKCNRW1hdn6xXPPJ0TIJN4mFFateEVulXVP
# mW1lJTcFvZAMwH0fb6Ss0ITSceYnkEeDO/RNS/CVg6bKLDVklQm63OOlyr1CQ5mk
# YirZxEJcAC9GDqW2Xa/RCbMzxXz72jO6LE5gjNRalhOjX14DX3usNioJCW0RNms+
# W6jLfCOZp9vt9HpkBpcrnsCNeELTef8Ntesd0nrHL6GS9XZxKUkZqvvxX17OzCMg
# 6bFbHBX9dRJvfyUxUdKIPq2fTeoS1yqWvk7NnwrXiJDO80XkKJc7cEkm8sY9AecA
# BT8i+ed3NbY+Ccw8vhqwO6yjAyoZsk1uSufcNEdI6efnpR0lbK4GNyuXxvfUWncZ
# 9ahankfDtO+Tha5P3UgI7C0AhIeEN0hqTJ8wrAY7fLSnSsLYlaEewnSU+TY3q3dt
# eFcf5w3xQrRxJd/KOonLEg8nLEHbx6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCAsyIyAvZfVniiTyypP1MWeGuqYdlaqnjrpr2fMP/LfcAIGagxE2aKC
# GBMyMDI2MDUyMzAyMjQxOC44NzNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMjQxOFowLwYJKoZI
# hvcNAQkEMSIEIOrDF0V86cHUW7Hummc3t3NzO6GbMREiUn8/pOTvYkAUMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCbFRtMY7IdaipnevgZLWZrfHakR58ilGe9eUPVf8atWQLf
# pfC28e2QLgwNTcyqqySAG0MWZkaKlguLskiGZ6lnYO3ppAlMzhpBwYqnLy4cphwC
# X3bHloVTPAZuBxITQ8otcqmiYkgdAGsGAAI0XzLs9KYTlLnychxMCpp2/GJXtavB
# ejQ7ReUOtnQIfuU7ir1IY4U2jPegjUomsVApjFq0vbJfimJX9bmUSD/Wb1ZzikJL
# YlnNSgPWcbwCG+Zi8mb14Mjih+nraectU2eIds8BXecGXNzQOgmj+LGQ2Qc/5z9O
# DCLzEmBrH/p+4o7Ma4Vvuv3J6WLkLe5LOVvGYd0XOfdmpEIOOSTA14LQtczpQrnS
# v6cNIhmpbXmjXi3ROjFmixX3Kvzdz/RV0aT1qWRmHjbk3fGDwDGKuOmItBbBD8Ik
# 5y/Cwt4wvHz0J8BMWyxrk1k7iOqb1vW39+7hKpcP+J1eJDThOe6kBx1C0di9Hhkd
# /k6PPU8TENESYpNIk+TJ2pLZ1X5mXECmswPhcCcnJvVbz/JY7hRaAG8A0qxA23HS
# JoedeXhCmeLvSuXoWavfSM4AAe7PUP2RWX0L1lsWn45oY2VU1HxqJIlco26DXmf7
# Iy4RTIzchqWSdpu9o87ZmuFMuUdnDdZGgjqYnHBoqUf2qYSFkS1Dn3zOU1/qmg==
