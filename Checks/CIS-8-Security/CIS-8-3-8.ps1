# CIS 8.3.8 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.3.8"
        Title       = "Key Vault private endpoints"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $kvs = $D.KeyVaults
        if (-not $kvs) { return @() }
        $bad = @($kvs | Where-Object { -not $_.privateEndpointConnections -or $_.privateEndpointConnections -eq '[]' })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.vaultName }); Description="$($bad.Count) Key Vault(s) without private endpoints." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCwnIWvu2TV82rI
# qTRbFVvMlNm5x200bu2x52taT/Ge5aCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIHYC1WJYMGzNcFvjZg0ZYhxtS5mASPO9ZzyHwUPe
# 5A3YMA0GCSqGSIb3DQEBAQUABIIBgFAtAfkVsL/CoDT0dD8vDbppersLqbeam22Z
# oFiuOsxbTY1wz4FOpBQbs4N2slYqZ8yyDwheBI3DjQ9vfa+NKGwPmFEXcm19GRHz
# MA85FNY5q3PIm0zyVQAnflE0mnHbr9tbOgRBlvbgwnlyYkKgQQltySashIGhCV73
# wnV3f4TnXLEYAnfng9SZ/qAlLaGFIgzlBzrmiKnQ4DcQh+LVbzvNYmBJ4RGSr8Qb
# ZPDmH4JWBzIb8JhsUcRpTLqGxlNFOC/8p7YikUf6R4JzYAYnDK93R26gIlfYGG84
# rhvX9PTntOxq1DoiKHVJBc0aiXl/zxL8rv1TDTl+mC6z/L0F8qjxtvxGyX3K/fX/
# LxDfE1XqpM6pvC+Oaf9d0ywldLsvP+hIaMB0vcCd8X9n/95W/NF+Mo8dBnc+kG7i
# TyYFvtXK+1AEw0aCxZR6ueS4pSV4rwciiFbd+6BNG1ukfCAolTwDMYz9UDROHWiF
# YzU1Kz374p4Vhou2dZelHb4+swO+nKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCG5FgiS5u23BQU2jlsHFU0Lpm0c/NXGSe7Ot7VeVw3agIGagxE13Yy
# GBMyMDI2MDUyMjE1MTAyNC41NTZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MTAyNFowLwYJKoZI
# hvcNAQkEMSIEIFXUxSq1+DFDCpKsx9WnmCG4F5R7VkF/ennssgZrtoyCMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBsCN/Hrjd1PWuevIR8a58ZhVioabx+c7zNj7Qslt2Ewi1+
# HiDkV6Tf9U5UJCQIUkNyh1BOjAeMNqMlQylDozNUENkU4AepABxIdlehrGa/IBvq
# C3/91P1mXN+cFg9ZZWS6jovrYfSccoETTGraAgRLyhYguRDI3iX695rY7T7IX5mZ
# OXdhjqhgzdnqhk5DPuENjdWrzhYbxhvtVH6g2zHoYFC1uSZ2Lbr+87UJygCj+hlE
# ti7zp3n0fh1/C68s0EsZgaGRPBh4vcfVOgvnEMxl9rIHHBQ3hWy8cvj1+m2dpGOG
# G9B0yYIQpwIjjPtcWJ9Ta8zYEMT53qi4D5tKBL10tKNH3ABul9VCNBovy+ICsNVX
# 7nzUR/7fYoZpMv+MdPCkU1phoQuGxZloa1tuhNyhcG4js6YUU5IDRSe4IrZmREPZ
# tLOMgdl8TKp7i9tVhUvPlf2AbwprB1/nEU+dkq/F3dmEViWEyWPAbQTKly2NWpnu
# UZPEexd2FMVEZimQwT6vkOkgcRQuN0A/wEETX0Ci1aWi+ezPy+hbFDfOZqAitbSv
# MOGu+IrlW/DGzfx9m3hFAgnRGethds+CmVVRBdT2hEQ6lgupD/aJhupkqCJAq3Jt
# h3Ik7iiqnUZB79QYhvxyAXm1d0/j49/RY8xj1eabMMMU+lvNXlcq5P6em4+PAA==
