# CIS 8.3.4 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.3.4"
        Title       = "Secret expiration set (access policy vaults)"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $secrets = $D.KeyVaultSecrets
        $kvs = $D.KeyVaults
        if (-not $secrets -or -not $kvs) { return @(@{ Manual=$true; Recommendation="Collect KeyVaultSecrets via data plane API to check secret expiration." }) }
        $apVaults = @($kvs | Where-Object { -not $_.enableRbacAuthorization } | ForEach-Object { $_.vaultName })
        $bad = @($secrets | Where-Object { $_.vaultName -in $apVaults -and (-not $_.PSObject.Properties['attributes'] -or -not $_.attributes.exp) })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.vaultName)/$($_.id -replace '.*/secrets/','')" } | Select-Object -First 500); Description="$($bad.Count) secret(s) in access-policy vaults without expiration date." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDdqpEel+7NYH4z
# 2GlHMm/JGOIcMuX4O5y5ZPjrYfoOYaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBApjP1hJjMIhMGVeiK9JPtxX1+aMLp7//IPfk03
# Ss1YMA0GCSqGSIb3DQEBAQUABIIBgF+f92W6GVCeqljeFAE6QvaBBqX/TfVl9VBk
# KSgH6bIiNk5ME8Kr8kZ0AphO2onwyhjuc4LOxCwrZYtzlZIl9vNPCndIR2LY9iq9
# JcDAZuYkmUSucIcc89aJlzbIa13LB1vOgWxbSdaN2of6K/7yN99m4g6A5pDbTrD6
# UbsWZDySeDzu7gCsnkLv3uumgSu/wcj+V/C1NSoAa+fI+C/6nMm8J+Yr73zo6aGM
# dW2NRW97RNJMrsIPY5DTGzJqV7p3VDAJi2BK/XairdYC1nzOTXwIBS+qV3SuHBNL
# HlHZ8KcTSpElrRt4Qlxk7czV80wqw1TvZ02Y7lW8o8uXG8DvgDTpJfm05ZGhByT3
# 4ui6Iwa22O86dBlyvEaGPRZR4+czUYUCXr+PffV55Xyau1sqd2TmBjOTPVLlNoDW
# 5HX6rl2qSlRpftA42mgH7Hh4U1/Gi7YOgSx+BxgtOkln1UQ2kbw18LpOAzV3dhOi
# pnXG6/HnP7SM/QX5UgMJ+YNzf3HXjaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCAvr5DVPYcbGGBO6LtOtLUGbB9c4BnvKVbRvj9HjHQIOwIGagxE2aLn
# GBMyMDI2MDUyMzAyMjUxMS45ODRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMjUxMVowLwYJKoZI
# hvcNAQkEMSIEILmeIg4gpF302i7oi5BADZr2l3dkrOxIO85J++UtUE4zMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCQMQHpQGHJZt8aYHBsOSeW28PeEKXqOLSKo6w5Xh6V4Kuv
# WSS73Al1agP/z1inu+ddKUiree8IOqmGD7l+zcyIy6PKTgknHLe9HdgVqvCcqDGH
# 9VfwsUqovPVHCvi4KziJz/Sn0I6U4WNFck5X4sQ47H686RnZ5qcqBNWUA2leJx+z
# As8/BZHPK0MKy4kFm6ke0wMs09SdgxrK+SLj4h3O3onxVmQSEotr8JidqPCrMtsU
# IVIpW3ktnJCjJa1aYu6U8aojoyptsOtFWL+9g89MryoJ7J3nhTSM1vtL9wcWLUhG
# z1CUojKR4OvL50g4Auew3XtMKGBnHvPBUwqJASmFUbFyLYSi+9LLg0G3i/Iq7yS8
# fQOG8Nh/nVBh9RozNj2I7n+PbyocDG9CbxQjVOneDG/cqFGnygTrgtc6H2fmJ9cP
# O/1L7hsqK2jyFJwZ0vmkddvz4IHVvfBDQGToEq/2fPkSZQF/E+aRTEwU9+KLHbNc
# RN5zo++HPdas4gs4336rOKUc1LL5+A1vxCnZcfX8xuPXQNfEwFLv/kVW8dnb0Yeh
# eFdsrCr7rOVeZ6AzRMCGirKJn2Xo0/30gu0zc4oHX3NntM2lq0Fwl9YpcnvwE/ur
# 1M8KyzqfyPtgeTVweYSuIkbOHO5WpLS4gu65S9FGb77+LHAHImYt1EjH4kmChA==
