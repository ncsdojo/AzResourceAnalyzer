# CIS 8.15 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.15"
        Title       = "PostgreSQL public network access restricted"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $pg = $D.PostgreSQLServers
        if ($null -eq $pg -or @($pg).Count -eq 0) { return @() }
        $bad = @($pg | Where-Object { $_.PSObject.Properties['publicNetworkAccess'] -and $_.publicNetworkAccess -eq 'Enabled' })
        if ($bad.Count -gt 0) { return @(@{ AffectedResources=@($bad | ForEach-Object { $_.serverName } | Select-Object -First 500); Description="$($bad.Count) PostgreSQL server(s) with public network access." }) }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCHSK59WoR4gU7j
# mz2nOTf2dEMoF14kJ28olcVCRBMwcaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIAnQ/hkgfwnMDOgKjIT9UTG9XIoaOwYqSaAQv0wu
# AxWXMA0GCSqGSIb3DQEBAQUABIIBgDJOEakjEqPVZoqOPDIPAem/7CjNP6tTeTaA
# 7Ro7QBYoStDYEmQvGSg1+sf2PzJbLFHM41+93FYKq+iCBxXxKAlGVMZK8nwj+pel
# WRu5GVm9PExfOY+SBddeWNywCh75lwvS7Fqe7SSTlOg4tjp72Qx4j1WLV5bYooYM
# EmS+4bxG/itktUNO8rw4gjUbIGi2BFcwwZ7RSAGRBcF6tYx6auKz8oBWoIWDA7Mc
# kHWJjKbJM1KIoVU8A0oJIZR88XiwFcxyY2kfBFjaEMkAN9YJGkEnltrYaOvpCdKC
# H61kiURAtVAB3GZvZ5JJ3csxV1VYmTYNo6d7WBLQb68h2TnnXMzG6+d6L7bTy3Q2
# VR5fyhjyBm2pSBDsw0tXBuwajQUD4oK5IBbEMIJ72p/mCenCM+dmJtREg8TjDEb7
# zv72ozwqbpSQLfYrxoeK1xFsnppdZlZSkPQ5L49s1PaGJUHmwiBav3PIkFJV0/Jk
# NYLRNaeUmHxZI/12JqKT8I4BoTftU6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCbb1WvZYWWII7kLchO0IRcv1w2FSCUdp8lrBes8uLi4wIGagxE2aA5
# GBMyMDI2MDUyMzAyMTk0Ni45NzJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTk0NlowLwYJKoZI
# hvcNAQkEMSIEIEWQogNbbe12nDY6z/8qEkmKTpJEE0jKn2Sfft9G2KOdMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCUd9Pi0kHidPWaceAEktAbtmhTcfDTu5AesFTsFo7q9if+
# VRzaVZDlsHiVdH/EjnYx5TPerNseShs+QpEcqjfCamvpmkTG8of/9F/5ThKWk5Tw
# 77JwAX+T1DhQUdDnS83QbQAbeWhR08NQDUX0ApAJY6RGDkZKy5l8a6EuKbpUEWt5
# GN6XYT0KKW4N8zoV2vNFOYWrspGxeC/K2wO8W3SH4+SjexhTHd5weU+kTXqih7R/
# vBRXaAGaeHRFd7XuLTRw9kAIIV4ClDTgS/mf6nGz0kuLYsAHOPJg7UyuqW2cGk1y
# CSQRE4Ouf3W7LCHQPlG3e6QIkQt1lL8XnumtWze01t9IweiEJcQurCuSFcR9Hlju
# Qp4ixIVq6UjFzAR02ZZ26Us/eHChWpxk9breStf1Thet9uHzwYasP51w5OWjo9Eo
# omjOJHMPQv4eZV71+pO2ui1iisxV3+MlaFNWN1YuqPFbGLY31EGM0XjZ9XCjrf6q
# TLH8mxkh+6KwMYuZM0lhlsGsNjYxtarxguvpiJkdGrEsbl9/p2b27wgNZX5hPoBR
# GndDEjiHUtjTmWtsdEpQe7cGjZXHkb5x8+96iHqI2ELJN7JnFcTbnrngeagJHwej
# T7T9nX6gGpqvXQsl3vl7zaGty+GtgiPMNiajkq0d79yBrRlQ1RM3JQ+D+3/b7g==
