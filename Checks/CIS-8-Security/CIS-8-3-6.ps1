# CIS 8.3.6 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.3.6"
        Title       = "RBAC for Key Vault enabled"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $kvs = $D.KeyVaults
        if (-not $kvs) { return @() }
        $bad = @($kvs | Where-Object { -not $_.enableRbacAuthorization })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.vaultName }); Description="$($bad.Count) Key Vault(s) using legacy access policies instead of RBAC." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBZgzKqiCi9w7j8
# d7U+ChdfFN68UNYuLK0du0k61PSQQaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIPHKYy6pJCQFKXoIvqy4tX4U8NwnVdWHpYHzAoEq
# JQgdMA0GCSqGSIb3DQEBAQUABIIBgAX1OVhregotdOxujMonDHEQY2l4wtIphEgz
# 9nT7rgZSq0fdK+OdDDyuYL1lhQa4wSQJt41WjTqogDn8TrYT6qKBhyry9zpM7PyH
# ToKazQumRGv7EnaOjbC1LowNScqsmaB5YkxnFvxmBMBwInHhgu0VyDMfin9x+hqE
# SIwyhzh+SdnK24uCDnNn9glJfiDxq0A8QdDKBcnc5Zd2kFSA8n+sH2/2ylKTA1ix
# XUbTKC5d/aEMFGVZNvWnOjcJXmqYSZGtS1LWrVznhZG/Gq4Fxttf6c2w2iANqGnL
# Au24dPU+rX7fLaiLEJMjQgw1Ppy8x+FwqpEKmvWQBM6wt5XU0ZShr5yR6WsZqb9t
# l5qHkhtb+VYIEjCLJR7mZyhKZvhoBBsdtGb9OEvvDS7ZzuWh1+s/JLY6Gq7gOCjj
# 0wa3H1WkqlodzFnER8gpeiMEK4domd06orCT//wRD5HhfpbnFj0MmsTX00uqyLlt
# x1WhLOuSiFFENmSBVvdAhc8FKtU7j6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCSheKHlGRc9Kd2Xko3YSSZW2IoCJOBGkKNkEqW2SDETgIGagxE2aMJ
# GBMyMDI2MDUyMzAyMjU0Ny4yNzNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMjU0N1owLwYJKoZI
# hvcNAQkEMSIEIJYrnIoqR6okrWNV0TpOs/zkYPHAvqdxrsD1BzDDLRokMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBycMFgmk7u3gQAtNiW+t0ilQ/ZT2sRK7lxc7wu+q/yhf0j
# KNC1NPsTti7GNcQzSgeDt5fvQiEB6q1QqCzAV8QrBvLUpkKwikgIxE+92Jyutmp3
# mjuZTdeTfM4i2ff9K5cQF1dY4e34Thq3VoTd5M/ubJaXp2XADaMqqGCPXW/MRmTX
# 8fat3IHRam9voW55KBw+gzjeqcbv/SiE3RpTINGt4xNst2hFZEK8URJozaYsv/a0
# gBD50s1yUuxfjUcJbnbX7s8I5XOP3ob0x2NfnR9dotemEf2elm7U1hyBKum31Mso
# LQyOwwf89liqGUBzbgCBba3YGfLhEqySx+l3DkfuyTJqlMbmCqC5JE5OEjg9VxrB
# GTULFfRoD2ynVfHrS/r8B6LSXT1tYfNnfL2lhoMt9MWJPV0am5wnosnuLWvZyV1r
# A/ANoIDAqpwDiugoAI9C3LwG26+Fdxjqnq0CJfaU8iY+u7jf7fwbW8Bjf38aWmiK
# A7TLaZp5nwBLvmxrhhlsHNTnFy/dFofMUXEczOTVdVetab/bWrkeDiKbhe6/XYgx
# 8mmFW7oD++3+y2dseK++nGWVSjGFn8qOaPARiOVThjwzXYxaIdMkTfOXmYtfkklt
# pUQnRsgnUppVEdd0SbA4YWi0obOKANTBjayLj9vKH5vv6Fh/GCLSHLu7KWbK2w==
