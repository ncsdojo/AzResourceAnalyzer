# CIS 8.1.13 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.1.13"
    ...
    $contacts = $D.SecurityContacts
        ...
        if (@($contacts).Count -eq 0) {
            return @(@{ AffectedResources=@("No security contacts configured"); Description="No security contacts found. Security contact email cannot be verified." })
        }
        $bad = @($contacts | Where-Object { $_.PSObject.Properties['emails'] -and -not $_.emails })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.subscriptionId } | Select-Object -First 50); Description="$($bad.Count) subscription(s) without a security contact email." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD3A6SdFaOyC+yQ
# D+benfLpoXMl6IQTZOszKx90tA5++qCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBuJgGumUL1ayDQOi1QkZ43vGXWJXOm99oYghnIw
# DtSxMA0GCSqGSIb3DQEBAQUABIIBgHBTEue7r2X10I9UC9/80WwxlejdUU9AD9hO
# hTIxT5CcBOCYw2iUhxASDIMp+I5MrTMAIwSy91HTh573qd4PjpEkYw4D/rugf1g6
# VQY9bc1quXcnSykZmvwhMcsI92utilpg9aBv3FjYWbCDj+CDH9OL6LKwd07QJtse
# XP0vkvL4nyXlUwvrGdc3uWLcXAneUeKBybu/4/n6swET5fABZj1aqnpWWY3WtWDS
# MnXUY37sqghuIxWu0ea3DEpX1L63gGIe7HqaLOvq/h91INYGamLabhJEUX2hC6Wz
# 9eDg6Gye43t9LsZvQcrs28DOAPqgrAQBrJqof2Kq9IFrECslsA5YD8z//WyncMYr
# NOYI/V3CNupwExYaCirM6h1VN7p9KyVTRnxEeeSnxwaEIVZirKwh1PlPZY6ZLpET
# hMgMsENv3wRWRKCdsLeiyi7hqDAmU7FDh3BjUvHza5C0XPYWuIOnQQwfD02C8s2X
# j98UbeLTQJkIqUQV74tq9xZmR02HqKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCC8yxIC8mp0QA+/IbtDfcbYe7IHK7FiIvWgYbDtaF1lQwIGagxE2Z3M
# GBMyMDI2MDUyMzAyMTM0Ny45NjdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTM0N1owLwYJKoZI
# hvcNAQkEMSIEIDMf4DSZZrgkwssPkU5u9/q5Lx+YrX4ZHtysdGbFzkugMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgA6PvftFRS4S0SJvpt2zPXpRpkilW4J1mfPj8kJZV3FGYzA
# nSz9QUqhg1zpfKYSgPBApvRgxf7RS1bCBMIqkFYe0zMpX6xI/rGHucEht9IQ1odN
# XogMRlwf4E0WS+AIMYNJjS0v1FlwBP+e4ij1y+ifUnXaqoaq6NFU0eSkPO+LESSv
# oSUqu3U2lQYI4Be09lhL6OA9J7ow9GZWJVdbkzxqY7xjtHl/eGJmYNPgjqZaHhEB
# Cva+5YainncoeFn54hhV/0OhRhl4AzuaNwIYvzQM7TpsCNMljhT+dHDL0b75qEQd
# Fam9kTffFw6EyeVmOkFE9SeKvzBCMwMeF/l+bou2q2LIFoqTLgNZ2mETrL3Q0piG
# E3LefIvYYDlARXHPplhEQkmFKxKO46oUHUhZZN1r34YBvw3Qg+4xbzwJBg7ZKT2E
# QAospPEWludrFr2bnv4Vl9Niv4ZTzs87jPe6/H2BzmCsE3fAIsbloP62ClZGGU2Q
# 8ssb2Mm4RYJCjiABBM3cvDSXt+Bef+aDVGg5elVRMXl51CheGd/gBhe/29firfyY
# M+IR5dJuwVJ+cTCZD9svCqYkMq8py5uUJAVJcGSrgAdjdhQtQYFm32cjW16zYMHV
# WI1Q7ZKLneAmPLPcAmLE6QoB8qjaVp9C/0qgqnY1Pz/BaU2B03WoulJmznylsw==
