# CIS 8.1.3.3 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.1.3.3"
        Title       = "Endpoint protection enabled"
        Severity    = "High"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $pricing = $D.DefenderPricing
        ...
        $plan = $pricing | Where-Object { $_.planName -eq 'VirtualMachines' } | Select-Object -First 1
        if (-not $plan -or $plan.pricingTier -ne 'Standard') {
            return @(@{ AffectedResources=@("Defender for Servers: Free or not configured"); Description="Endpoint protection requires Defender for Servers (Standard tier). Plan is not enabled." })
        }
        # Plan is Standard — check if endpoint protection extension is enabled
        if ($plan.PSObject.Properties['extensions'] -and $plan.extensions) {
            if ($plan.extensions -notmatch 'MdeDesignatedSubscription|EndpointProtection') {
                return @(@{ AffectedResources=@("Defender for Servers: endpoint protection extension not found"); Description="Defender for Servers is enabled but endpoint protection component may not be active." })
            }
        }
        return @()
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA+aHg311ERtL/3
# IKyVx6623tn1PLlvrv+7P/rCBSW2baCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMB7bOvpyeLPOzgf2wtO3jsjb8CfhABgw2z8+TQZ
# YKBuMA0GCSqGSIb3DQEBAQUABIIBgCq8jFdYzGRXBOzAp7HTwWwLQr7tYmE/sx3j
# AFLkcSXCy62rkSADgGrmls3TUOGlrx63UcZFINZWetlDQX5Tlx3g5JFPkiV6CzMl
# veYrkyA5A3gBrDGdR0L0nsHscIuIRCEhqD2GmJhO3bkVy/pe0pH0wERsZz18O7im
# q09fXc6Ok6KGPrtalk2NXUOPyaoaiskQYi+Bd2lxbWTRhFcr8jTdqaVmxbGUi2wR
# xyCviPnDZGa2OpbYUt49L0l9Yc2AwkW7h33APf5VO/3o80Dvkq/pDm6Y6DwwoGJn
# 9OKtHCtltLX/NdQK3colv32gDMjKPIem9wjxzzgzKgtmRQ/zpKL73pWbWjTEA5Ra
# tNlthTsvx42U0pDmdyPphweLDo3eVm4G1EXff+NUgKtDJswYyo3RBnscMGXh0x48
# 0RApObn5vTjDjy9s3u7JsbP+ETUPKpDJJ94/Mew1GW0u5tPCFrjH65QzcoYjpbG3
# 0OX9OH+xscZkKSCEPYcClbpvedgLdqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCC+8hgD50lHJvXU8gMvN/P/G7mZMoOCBwARqhi957F9QgIGagxE2Z5p
# GBMyMDI2MDUyMzAyMTUyOC41NjVaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTUyOFowLwYJKoZI
# hvcNAQkEMSIEIHEkx6BLtPjq/uMkFEmplpyzXWAWvPVWfq6EAMqblU9xMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAbz4L4qzORJFKQAubM16ZAm3zw/+7X/iuHinPLR191PUZV
# 8clZ80KTELlkKv/YuGb3hfSRws3/L9BMtGTq0KNyfQ5Ub/7Z3vPLwhejiVqMRPyi
# TlkDDXkweg3py9KB2n/SCEZS47UWF80vQjpq+xNV/IXQtsjy4OQz8fYIDDPOyKSQ
# KWIg55dEU/vwPx+FjcVhlavQZsJcA9i3BqbEmT1AICsL/fax6WjdtMzsm2SQiL4d
# o5ilT1YFTGclMVi0YhyrMqehJTq+Svx2TGjn0MqVSZ5NI+Bmip+VCskeVoIP9lbO
# R7koms/3U8duSh0HUUX63QtnpMs2T6XPZmmWyrx5513WgsVj3UuEsoY3+GOqNWsM
# x78zY29aCOcQt0edR5PLoEsedzTPDwaa1P5OKKSlaMLrssT+YeEb57yaQCso//QI
# c9NLlDnLHhEH4csJ6QIoyxy5i703h6goYk2lPnmz+Ynv2UwFq8oM22bpYHa+2Yu1
# M4RzX6Cd8G1tpbRaF35ZgrK4LQCnOkxU58Js/4aD+oo2G9dKA9SsrpEzjbze5Qq4
# Fg0uaEM5i2ull/axOwBS1pO+49oDhqRr3HGmeGPtd1cXtF+rU1dvRpeV6BKugMCG
# ElqlNIi52AfYo1LMtLV9MHBDDJ2T6DzNorCgG9P/EiZUorJWCi5CVKddf+irTw==
