# CIS 8.3.1 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.3.1"
        Title       = "Key expiration set (RBAC vaults)"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $keys = $D.KeyVaultKeys
        $kvs = $D.KeyVaults
        if (-not $keys -or -not $kvs) { return @(@{ Manual=$true; Recommendation="Collect KeyVaultKeys via data plane API (run export without -SkipKeyVault) to check key expiration." }) }
        $rbacVaults = @($kvs | Where-Object { $_.enableRbacAuthorization } | ForEach-Object { $_.vaultName })
        $bad = @($keys | Where-Object { $_.vaultName -in $rbacVaults -and (-not $_.PSObject.Properties['attributes'] -or -not $_.attributes.exp) })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.vaultName)/$($_.kid -replace '.*/keys/','')" } | Select-Object -First 500); Description="$($bad.Count) key(s) in RBAC vaults without expiration date." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBWXDNnuoHEQTU6
# 3KYy6OhWwKyD/A9ZJae6J6WCPdmovaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIA1YR4Bon/zov2UcqcriS7Kkj8eQNh64SU4shbwL
# IvYTMA0GCSqGSIb3DQEBAQUABIIBgChkgnTpOQdBdZcf9c4le5R+NkeS+KkaowuP
# ePpxmycUde0/2cYUzhElzx5CUUDYSBfTz1mZXI7yvB2+JHN/X6cWCpT8HsD7EE2b
# ACdx0aMpZ5ARg1JTxAXkCeO1E0aFF1Ib0aaZ6Lj3n/kqNzQDtGq78/XKwa+Ne8si
# 9bBR6sRqNrjNrh+00eIIr4M0Vu51d+DSUkrD6NwVH8sPjWNfDFuULcxD8lEgCToP
# bm0bNgmvDaaAEUReBc82PHHuCqc8DI/SbFEE+lxhblb87RoLWbd1yBIuHGESzt4s
# NZO6rPLDKSVOpDlitEHeeebSN3uq9ZDCflPH/MhYKwCLxkjI+vdwdmjFi41/QOYa
# ZO1lM5czuPf+jlTJXjNF3IaOatOzKaIjs2IripugGJ3Mbw9nzhLWfarcEwYlYxBn
# bd/5cC2kd3S1+T9buaZYdC4nIX5v/27A6upDWJHjPPvyRVpLSh+k97v5NxPirs1H
# 8M3tsZ5fhD6XJwfeAL73qXCpgzTDrKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBX/jNVScxjcnAoaax3ykJRxmH3KNlCrvi1d6Lc+1M40QIGagxE13J6
# GBMyMDI2MDUyMjE1MDgxOS4yNjdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MDgxOVowLwYJKoZI
# hvcNAQkEMSIEINP9ljoAEUCMqNsMeVPDlE4r7+p3AJtO0tFfdKUHfVwqMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCqq8wUkQhhY6kmU1+3JyQ7+H2ss1byWeDMV+5vmZ/MkfHG
# 9RMUtrZEdY4ts0iRVr3JzNACHHb/uQl7hha2vBvo2Zf4O3z4kHrBlnFbeeOnOUlH
# 3FevPckDKjiZosiaORAIv6FF6cqEQnHJjUEeN65k+yGZ2eZdZA0OyBIC/jbiygti
# CXvvQvX6/5PWIpWDlOGter0N86CUp07nbSNPVomuSefTpWhpmcLNe/KyasVB9UzL
# XE8EKCz4MacrQsPZ83s0H76Tx83cYdEZoEgrAeCZQeaExGMm05bFaU6bjrHxVdGz
# qqlWo2paf1WsmGCNWUpz5TKWNZsIpnpLEjvlsfbnw2LzkcmGayl6oGjg1GR2lWFS
# 60bBQQsWr5pzHFVgwrC9t/7LLoNK9PumeGrbl8L+wKwOV6te4QYQK+fRNYh4d74E
# gv86mRAja4f0M4RvsBdgNVqODgOSWMYA1hASQu29WcKRZ/TYbHAanTYddTMm+C2A
# PmZa+gIuNYS/eVZeDzUlWkJh/A1Ona5lbGNYp818KFIklxsNF5M31EcTuN4U/9s9
# Gdu85+nYAsvYK5jdHP1CPFmkPS9ngTcUL8XgAXIU7JyYr4rvAXsrBuKNIzhwFafR
# /r4PEjMxTJ09VSzyZHcLyrSS9QYsT8c9T0Rvai3I7NDlQChrF/XAXsTd8SUx6g==
