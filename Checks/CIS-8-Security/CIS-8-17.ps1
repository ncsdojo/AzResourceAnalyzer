# CIS 8.17 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.17"
        Title       = "DB backup retention >= 7 days"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $dbs = @()
        if ($D.PostgreSQLServers) { $dbs += @($D.PostgreSQLServers) }
        if ($D.MySQLServers) { $dbs += @($D.MySQLServers) }
        if ($dbs.Count -eq 0) { return @() }
        $bad = @($dbs | Where-Object { $_.PSObject.Properties['backupRetentionDays'] -and $_.backupRetentionDays -lt 7 })
        if ($bad.Count -gt 0) { return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.serverName) — $($_.backupRetentionDays)d" } | Select-Object -First 500); Description="$($bad.Count) database server(s) with backup retention below 7 days." }) }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAfnhx928L1aMHg
# vt7ek/mPS56ZCKDyeOdnvNerScNIWqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIGt0uVLiemQBRt2OGIU86XGTycT6n46oyznLhexo
# PCfvMA0GCSqGSIb3DQEBAQUABIIBgBaD65iDRVn1oSSFWM0+7WI35F4db7GQeg3g
# PuHtUlkZ4LWXoI3YR3n6EoarfInVa3qGkhQByzmUNwO3rVLswWpl19JlSb0haCm4
# 5aSRcntn0IOfBQdjNEjU/6jl20hvSEnyAyXk4rtB3e5qf6pQA7iF8dLs0HyXusgy
# bqBHTSHIiU0KBn0D3ooEZwQsp8hokFTfhjS6GHiIELbn2b/Pyy/kw9J9GWSet3io
# BzSrCI9uipDJPPn+k8Khfh/K171eyG+Bn4u2m34SK3N0jcg6WReP3t6RqMtvhZqd
# OPCu6B5ObJqiooGEluFxKBkiUJ5s6vWWXrOdAOx8behIW2bQrRpbP9vvlNemF4lh
# iP51jD4tVdzDrYlfL9TgpNMh9cg+HtP2ymWyRD2jn6iZ/kERvBxgg7pS1WutJ+aX
# 5bJsQv/Eu8RAY464boWBVlPIJUx6eZG9bcnvgW5sOj0+7ZtIlAYmPdI2wIBI8Sgz
# 2EzIfYTjlBsh6VcT3Zpcy64uuxNQ9KGCGBIwghgOBgorBgEEAYI3AwMBMYIX/jCC
...
# AwQCAQUABCDMi05sqndUSW5C7F7XKrW6JJG/7buLpRfIsMQuLZPU/gIGagxE2aBt
# GBEyMDI2MDUyMzAyMjAxNC43WjAEgAIB9KCB4aSB3jCB2zELMAkGA1UEBhMCVVMx
...
# SIb3DQEJEAEEMBwGCSqGSIb3DQEJBTEPFw0yNjA1MjMwMjIwMTRaMC8GCSqGSIb3
# DQEJBDEiBCDVe2l20XzkQrqj3BkybJD1ZYsyg/x82DCuiDFvor2o9zCBuQYLKoZI
...
# hvcNAQEBBQAEggIApMpT+XeEnuNe8cZTycXdzYHB3p/lP+Ww/38kJB6nkhMYnooa
# aKeu5T0K6yjqoWl8A3A1xYib1nKyY7Pn1LvWzJlH0RCS/6025+Ro25xj+Mwf/YG3
# Fj1k33Zm/YUwi86htPLTfPl80PXMbXcV7tnrcCE+hMg4atrIUhh/zQUxv2gLM0Jc
# lvieta2nQdQNHgSXHV+yRDCSkelrn+OK+8afCmGmxMoQ2iIRqEYmdAgTTDwZ5DEa
# R7rEdFpV9JCtPRIFWvDJXe5kuuHsS9aFd5CxivGxmkv9TLm52ixEvhhicBM1wRf9
# qtRq3hqUEL1yXI2AKT6Bda2SJtJXgqulnWYxLaCkxvcrPMgn7pa52thWexa82TC8
# 3uUEvAOQ2XJAcxxM7FO/z7lH1Qn922gFih9aKKv2Qf9LR3iANlE3z9/ygrBhrlPp
# zXoN5Xa3h5HpdV72Qy+LMxZ3ozKGWE6dkr9vfsbHWib/4NBqOBzze1pJCC8vMoDx
# fBbv0SqHXHFXK6PR491TX3kXlQ5P79fV8flxUDX9pdAH4+h4U2WvxYTLW/Cl4rDD
# luu8b44Mg9ZBdrU7w+qhSFF0YBzOAuGqm8eHiSbVZ9SiCFcpX36OTA9QFjuRNrwa
# uyx3vzo19dJtzJUO3pr86uuc52AdCgilS1zW24VKYvzb+S5gF4l72O5yu0M=
