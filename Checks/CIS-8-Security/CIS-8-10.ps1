# CIS 8.10 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.10"
        Title       = "SQL Server Entra-only auth enabled"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $sql = $D.SQLServers
        ...
        $bad = @($sql | Where-Object { $_.PSObject.Properties['azureAdOnlyAuth'] -and -not $_.azureAdOnlyAuth })
        if ($bad.Count -gt 0) { return @(@{ AffectedResources=@($bad | ForEach-Object { $_.serverName } | Select-Object -First 500); Description="$($bad.Count) SQL Server(s) without Entra ID-only authentication." }) }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCSh8MSvGLcdiwS
# bFOLj79Co9snwDr8BRu6iXHXNkAAQqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIH6yLRTBr6xPZPNWo8Z+KoSe5xwoLxEaOBpdGscF
# RUQbMA0GCSqGSIb3DQEBAQUABIIBgJu/bFr3VGRlturupz2yy7j6ePvJjPA0cvjZ
# KrFg4LxvgA01QaoDcQcvSb74XB7gbrNTKyVk95u0MQ/AHFvAJRSabnYc3zJjFuQq
# gObjM7Jf5f4xbqBvIHlfFcPDqDrEXy3oA3ZG1SmkklFfwAuO7LLY3mFyG3+d4PJ7
# O2dM+LfnCddQtnmg4k3ExzL+f2wORvqQByz7Nn3kT1/FFqt7WCCIKD5RBPr74RBo
# I8RXaPskrjJIHVqS29S/+xfdhqDbe6KqaaQWTs0UrVUFtaL8DlSt7xWP7snZZ06u
# aGeHgChPQGs7UbJk8N3HGWmLXNXz4/3t5RERiJ/nhCw3VgZHWWYCBYjjT3eI5l5s
# ZbMn/NLiaM3QIYsHJIqlYWmh2qjnVX4P38MT2ikDKfRQDJtBvZEZvlvN4RBG3mxy
# F/ooec2T30Du6yE3XLc3y3SDLNV5QjCyDuhuS8+5zwMvy/pn3h7y30vNy0Wetexo
# lL9g8VYKMSAQRsQwa1Fag1cOfllh4qGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCB34Fdf+8Mhpr+r10pmGeBNrYVgCik5YAeBC0nBiCfn1gIGaeiBPL05
# GBMyMDI2MDUyMjE1MDMxMC44MjJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MDMxMFowLwYJKoZI
# hvcNAQkEMSIEIGdBMsXpvfRBgZEPY8ess3qrluCKTy+ZYhyM4Ndmlx/zMIG5Bgsq
...
# 9w0BAQEFAASCAgBCkhfFsbjBMaslCLAHznHS2I9DCabWfSX/j5kmdWY/qu30DlXK
# Oz9gK4VcL0NtYpH8PCe6aWdyImLgUjcrvtsJlRzIcGZ3BNjO3rVsDSitkHtV7+6P
# Eh9hbuv/L1SFF4GJKI6QxygfazVjCgTIWDD6loln05A4YhlWgFomWOwvz8IET1z9
# VCpyp4q7ANg2TQ4GYBqtw89ax1SDHFrPe0k7lhwpJxtXgnHFb1bRAT5sPqJRo4id
# +0j1fYDkYKkcW/JTk+zjV1SwJhaL0UG/d/OcVRC5gZbvB1+vZTxjswuybYzRwI/D
# sc/8DVxbOqzBWUuS6gpGRXztRYbjvwZ8BOyOZ92fMbCC8SvtXWFJ8fuRke2W81Cz
# 8HM72udQV/t+WxVQL76M1uiWVB4TkKIoYUG82ufRvt/h4HoW6f/oa+nbxR+wlI+3
# YKUQrOKjaR437TduSK3zpKQe2eUlJpAj+oVr0TOlrHmvi/0NvOcoHMX2uAexNTvt
# 3XhjuI7XT74Td+Y3PsdAjSX9jHkPDsxJZBa+lTcCsP69q/sH/w/MiOS08mvd65Eh
# S4+NqyTBbItk8B2OibWRlwFrZhmZN6H0a5EJdlEVCtGRfzDXNpdIHWNAOBEewWYz
# 9nacReKEp54S3iAkan9/aFMzetCd4PU+mKvFyD2+rLiTM+uAc2W1+8Q9kQ==
