# CIS 8.16 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "8.16"
        Title       = "MySQL public network access restricted"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $my = $D.MySQLServers
        if ($null -eq $my -or @($my).Count -eq 0) { return @() }
        $bad = @($my | Where-Object { $_.PSObject.Properties['publicNetworkAccess'] -and $_.publicNetworkAccess -eq 'Enabled' })
        if ($bad.Count -gt 0) { return @(@{ AffectedResources=@($bad | ForEach-Object { $_.serverName } | Select-Object -First 500); Description="$($bad.Count) MySQL server(s) with public network access." }) }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCN7S5VbJcT0VrF
# 9UGIK6T6S3WxSmGLSCNFjTMux+Pi0qCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMSCeBa6gnAT2TGIUPQSs3WtjQI6B11+YX4rxFIi
# xJafMA0GCSqGSIb3DQEBAQUABIIBgD6bdMcIX/B8wFzdTqviLKKwsBgVGXiut2av
# Oam5V28185C6JLDGnN7vAmr+akijgOytwaic55cEx9IxS2SFZWviPHpSgykpYibF
# qR/3XiqcrcxrwyCUfnBpDbzDe+LJiASro2MFoIabimSP8H5TEX2zUQAguW4E3P25
# mW4wHwQdphiQXdv8uuLnWeKjR/hemYTdUZjQu0llCXdoomcjWP2GKFEPEOQ9y/dH
# +uldglWcEMcDMS8b9hOR5ZPaCZjvRlR+JpKVHKNLtcCndX+5JdUnXCEbg2/Xs3FV
# 7EkP9Onpa5256nhi90Y7T0cwE7MfqVXyy966gVil2H41nvM+MSRLFX5M6Lbb2Y2/
# WTZD2W8mWfVCw/LYbAtkTa7kd5oOPqy6QwbGeJe2TBDYBuPuxerYJrdNDqdc2S+Y
# uQ9St9Fs0iCh05rb15H6SzPOYdN5IoiQddiJFfxJVbBD7O8OYOqa1JYNHgd/BejO
# D/IZ10sYGIC2jbZUEUvd7S355cMEO6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCC+LonrJPMgRJwAyApjnVIPLRf693+/UTNQ/7HWPTaHKwIGagxE12sm
# GBMyMDI2MDUyMjE1MDQzOS43MjJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MDQzOVowLwYJKoZI
# hvcNAQkEMSIEIJioWM2dqD05wqQAvFfUa/DSZFs/8VnL0dCRpIGfT+WVMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBnP3gZGs0TPb9Ac+5iore4n+Y++hDajNmQYy4a43hT2+2i
# J6cghHAHZ7x3WM1QyKU5S5rjhGYe1KZ0zn64/zpNtffM4GWLb/8T5GGkMsFbmCB6
# C+2XJjZarQDLXZvN/KKp5eXGzWmiKlQLumVMTCCgZtkRxLVEGTjfdAFMkUQ+QG7x
# WgchYVOR6xynhNFPgfEohYizvsKeRvbuC22/cqu/N+4hDbBHz8zZgXynT7SKB9Mg
# UUcvGHXv6jHJ1rN26/pmUGSCW6M6xQthnP1cFxu3ldL/ntQbmFwbA6qDpWFJnzO6
# r4j50j4HYMRdzR+7gffAmJOERorjNn8XKkF9jgO41yJU8Mr0l2eUDZaudXZwy4ZX
# zkriY5++KV0xno2biXUk8fq4/PYiypGLndqdyUf4f3/ggpsjaRZuRuuweSNn3Vn2
# jWSyOQGbPNVBVLpArn8XIdrqW7oLbbpGIiRVY+QP2Famz+xl0SK6kYGj+k9HyyWW
# bJ5VZdsnn53R6/ccFRDyX8aeU9GmSZlKavuQu8f4lcwK6qCwG2WlOeDYhjvLk+La
# kp0YbqLM1Fo0ZBp2Ud6bhTcn4wA4cTTYTxdX1kyfoW+rhL8Qb/Cif6IiHQ+eJoz+
# W2tfJ9fPW3gcV8lwl7TY4zHgKqMDQVNb+QMPJI6UPbKBhs5toyf4yjSz2U2I2A==
