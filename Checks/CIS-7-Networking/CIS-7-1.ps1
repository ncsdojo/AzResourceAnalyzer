# CIS 7.1 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.1"
        Title       = "RDP from internet restricted"
        Severity    = "Critical"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $nsgs = $D.NSGs
        ...
        $bad = @($nsgs | Where-Object {
            ...
            ($_.destPortRange -eq '*' -or $_.destPortRange -match '3389') 
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName)" } | Select-Object -First 500); Description="$($bad.Count) NSG rule(s) allow RDP from the internet." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAxgtID73ib3oOf
# ZUvhQ3TsaP4hNASv6PBtuLuCNNDyMKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEILHdzwcR7BsuQXO3bwbJpcDT+b2HuaZcE6uIzoPI
# a0BSMA0GCSqGSIb3DQEBAQUABIIBgJD/a8V80OTkK/NR1QcqSrTS0xeD8HXWnMX+
# t16WpN2u5g2FElGWn6GFP/BhWAbADVOD+Q2z8+SwvBj59vO6mh2I9Vht9DJd2kVP
# MzfZHhkRFCuMjZNsG0GDtVbp2bBjwD7uFp3x+DE7uxnE9Z5zLwCDfUimZJSGFVVG
# QUlZeKzjYmob3Z+sPSuyyVyn6w+a4UgpJuGuCsz/brZC+n/AHi4XL37Yn5S1Lydq
# tnjijI/HgMrLhhbtaClvyJQrZzmY7h9kBuXxA1d9pbUihXTIDaYAK7ov0yc4+Iqf
# huUEv8+174y2dzfFMz8Z51x9S31NphlmK9TIuhodBJkIJsIuzvdLFSEVXMfmscH0
# LyAvfsXnk2anXNsWDh2d4Fr3QgjtFuQvWeEadyeO9Uf6cQVer38mA/tThReJjesl
# pi38bhppuy2j4+LMOjKtcdrLZRgFYyIVxwguXHOwvdZKYt0YxxdphEVgCbFy7rhI
# hwUvd5RHVEJqWmzlzYANaN3irIDHWqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBV8JD30vdGlUrm9b+XvgazqokmePP46qDjXgvHtIOqUQIGagxE2Zf8
# GBMyMDI2MDUyMzAyMDQ1MS4wMzZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDQ1MVowLwYJKoZI
# hvcNAQkEMSIEIMYesmc0wucKiqAXH9B+tyRLO2ZIfOeVx2SlJo9Mfl1DMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCLEr4KCqkrDguh/oGjzRrKNxVtPY0HWj7rW8TLz6AyRBM9
# iNrqz3ptLnlsvfkdW3/FXNhuAxFvJ3WiPiKwtsiecxp8XQZC9y8JUVwg0WOA9gp3
# P0xxZrtFOLAiH0YPVnYsuWWb9YN3yajoiKT2ELjptdiZ/fRdzevCWp6tfwEJrv1t
# gtqupHUm3YrPJ4ZEbePF5tXbZe+CzqUM8pQAnstFyDeyY5axa2DwrwOAdT+y2P5j
# X0WIApZv25yC0nHNgLr7ns3nsjPVZnmG+eHHA8pIiQ5uQtXcfPGSMxTlmR4Vbx5T
# tqStgceVGmD+JKGj4uhRDrjxtNrrAXwfBGXSlrV5hBXgxIrIoVlAFlp9xGKrsMTM
# F9vvOMUhcdkRV9hn+2e/tVRQDL/tj9n/j9/ATnlxpR+tHwSHHNZ6QxBsdz7qZrPz
# jw8DlDTr9KtNGpzEZCrHVNjUPh9Whxkya5Surc02h7nr8hAyFBP2+wj0oEp0VfFi
# ALw0H+PbT0rv77nXTjV1rpR5HyS560/apEq793j3UupovkAov56D9DeGGKlUEKmY
# 4D9uTLxGtB4Em4xRD2rWpzngsrLg0ts5xEGIIsxoSOZ1CGyPUrSgbXPsf9veDLxc
# 4u79nUvizRmy1FD3NlBVprtmZj49RF/t4KnqIrDf9lcawS7Cemu4Me2+3K1npg==
