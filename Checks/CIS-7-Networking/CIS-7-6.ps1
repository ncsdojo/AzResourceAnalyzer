# CIS 7.6 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.6"
        Title       = "Network Watcher enabled in all regions"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $watchers = $D.NetworkWatchers
        $vnets = $D.VNets
        if (-not $watchers -or -not $vnets) { return @() }
        $vnetRegions = @($vnets | ForEach-Object { $_.location } | Select-Object -Unique)
        $watchedRegions = @($watchers | Where-Object { $_.provisioningState -eq 'Succeeded' } | ForEach-Object { $_.location })
        $uncovered = @($vnetRegions | Where-Object { $_ -notin $watchedRegions })
        if ($uncovered.Count -gt 0) {
            return @(@{ AffectedResources=$uncovered; Description="Network Watcher not enabled in $($uncovered.Count) region(s) with VNets." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAAApwlZezKXV8e
# uTTwwcHcolPiSCl/poHgpEBWHy45zaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIIZFkAZiW2ZTjZXa3VVD02jhjaUCw7TW81j5xXt/
# QhbkMA0GCSqGSIb3DQEBAQUABIIBgJ/4xqZJkDEUQGysxbjjWNfCQnZygF6ut1fC
# gdY1DrVP3DDXXp+33TKQgDMZvH2DEmJ2UFLcJdZYSwh46xPwBzEfgvfm9TyZJoeF
# 7PpZqU4/zLkYUyEApTZb8jopg18zFjJNbtz56vMQ2mB3+DkU5HqsVkTAiEVYuRVP
# 9wy76yEjrFmszVDRUSmWevhj/XFD/xemuyv0urPiUntMTl+nt98WKzXorj0Sy5M2
# lGxQ956ngSG722WDjRuJuF548VWMjHMNUEP+BSCbUjgEqOnkVmYSdWqHTplnLpUt
# iS9hI856LYhvUT54wuvY2Td4FSuTPnf64LKv6ll9Z8VZ1NPe+BYuSBKOUPpiw9CE
# kxL7zYNvsHymi83EVpv8JoORD5UkmMcFKVNvz9eh6dx2RQt9nsDuYoj+Dzt4euK0
# G2FUib0B0xJX+eUIHnAzFOd1ktFjFYR6dvozNin6IAKi89v6LvJ/k2hS+9btIBXx
# T+MDsconU6RxzhE9APMjtVSjUvS0JKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCGxzdrPVLPEa6HOroQxuBt3HS9rNgl3Z9RDcTX3wAS5AIGagxE115z
# GBMyMDI2MDUyMjE0NTYxMi43MDJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NTYxMlowLwYJKoZI
# hvcNAQkEMSIEICApljQ8xoXMv6DqLwYjy8buLFNaFkxV9EV5FgLqLevrMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBa1olxML+uqnwUECk4ujW2OTr1i4jIlcNuvdvUbDAtgjfG
# 2LC54KxtTB83aCF3G1LP2h0bDbBI/cIbSg23DxBJEFYz6HObni2owzytZUZE8bg9
# CyME4IQCXc2zIgY/7x+a/nW1UQf6NyEI3feV3c24bbH/3VUcI4ZaRq4+wvKgyqNT
# Rt25/IvWeis2A58RT1ui6UGFBeAnteUSI6v/rumyB4zfpASdQ8/ukx7n7ggbBtte
# GrRGlgrv/q0bBjxHHxSqV5xbwbPBSki0Y2KLkPwYF2bfJJzlERVE7nIJzpoNV/Iz
# f1IcJDWq4Yf0s4bvx1fJ+exfsEGDwSCAw3K1p6hXI2qMq4aQsWioR1tMGpcvsL2/
# j7u1Mz6KTyL8yAPDMfy0As+uQhTp7OLtf23Tm2Jri18neVXQ5a5U7FTBcnKBhZXy
# HjZ0QJ4NKi1LIlxRMG71S7wpomheyp/MU5OSjyt5QZ5sTEolG5jROCtZrErW7FHw
# pq03RX5a1Z5wiOV0OiRefRrFM4mz/54ePkV7lxES0NGyWak2dbHWZExu4WASCLKM
# wpacTOJAfGf6VUJmvmbKzF0Gdl8SkhQI9ywwGeFYwl8GvdnuT7RYXajtZlPIGBEX
# XC46ueg49+xRzwwUBnrHhAEm19Bgpyq8YrVro0UzWM5xVnww24H8bf31Y3DfAQ==
