# CIS 7.25 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.25"
        Title       = "NSG file sharing/Windows ports from internet"
        Severity    = "Critical"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $winPorts = @(445, 137, 138, 139, 2049, 111, 88)
        $bad = @($inet | Where-Object { Test-PortMatch $_.destPortRange $winPorts })
        if ($bad.Count -gt 0) {
            $details = @($bad | ForEach-Object {
                $rule = $_
                $matched = @($winPorts | Where-Object { Test-PortMatch $rule.destPortRange @($_) }) | ForEach-Object { if ($Script:PortLabels[$_]) { "$($Script:PortLabels[$_])/$_" } else { $_ } }
                "$($rule.nsgName): $($rule.ruleName) — $($matched -join ', ')"
            })
            return @(@{ AffectedResources=$details; Description="$($bad.Count) NSG rule(s) expose SMB/NetBIOS/NFS/Kerberos to the internet. These are prime ransomware and lateral movement targets." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBs7KuzwELejxlR
# 1f8h9AIZ3KkS+hyqStGRVMkPmQDGgKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEINpwOAtqkKsyVQIJ5et7K+UEp5QgnfwsQ3mSkZ5G
# PrBSMA0GCSqGSIb3DQEBAQUABIIBgFUZBFVVuq2WeAV68hvBnxKvHcWvp61d1/kY
# pxA/xFEyHGhNa1UJPSFAjxu2wOQ6N5xmWWVXFVvGJGSaKovckUNp/jaWBTqXR3lr
# M00F2fj/i9uHCegEjYXK+Z42KTsTe92nb5CefY0HiLEbxZ4sceuMPpMLf997dxxX
# OtGfeY7hXAl6jqseRu2MNs/YWsBOoRa5DyU42Hwt+WzLzTkmLOidx7HAcw54RRvk
# fhd4Ydg3yJZjoA8qKBIgrGBZkOPgbSVPE7GJ9RwWUfy9tu3FVSKNMT2bMDMDvJFs
# iYO2h82z8tP05z6mw7Z2A8+SJQUt3EnBMWpnEIz3pyMFw+naLC+pcO5yOCnv4qZM
# JxGFxYyZ9fHSefiXJG0jndMG1nZKIIVsyH3qXAkTEP2iawmYwU6ym2IlL9TaaWyv
# LAMzBqRYm0vqvknmD/5QwJWoUHnevvqiEK/V/yM1X332Rto8Btc/Knzct3FvhPf6
# nSeaMRqoS+5H9e3ApyWI0uvxVNDdxKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBqjCpKrRgNQ112mUaVODicpX/FXzWNYwpo986XzgXSPQIGagxE2ZsA
# GBMyMDI2MDUyMzAyMDg0Ni44MDRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDg0NlowLwYJKoZI
# hvcNAQkEMSIEIAIfG7KgMgcfYWON1/aTlZBroFBI6pl7oYYBkcMwNI7YMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCtSefJ4Mn19E/f4XNAf2yHeu2NFBrwPH1T1IsBe7tJF2U7
# w7QHj9f31MMaNkqoep96rmDuI2TllNE3mPm6DNUXh9az3MNVMMNjSCYA9t2giMCx
# 48vOCDkcvG4+zBko0oyg9OSnX+8FaGdsXtMg4v1oUfkPCe+TgtKQ9vrrpKrVMfC+
# YmWrXBFJACYiGqBItf7f3KMxt1dm5GKQXrguMtPS49Unp0WSIo5u4HqNE4sHJQqC
# Rgjoz4oDVaM/oYwRUJqjgkvjgXntcne3SMTe2L8O84/9nX0Mzn7UEYbObV1IQiDM
# 8WYBHMAWrAdjfELD++Zr5VT+FB/JWObgiI6tR5YdSnApDpYs0iCdWT5BZUDb2LoM
# jzwAr1l7xuAISdCzyXsZvfwXsfRF1F/8NNILHK4kuDubem8CWof/r5k95XQctylk
# a5xCYsA9PgpweYK3+gEJtPBjRVEvxgJ81E4XeR1ZfyjDANhV4V4Pjq6iZbWpBDL8
# TMhu1nqVzeRXFCuqxiMeQ4CFO/91B+QzueKXqisPqbZVpAynsLdmh8IR0V9zAYCs
# tS252/v2FpV3hnBWdsvCaCuWp4yA62X9B4LX4eJTBOH6hlWeR3H8oxnWANskKbON
# LgjdvxuZVu7yoEqE8eIGiqfPK1ZxCmdqdtzCNPHgILiHmQTczSzBdIImi5GoZQ==
