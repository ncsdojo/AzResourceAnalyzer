# CIS 7.33 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.33"
        Title       = "NSG VirtualNetwork tag may be broader than expected"
        Severity    = "Info"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        if ($null -eq $peerings -or @($peerings).Count -eq 0) { return @() }
        $vnetRules = @($nsgs | Where-Object {
            $_.PSObject.Properties['ruleDirection'] -and $_.ruleKind -eq 'Custom' -and
            $_.sourceAddressPrefix -eq 'VirtualNetwork'
        })
        if ($vnetRules.Count -gt 0 -and @($peerings).Count -gt 0) {
            return @(@{ AffectedResources=@("$($vnetRules.Count) rules using VirtualNetwork tag with $(@($peerings).Count) active peering(s)"); Description="VirtualNetwork service tag includes all peered VNets and VPN-connected networks. With $(@($peerings).Count) peering(s), this tag is broader than local VNet only. Review rules using VirtualNetwork source." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCf/Nqo4ZIz57sb
# 9jbjPE3LPVVdVulJOofLXM2I1XRx0KCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBTKITHP8sGibX9mfY8Dk9GSrREFSGgv7snBFMQc
# v9pTMA0GCSqGSIb3DQEBAQUABIIBgFpaNzLKacJzrWVqFWg0BLsBeMxhZV20fjlt
# vQpWgZSUbKL3ohz52Zf7z4uNJzQd/cRcEPrhdiKdvA7NZunDC0sn5WC1EgbQt13Q
# kEQYPMXNKn3V+uZEC21Zxk+/KNJpqiyPnv6xBbhbpSSQpjLFH2Rin1Giq9pqkOS7
# mCJxE934fdfsoQC1kFJ5LBppBvM1MP35C3QMzObWn/vSEgYUrOIsWmxMdk1Dkb7/
# qCMXn7n+zVG6irDM/v61PqDyO+0p2BKbooH7farPHi4DKjWc4pDAM0Rcd0JlRpBx
# xyS7Ti1I5NaoEok2NRWWb2GKt9wEqhrBNVoLijWWeulctE5qsJJdVHdvK0SuhTdB
# kaAAHQOPwitqtnbgT6AslQr1Cs3Khe3o8Yro0SUh8ez1xGqh7VhPnu1BVLxtjVyJ
# JTY4Kza6EmjAU6W/1kdBa2eHbUTRyeHpbbdKpLGw6lGdB3JcAvb+b4qPVJENhSSN
# 9tuzsD5u0L28onNFmN+npPyRyfEx66GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBYcTnmC08zOCB7F6ZeMOtc0ze4nZW4SEFRiIp79EJJEgIGagxE2ZxV
# GBMyMDI2MDUyMzAyMTA0Mi42OTlaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTA0MlowLwYJKoZI
# hvcNAQkEMSIEIG5brEi4kdGedWEciPB1eFVxgf35B4QwzYOxGiZROHrMMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgArA0YbWkwwBVzr5RjngR3vz0265wRAgSG4CZHiwtKPR6oR
# I6dppwUGwe8QGJNrCAyFHc15qFVeMy1NjytRz2Eg/gYsJW0Wky6CmVLrxLC5CVzC
# 5zDscF3tuTHKTdrNh61/aDRKZNHAscKYivRwo3c6Vl/PPyPXUjqQ5gcWV60DwdUX
# R8TMU2/AyK/hHDQXeHwSAgUcEHIGbcWZGvrHZmgqYyLeWTzhFqdGP3Dbb/9aA79J
# ZcNFEeCZ+LodOx9PithcQRkdfHWBtRGPokHqX9bGg2Y5Xv25555x9hBpXIxJppzE
# NTRyBwS5IqiTbT1WxKjjHReN44lCMG2f1K5HjS648x3++gO8k1ZiBw/o40QwZtgq
# F+R9n6tQfcthXiNCx5z3u/npFkWCc++qVfUWjkROnYPcrYofH6qvX/VTwI4m/GG0
# Nei5YtYG+G0bIW95gvRsJw0OdDrzYJUuOIKDbD1DPIQRzsIqiaPB3ecSXnUFc+bg
# qn7fDafZlEw55PbX09otttxaT/1f4OCqqmN8Ce+NeR2HigpSAlkKXiuGk+LMyhlr
# AZhRh28n4JrlgNVJzqqoSRPC4vQmp4UT7euOTWvC61hH2YYYko8Y4te1y3n43FKS
# UFr7sPwFm9sOkndey24c67J1vVyPChrE41RV6k4EdDvYAVEtdNprL3ae4UJw4Q==
