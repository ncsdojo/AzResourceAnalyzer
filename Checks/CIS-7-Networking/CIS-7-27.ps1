# CIS 7.27 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.27"
        Title       = "NSG wide port ranges from internet"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $bad = @($inet | Where-Object { $_.destPortRange -ne '*' -and (Get-PortRangeWidth $_.destPortRange) -gt 100 })
        if ($bad.Count -gt 0) {
            $details = @($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName) — ports $($_.destPortRange) ($(Get-PortRangeWidth $_.destPortRange) ports open)" })
            return @(@{ AffectedResources=$details; Description="$($bad.Count) NSG rule(s) allow wide port ranges (>100 ports) from the internet. Overly permissive — narrow to specific required ports." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAAJ2rVtXOqL1vd
# Yqeub+nmg5m6IcMiF8Vw1YY5RK7vm6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEINwnurdq4uNytlQVnEX8i0TzJ9Hlk1KtRIBS29fZ
# pPP1MA0GCSqGSIb3DQEBAQUABIIBgCAltm193OI+USTaz12uPnB9zvlY8j6wj+k1
# 1OwEEuDbEtARCMcMQtfJGDAH53xOKvgjQS8ndiNKU3VJB1RXDNtVUyoTYs98BabL
# DMiIgzgiiB/AWu4HB9mDI4iHGJUc2X7KvT50qh1oml6whrKhcNQ5b6XLtP3yg8tu
# nKfkHNs6/Xxx0AQOB36Lreojwr9FFVdqzLgBaMsQBhGyCzh55iokvg1EXsyuOo+n
# 2GsTaS8YHd0uKbWJSef0CiuezZM3crY5VNZTXVvm1J05FvStLLAiRsUctgk9G/9d
# qxIZBUQ7iSjbQMo+ec2jiqUIPesgP4HNpls5KUUQZOmzS0FEag8LV3AnH0oeGsou
# pneKS50JbUhyGS9uUzgrDGyzOpqVSyah+GfE5mjj2APbNN8NItxhxiwCBFuvY7OD
# jLaP+o1dL1tmAokODNhIgZBur9CfbOQqVzwnu3dA1YU/daJ8uxae9JjM/5FB+Sp0
# t1iBkwwBnf44q3fIBuPEe1/aAUuck6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCB+aL4EQiKQ6Yf+kNBnxyKrGX3SGuvNNo1W8fUXl7uM/AIGagxE2ZsM
# GBMyMDI2MDUyMzAyMDkxMS42NDhaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDkxMVowLwYJKoZI
# hvcNAQkEMSIEICDgxA0JcYPCA2spcOFNOsJMJ7Dp+5ssF2d/E3vWRzVEMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgA8VdtPo3xhvgqSiaYPB3x+E8dpSYpgxAA+S07Hv0Z/hfhc
# msREFtV248RfDugKfdUht9ctnxvc/KYt1ffMLe2H36owgEt1o45WcLrFbafVWbGA
# pSG5nCo0A4flWjlYhJmaIKyl7w7mNpekec2iLfhMgCK4+cd4rTLgjEGE2u2LRm64
# qmxK5zZXmRcBC8aWfgwPmaW37ehYlFFnrqSILzDZvZAiyFdZRuGweMR7oKqkCSKt
# z2SIadBtzC6D4LXW5+B//LPy4u0vB0pDJEPi/oNJnQCYbcn4X6g9hbSVZmIcG3I+
# pzBGkiQXMUlvvyAAxAOft+KRqSPpVUU6Zz4e4TgB+HMljideiQeyLZKX9O/0r0TF
# PQZHV6h/658DLS+3qWOaGI6Tv3SLbOj64ieRADz+B1yu3SP48YbGs1OsC0UUok7Y
# Hh3vCDLWjWlNAMwCxwhBgmjovB/U5/OXQM/4VggbfeYJfPCSirrz4WhlhaB7SSVR
# LCXkGE8P0P+MuoWALLuVxMafqAYha5zse+2bmbvTIvjWlWgTcjQ//R/HTzSXDEnb
# ZLr6VX/chyMl72EZOMiLSxw018KBW0u4W8I49Liq3+FqDOba3ztWB0vIJklMFmEr
# NeqlJ/68E6ybPQ8nNgWLoWdpAWMvpNq5WWM5y/hcVxSbpNAx0JhDinZ0VKdBeQ==
