# CIS 7.19 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.19"
        Title       = "VNet encryption enabled"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $vnets = $D.VNets
        if ($null -eq $vnets -or @($vnets).Count -eq 0) { return @() }
        $vnetNames = @($vnets | Where-Object { $_.PSObject.Properties['vnetName'] } | ForEach-Object { $_.vnetName } | Select-Object -Unique)
        $bad = @()
        foreach ($name in $vnetNames) {
            $vnet = $vnets | Where-Object { $_.vnetName -eq $name } | Select-Object -First 1
            if ($vnet.PSObject.Properties['encryptionEnabled'] -and -not $vnet.encryptionEnabled) {
                $bad += $name
            } elseif (-not $vnet.PSObject.Properties['encryptionEnabled'] -or $null -eq $vnet.encryptionEnabled) {
                $bad += $name
            }
        }
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=$bad; Description="$($bad.Count) VNet(s) without encryption enabled. Traffic between VMs in the VNet is not encrypted at the network layer." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCALBVKNhNmVIPzx
# rRHETaT7lMaQGpx3Cx/YgH92XsumSqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEICeyb6/J12gUFNS6D6e9YzufEobFNKlk/mh62xVO
# FGauMA0GCSqGSIb3DQEBAQUABIIBgGaIZxSaph49+vKNspdbqx73G7uNzM4Cy4RH
# IarC7If3RBlqZZlWgS0Fmuu5ya5qOFFmcoU7RUq6t84Wgj1MqAODVY4eWv2/18/g
# rt3Tdyp0/YLz2tgCqkBj0I+sgZwbzWBINT1ONlY9piEuNfypBdR/RHov/QlGrJFE
# rZpFkH/VZNtTmvijrHiJroUSCAnlY/wpx+jTcNVv/VzSwXcFD5oDa4via0JZIUJh
# UNCZlBGLkTizQNhMX9uSdfwDJkHuMMpsNL0bsaN54y3UjyXSE7SNnpYLGQQwM/wd
# UX1oO9pFLjn5tn+LYuUVQbR/i9fRi7972HVaqi7ayAgVTb9rVTPx6obgffhMZf4M
# IqLY2FBR6ytjI8WeLGiv+wzx5xTcsL4MGnkPKViXAgGj+TsbeUoei9hzeH25nrZE
# +lbCNxuzIQnVbOnzsgLYFzY6nTnZouDJhUMXQbYmAquQLkG0TdiSuKSYj47uYXKT
# RQ18N/RXsU9ZbC3udtk+taVq2YAOUKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCPsdKCiCJuKD9iZUhwqLXPJIwXy9abhhDsl5v1aM8mCQIGagxE2ZmT
# GBMyMDI2MDUyMzAyMDcxOS41NDZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDcxOVowLwYJKoZI
# hvcNAQkEMSIEIMAQfYaE/6xHpmKJECKMjwzjo8LBe8b2GBZU4IndZaJtMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBQ1rtjyfEtb133PCVeAuszFvxL+LXJlDrI02DuSI/5tcH4
# VsdClOPt37ahoPIK4SyfjU0Fkq/n0qN3CX6OM7L8vIUNstwPNI7n2XBC+A5EbTNZ
# I/XEzyvvhd/Kd/96wCzsXBfjWLO3q1gdp6LYN3Li2ilLct6nWWE21kjEuHdaZtnl
# EAv5P9IM5ofB6C7jXVy+KBWH0SEnL8NAf+R3ByTaTl78hVqrEUqV9F2sp8rJ3vA1
# BefEJByBcWXWa5YZWgKkUUwYpDYxgAxQF9R1qg0SJbfMxU/58DzLOQMibw9E3Z3y
# oSj74stGgWzTcT0UHFo/cx/+17RAc34DFekqVtb39NV0N35qTa8scJd4Q2aQmTG5
# p9A69qGKvR96DcWxnUzQwScagFyEv8JAzJti2dAoWNNbs462ZGPzYSBeIvXfMT5E
# D9lIwt+QmV8pnnK//sNYi4SAtXMzqqSqjp3heN9VxV/4JbNGGnWLsUVulpRWALRP
# 0/MRs52SkDJcFcs3ImdBoo9QZmslx8qlq1rKzFh1/mhQd81t5BiWDLgSfQs5mXIt
# n4zLWcCXFzpWf/LRGBh3NN0KqNoOEwLZv4AMJsJwK8AHV33VEiAlplzEMw9m0zPk
# BSKBOaHXY81ApHoKF0MfzWfizp5QYEXiiM88ck1NmZaUzJwy7s5aNeB7gjQB5g==
