# CIS 7.35 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.35"
        Title       = "NSG rule risk summary"
        Severity    = "Info"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $inet = Get-InternetInboundRules $nsgs
        $customAllow = @($nsgs | Where-Object { $_.PSObject.Properties['ruleDirection'] -and $_.ruleAccess -eq 'Allow' -and $_.ruleKind -eq 'Custom' })
        $customDeny = @($nsgs | Where-Object { $_.PSObject.Properties['ruleDirection'] -and $_.ruleAccess -eq 'Deny' -and $_.ruleKind -eq 'Custom' })
        $totalNsgs = @($nsgs | ForEach-Object { $_.nsgName } | Select-Object -Unique).Count
        $summary = @(
            "Total NSGs: $totalNsgs"
            "Custom allow rules: $($customAllow.Count)"
            "Custom deny rules: $($customDeny.Count)"
            "Internet-facing inbound allow rules: $($inet.Count)"
        )
        if ($inet.Count -gt 0) {
            return @(@{ AffectedResources=$summary; Description="NSG estate summary: $totalNsgs NSGs, $($inet.Count) internet-facing inbound allow rules. See individual findings for rule-level detail." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBUyhovcrm5HAdr
# 5qpRSAm3ch2uOJcMDgMBGQAvNpJWp6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIDkHJXR6xu1b2CVfGVrKKavhyklH6PvK6UDC3RyX
# en3fMA0GCSqGSIb3DQEBAQUABIIBgJTsYRbc/wu9+wwWrPvdYyBFXL62YLaiOLU/
# iXU/hTsJeAn/3nzCPzn/mG0t4XSCZM8NSamS5W/rAoPIcZwxi9bhZLgD2bTsk8hJ
# xThwf82fFInY7iV7e4lAeOfle7QInM7bLSicItuQuLnQT5CuLGP9uEAifrfpWfxE
# pLTZoUJLK9oBZf4QphII8Rg6C/d74aRMfHTG0b6d4QJWtJuu4W+NYqC/Vyy3Q7Y9
# RKdtvvOWndClg8HN5D0gFrXwa007r9S3zI0nd/WBYQ/308dTXec3KohWM/QOJVBa
# kaMxjWsR32bj2TFP9NQa6liQ6NroNfe2nQnnIMuYNMBKYDbXwyo4PfFdpR5CzTbX
# 3YE1q8+jVNJX2MXUZakuuG4eX5NWKiK5wRxeLPbYgro0NMQmbEUnXfP1dI5ZUqCY
# gaXUpg+5ThtfCnMEDyrJgC70joks8k9j9FON/TsH5bxA+o7ZwMNQurQP/oaHqKBa
# DOJjYvT+8XaS/CouKYJ4/bd46edR/KGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCCAsDWsEI1sZl7z71yfQtp9w6xTx1gsQeBcicpcD/rEbgIGaeiBPLGT
# GBMyMDI2MDUyMjE0NTUyNy4zODJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NTUyN1owLwYJKoZI
# hvcNAQkEMSIEIFazqpohoiKyqMGr9Tc9hQ4s8ww4wlz/nQ/IH4E4F5PjMIG5Bgsq
...
# 9w0BAQEFAASCAgCyheEJgfzT12YbFFifJRWClCNt/pNhP8YkT1VgGoHEyEqr8cGX
# 6S99ZUwmLgxSzZe6xodF2P+pq0XB3zxbMtdLJVKy8L3VhfMknM4/qJWU1soT1oH1
# 0cngxdIAsVHfE+Ro4LdFviUs9Xr8FH2xko2OoujCEkytv51U5BnVPlWjVTINC15F
# uyxlXXHEzRkaARusFE5YBLkE+ATsstCLDk/V6Y+b90DaELx5d2krbGFHpUx7TbWn
# 5NmpsSusP0YnWQWOeeHJsinXFntStC8vVmNBjsQNwhYVBgyjhoEtbCFmoGeCh3ZC
# cRBe66+dH1fPr77/ywaXuj0lxdhC/sencRQg1HoHCPHnfKg1GGNeRRzsHSdrVaGU
# OJk5gq/SaQqdkzqaJ8vN6voC7WjbIQVqGQGCPg59ru3uFUJhBsNdXt870r0MvEZy
# kHU1HY7HcZiBwcHbk3d/Hv271n1WyqwGUxgWO695Y58gG4SVMAhgfGvujgtS7whu
# qVOoNZFt447zx/PDGr3+z7AWai9HvBCOKUDoq7fXtdIqz6nfMq+XnlLA0BTl10TN
# ytZGS8ohL40im6evbDXUBtGF6tNGch8lHk31xY4c1EryudUYPzCIKSUhylPhLi+2
# Os2179CBaKL35FPn1HyTkUdA+JYOpIBDmM7W7CthLAWzb5G921OefRs8Mg==
