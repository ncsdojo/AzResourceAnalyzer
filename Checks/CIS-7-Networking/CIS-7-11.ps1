# CIS 7.11 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.11"
        Title       = "Subnets associated with NSGs"
        Severity    = "High"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $vnets = $D.VNets
        if (-not $vnets) { return @() }
        $subnets = @($vnets | Where-Object { $_.PSObject.Properties['subnetName'] -and $_.subnetName })
        $skipPatterns = 'GatewaySubnet|AzureFirewallSubnet|AzureBastionSubnet|RouteServerSubnet'
        $noNsg = @($subnets | Where-Object { $_.subnetName -notmatch $skipPatterns -and (-not $_.PSObject.Properties['subnetNsgId'] -or -not $_.subnetNsgId -or $_.subnetNsgId -eq '') })
        if ($noNsg.Count -gt 0) {
            return @(@{ AffectedResources=@($noNsg | ForEach-Object { "$($_.vnetName)/$($_.subnetName)" } | Select-Object -First 500); Description="$($noNsg.Count) subnet(s) without an NSG." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBHdWtUfoMIdCir
# 1vSbq6dqZZF86sDZnf/bp1xqzfZrZ6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIKaQewC7juHqjgP8v/vTz9ImET737mO/8JTT2FLM
# dtULMA0GCSqGSIb3DQEBAQUABIIBgAzkMUzMIYnh+taWFwVEg1dUyXBt/6+pRq+v
# JFXZn3EoSBt8Dr/Mr1xtV5VyzIoD0N0plnlVzRpZKLdfW/7WuxGGh8+LmrMOZ6Nf
# FKt5O8EC5YFft51vkD6AVR9wLmCYql/lBxz+cHLmbuEepOElx/VRbc5ALB4kLwys
# qGw8/1kZKA6Uq/OHgWt4MXJFuebOAhUSEF3cyAAv7PVvHfFAzDXkMSHwQkPph8TI
# FKvL+4nWDCCiOIomrnbDEqHjbP8A39LYwZafjC6GxYnUtlAzFSd/0nX5tKTQEiA8
# 7JvUwuZaOT2wqh+TQnbJ9ll7LzNlBZku4gr/JqSs6C9AkFHbWj+sr0vqKiJx05gq
# DWIzbnRUyZ3rsZ+ZQCyDvzr0MfKUB8QdvucGowiRoQeTQh8hohclCYraRzvRHn7u
# 5+3Hj1oyU6qGW9cdzB7Af2DvkzOucb885nUqHXcbk3eKNmjWlUZBIy+0wXM3VZJu
# P+g6tr2M84U6AicY2sxkZaksnrkkCaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCC+VooLZEnnERaCUQ5cJGwT3tIEO3Ti2e3i3CmHVY+QDQIGagxE2Zgv
# GBMyMDI2MDUyMzAyMDUyMC40OTVaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDUyMFowLwYJKoZI
# hvcNAQkEMSIEIHaOdaxC+7rFRjQjz9MZM+pU7VtycUE6iAAN5HrJMd3CMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCKEbaYZcLHprH/JTek0VACq19uaPcUMd9Z1cUfjinoTjZp
# krGWLoRQVRi2GjsB+uIsH5JF3z+2yAnP7gHJr5IU8XgrOLZM41+wFyo8lS4tG+nA
# w7x1SL6ICYvjPaE3Yzoa1IL/UwDyfS7sbIHjFBf8gkx9T1QPYkyNN+RalaflEcDt
# /Ygz5Q+TpxBSHoXlRJAq05UO+iWJhhVR9TAr8rVkNxE/xhCP1XuIhVBgqgyzIi61
# fwPiLbeM0+AcmP8fKPq0Lyl1XbJldycZFF8PLp0eKIVTNyaz10AvEXQeDfLQsLZt
# VCmoVuj3zT1GkbCQ5XzIkhLWe2OL3FGd1EZ+NyRMMFRZcfwckPQnP1f7NnR7LiyV
# ZlPfMDF0M1fsGsIK/IGTLh6S4iab4AITOyIjvHFn1O3TqGLvd9ICx4J60ZKXi3Sr
# ejgUf6+sIcg3YlXynTQjS1JLdSJQ+7/YIEbMicn2CFZvrng/PBHfl8Udi4ivSqa5
# aol5qkO4yJ/Js286oPo5CkA890Ud/UNwadlouYuf3Nskvxvb/iNxGo3gzsWl9Q4V
# Minw0bo4yIXVFLHb2kPYzKyeGEY/bABFfHQ+C9YilWJsV/CkvAtQwNoWCDXlb2B6
# IAkm6JHqkYveJsEWmrxLn5kYCsuh0XQvoTeUaLWDzFz7n4SoWtChfFCOW7jnTg==
