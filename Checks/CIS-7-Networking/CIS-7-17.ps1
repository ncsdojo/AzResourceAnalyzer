# CIS 7.17 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.17"
        Title       = "NSGs have custom rules defined"
        Severity    = "Critical"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $nsgs = $D.NSGs
        ...
        # Get unique NSG names
        $nsgNames = @($nsgs | Where-Object { $_.PSObject.Properties['nsgName'] } | ForEach-Object { $_.nsgName } | Select-Object -Unique)
        $defaultOnly = @()
        foreach ($name in $nsgNames) {
            $customRules = @($nsgs | Where-Object { $_.nsgName -eq $name -and $_.PSObject.Properties['ruleKind'] -and $_.ruleKind -eq 'Custom' })
            if ($customRules.Count -eq 0) { $defaultOnly += $name }
        }
        if ($defaultOnly.Count -gt 0) {
            return @(@{ AffectedResources=$defaultOnly; Description="$($defaultOnly.Count) NSG(s) have only default rules — no custom rules restrict traffic flows." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAYxmwUCsw1XM8o
# 9S+Sz0uu3hOKO9A38k+wBESaQJAsjKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIDbUNU9RxZb/os6pTKE0QLETv+iIprGihOMcqpiC
# ixs9MA0GCSqGSIb3DQEBAQUABIIBgGgCac/qSGpnq7eU0TK12TxwORtGRqACy8G0
# jOQG9yolY+QfN2dCjfyP2ZXFdC70583Mh1wgdFfXTZKB+mOR40B7e978qvI/eF75
# pArp8UA6+bGeMaGo02FG9XOqiEhC3DSnQ5nzuKnb/eVxhkOl6AybQPkmuE0W/m0Y
# 597Zl2kIbJP0Lp4M5eHaE6niutHwZ6DyKzyzPUrHPhE9lhwH2Emkc7v5PaIlJxLC
# frTJhM89zas/uzYiOAQ/ablzUPbD4ZS6uq1myxxwMZhDNHCPy9gCyylsJooisXOq
# sIMpUnGRvZVPsKVhgH7DPcLXfw5pkTIsHnqjsea0Xie/sYWkX1cHHWjLXFPZTVMw
# OC4TlSHCZPo5Ok57I1k/ihbc9sjDBPehg7MrrNKvBDCtgtUXPvh5bJbCKEZzUU47
# yeGxSjM5/KX7oMziGK7+c3UET0oR7ph4AJdb3zZEsnvhnUHomrefOw57o3gbUUIZ
# 4DBCRCJrllH34yTA9mWqiMQUTTcJR6GCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCCXKh1eMePJoXlZhtN5Kwp25uEJO1UP8R9OroEoQM6hRgIGaeiBPKxh
# GBMyMDI2MDUyMjE0NTAzNS41MzdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NTAzNVowLwYJKoZI
# hvcNAQkEMSIEIHcXJdtfjnwuLWewhj+k/+CrNuelrV3+1pnVj5IvDownMIG5Bgsq
...
# 9w0BAQEFAASCAgCGUxX7OnT07aLVm3MbHJvCxOc8A649ct9YRxsGraXOFrD0V4Re
# pdcRo1wgYVp0FeFYp2QLJYRos7Ocrd887VFUR/iRcycBKQ+g/mjzuIsTRee5EjF9
# HdNk73tqIRRUGxR/wbqD5/Zh3unQNdjhZOSks27QZ5H8rXInNKZSI3EBuLslly8g
# L7jUrXYv1k+f6IiqqebTOqbWcwMVhQVzptNF00hPczG8qIu9REgl/3dkf2m5vaNz
# rnwZlzJuEI/DQU4vZmK/Gx/EW2RAHFTsmkOwGuRxy4kjfig96CIE0OnSBGyXjoSn
# ejrNiQbHVRKiSIBceP+7svz3UoTXYQohTo4t+7IZGkq0PBi8MD6IsXF/eWM7UeGO
# Up998C+dkWHltT7B2IJYhwndvHSAKy5ORlr7Y3HvPprm26end3R1D65RwIu/la8a
# 5yIu1IhYcIVhKI2BhcXfry1sGp67PdSfFckbclcdJjgjvAU98eKnsevap7D+M/Z4
# UFt811pxN+VmRY2AoDTp+aGcPIuvEY56+bEYzmzbZzUT7Db26INneqPzFMCKkhLu
# igc5c9Og1V1eqroioKJSc7QSnmBb2FBiItQOQe0nyP3UvhTfgyLodrOJkWqRCFxo
# jTSoM60DQcu63B5KnQivBcGutf7eSFW3CC9InqIgQx5IOgS1DvBzIXXF8g==
