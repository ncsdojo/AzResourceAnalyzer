# CIS 7.22 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.22"
        Title       = "NSG inbound wildcard source from internet"
        Severity    = "Critical"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $bad = @(Get-InternetInboundRules $nsgs | Where-Object { $_.destPortRange -eq '*' })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName) — ALLOW * from $($_.sourceAddressPrefix) to ALL PORTS" }); Description="$($bad.Count) NSG rule(s) allow ALL ports inbound from the internet. This effectively disables the NSG as a control." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBk4ahdgKfS6Vsu
# IySB6FoyY90rANhuqd8OpNyc6OyfjKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIE4poiqOkUsD3biIJiBMcHirkqYheIeBdgUB7itG
# CtnpMA0GCSqGSIb3DQEBAQUABIIBgHPKnzVyb4R6uH25ssZ2uHdUlUZUehEA1mAZ
# yLLFKV+OCHAzPcGXpi50XTzZCmODQx+R4SKKOP9ekeM5x4zcixg0cgp9f3OSm0Zw
# 0jLOLvisdlJ2y4ue/9qliUqjVta1PD2W9e0NywmLRfTcHuW7TqT4i71wWL6WnhQp
# hPYwmcIa1uOk3eIaNSU1QnY5C+rLXQP4Wv1yKR5OnSoJP8KWj1LubGJLPKwPBHtk
# +cdqQO/ZQlnW4XUSQksDBYMrRiQ+EjfT+UbGMi5kkp//YFrbVZZo2ZP0mU9JPUYQ
# lH2cvaR2JLt0p84SL7Da04WsQfZ9Iv2VNpWCmac54n5XPsoc9N3PuaZ0eJFLGbMs
# 8CFyFe2rNi5nzInLFXkVLpDxTwGlMwLg4Lz7zin1W9H8Dd/QGidiTXi/4f0g+Qcd
# mOa3CzrF4QR0EAfqqwo1tTysLGZK2tJ8y/RzO5teJ/z+TssVFeg/E209qzCmObxz
# Wc/7t3A37EUeaUBEjJDs/BKNvfn39aGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCAH+Xulq0h5ggN3zn9WDgQVMWcfmzeRdJjIrmGMc5StYAIGagxE2ZrA
# GBMyMDI2MDUyMzAyMDgwOC45NzVaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDgwOFowLwYJKoZI
# hvcNAQkEMSIEIDqlIV5QARhYtDn2nu6Lv9eDBzBoJY1dN132mk87IjLTMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBjxyT+KOFnbg+2m7eJql/mP4MHbhdpDNtLzSj0r8M9AXGs
# kx7do8mmN0BBD0nKcCobhFwgp+YYqpofPKQXSaC/gFY9nYjfo71gK+mcS47oQn1U
# KOSZMqtEdtuGKcs8LCChKtVYpcfBqsIx7Bl3+LU88hjbmImci5xmxwPLzGI9K2BA
# 9t/pS3PNd4yuXOLoFzam+KQdzjErLcIJl0UpRcEfNRmWGFdu1VVQEOSh2ab0j1Ng
# 4cDFe4b3Wqj1g/vLqZbaV30l827kaRUNGYAGibKPtTiTS97l9rinSL9oil2tlzao
# /Q6vkSCUoH+uFYlX6oGKjhMZoAC+cmU+CNOuVN0G2vNdt1Q5hKs8vop9PjbHGcv0
# aUSyX6TFIhwgyslKIpgkFTKjU04oDHQjJJQ3NUx+sZsoEbg6fUF8mFvkullOQdjc
# UkllXn8R/54Coy1RCkmW55nnt9YOBmwsolJsHtRVVutmh5tRPLjB3eVTW8iNN3dJ
# 1UY+NkTMcc2RuRQxb0PpOURIH4DsFGlpHhkWj5LEo8ZAB0qvi1obhLG30WqgcYdZ
# YBuahx2fejPiaAUdCOA7OwmBg0A2ICbzvNozwo/FgR3Op7OqtV1+dGPmikDlfquH
# k98EY46vj1hGvOwkh9VaCuGOl2LsfDmKbvuwzQXone8X4ka01W+D49fojiXZxw==
