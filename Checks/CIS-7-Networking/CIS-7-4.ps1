# CIS 7.4 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.4"
        Title       = "HTTP(S) from internet restricted"
        Severity    = "High"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $nsgs = $D.NSGs
        ...
        $bad = @($nsgs | Where-Object {
            ...
            ($_.destPortRange -eq '*' -or $_.destPortRange -match '80|443|8080|8443') 
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName)" } | Select-Object -First 500); Description="$($bad.Count) NSG rule(s) allow HTTP/HTTPS from the internet." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDDEKqBal69vxDw
# xDTe2/CXRlQ3u6NszyJGF2y6LJjMHqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMdm77iN0srbYHe9NsjsLgt0bheYEqPQ7zng3PS8
# xZ64MA0GCSqGSIb3DQEBAQUABIIBgE1GBfVzqpf+9rBNVamoUw82io9o1/8ZOrOr
# ayT4ODO8tY93s8R9TNVdq3UwupFBleNd08HL6AjszLsBAWMwc2HjrgayqLNShzUv
# 5JhdfogcYb5WlJ0a3jonuEG2wdoUmMG/yIaIEZ51GVgeNRVtYJOjcpGTxuj7l+If
# YP650q4qUST1i8lCrZFjGKl2Fg2kGGxZQcho7rLrQ9sEV685bep0kSpVDqr6Dmsc
# n/eyILK2papdNqOYjTbUxsmUrxEc5i1DOg+kQVmaKx4ETP2zDgkHv520I0wt8eI5
# cV/fuA+rY2P36hQscCVBkrzn/wJXpkYqWrO3bUFxlurzAAu0Ydqe3Dhm3z+j6dyA
# VJx3jGyXPLeAB+wuHPTxyqcdin2BxP8d7mggSwxp/QV5xZLYnIUkdD+/mfka/f50
# hD7tdCnrN8IuVqA52V7+v2tGhAV0CRIPFEAksNgvzbpTqo0MmPfBv2IacEQimusp
# 9UnrE1wMk10mhzA1Ty+57vW/+N8QF6GCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCCpgud2t7dzkp6X/kQ7psMJUkLiZy5g5cY1qnKdICY82gIGaeiBPLHm
# GBIyMDI2MDUyMjE0NTU0Mi4wOFowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIyMTQ1NTQyWjAvBgkqhkiG
# 9w0BCQQxIgQgkNKbing7rUyfN/17TUbuyAgKX/l/i2MwJ0wVny8/8pEwgbkGCyqG
...
# DQEBAQUABIICALBkRvaZ4tsvsgnQ7UfR/eUqXgtqRR3Y3e16dJt3X9XnIRTYiQ8c
# YI3UTOcCi2tRNJdpDg2nuFBI72U21i0D5NjQAA6h0pVLQVeltcHIq66qV19PuNZ8
# T7HY0f2m6nsgYPR2DlvjWE2bKuojUuq/GoGNSnd/kH6NA/sJNI+f0l7LvXgxsNWz
# 6tvTQohjhk1TsWn+WLIaAdoui0PgK516WdTtfBVv7HZ+O1O1XZWiYsubv8zi6W9G
# nanN1e07knB+59mUOUkX1QwanksVs7TBzFm4uaOLywpLHkdt9BcHgLR3xCHegnu7
# oGhd1YBQRxnyGWDiO+cePy9cap7/Qyu/3RlS1zDIocOoiIgi7Zy9lLo8dhxM+1Pv
# s7li92uyLhJukAPgSfA5WQr2TpHG4UFobeJfZnI9qfr+C9nHlLa3r+/dLb5MADOj
# wzBzDn5ExYIYCoyHYWvJ8kEw8qvJHmq+ScnlCn+USTPjQgPgUWpgRSNZ3XoE//j7
# /CDBlp3xqHzqTenSxYTj+ge3RbhINpmwqDTqPVpoVNAVcYMSxQuO5b3+wcf1dLJb
# BfGxacUGe7oyisWgl3zOKBLh0XKYBiw9dbdMpHFGrqfCPWyl7TTZRJSwi0eyUjg6
# 0QNEe6a3xq215o5XQJ5vhaI5vweUrmBcp8cidEOUKGOQ/5+Z1d4B6mAf
