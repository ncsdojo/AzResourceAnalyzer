# CIS 7.23 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.23"
        Title       = "NSG remote access ports from internet"
        Severity    = "Critical"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $remotePorts = @(3389, 22, 23, 5900, 5901)
        $bad = @($inet | Where-Object { Test-PortMatch $_.destPortRange $remotePorts })
        if ($bad.Count -gt 0) {
            $details = @($bad | ForEach-Object {
                $rule = $_
                $matchedPorts = @($remotePorts | Where-Object { Test-PortMatch $rule.destPortRange @($_) }) | ForEach-Object { if ($Script:PortLabels[$_]) { "$($Script:PortLabels[$_])/$_" } else { $_ } }
                "$($rule.nsgName): $($rule.ruleName) — $($matchedPorts -join ', ')"
            })
            return @(@{ AffectedResources=$details; Description="$($bad.Count) NSG rule(s) expose remote access protocols (RDP/SSH/Telnet/VNC) to the internet." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBo3MaUN+17a5Zz
# G5Qhvi4JbcKR3cXde/lOA4hisweEKKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEILtv/f8UF/8H6CaVdMoz8y4m2PI/uHXH7buDHygp
# fq91MA0GCSqGSIb3DQEBAQUABIIBgBMIoMhOTPQJtkDgZ/BPR++TmcTvyYLNEYk5
# ERMdqYT48EJt3QHL6Ocq8OT6a7HrJz1IRPxp7rSbCrrmACmG1I1HL0MSodij3WF9
# GpRhoPo9GEznM1NUFaD4kFQHo4yhQhcfM/z5QI4X/LQlpty2MQgOS//c6UI+D4+b
# lfGjc5f9UzTyafs378CvPrcrageXdNLn4DivHci8bfIJ9iV+4xtPFFtED+wrVO/j
# /qFB09XZcQSvVWB/Q9tgdaO3jggFKPVAJ3CXAcF5lhZ8De+Rb6NVlt25Mr5Qn4CD
# s1aWh7Xv59igh6sKvUcftGouQ10N+g2StcjiC2ghU8gKvKxiSjBhtwLmEkfeHg+E
# xrcfISPgUzLQfAQHPe3Xxv7awIeXhB+HG0MzbBa1b5/uPLCO8Ctq5D4RGJ36xZ5p
# wHEn/7lgrevMMn+mDrUyRlLjQRCXvVtFvahB1Fp7oT7oDE+MRbQHYoD1TOgTrXjb
# HKGLKCNAFu0miRplwEKh90DzoSEKBqGCGBMwghgPBgorBgEEAYI3AwMBMYIX/zCC
...
# AwQCAQUABCBV7zDon6wYGRJPpKBTfPePwE8/fGV3zby+W0EXqX/KlwIGagxE2Zrg
# GBIyMDI2MDUyMzAyMDgyMS42MVowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDIwODIxWjAvBgkqhkiG
# 9w0BCQQxIgQgyAAlHLeqhddO3WtQDuxSvW2XTttOm00S9ApP9aDncN0wgbkGCyqG
...
# SIb3DQEBAQUABIICAIjW0ouFEQ6QieNEaqpVBucAycnbSfDRSt0dnvymliyHb5d7
# QrRQjQgbTIcK6FotvrkvxCsckdEYmYeZQPULIjqczOEfWod7Qyt078xQE9UYlXYz
# h9gqiEzfZBAZK4AGWV1NxuNb2qdYo7XoDSSWGLCSPayGVdAciJONi9KA1XAC7Vm6
# BQZePGerWdEyK2VQ6GYjpaTh0jo87YKhjTziEei1SHih6iWeVBvpP2mTNGOMP4uM
# 1+zf5HTauzEO1UBTQ2TNXe4IbwAYN/bvMgWXilwRXhbd3hba3gV2/HEeLzFPIcJw
# H4+1mT7+PhpknGcSkKFM+oX2uw4Sq2bI5663bmWBS/k3l66Wo1BxUyzFazruRF7+
# A1B19mdYAU9qKzsNt2Wy6DfHeb8MZFM6RyX8wCTxYxqcvGztTDZ3nSp6xDkcdt02
# Fg0ZOpy8+YbF0+yiCv5iXPqvKw2t8T3EhyDWkPSy6pGKeAO8C0RZmYhpBccZaCDj
# 4LREMdAFKTajrcBzMCoaiBbLK47yasX1S9A76u2JPirVVollWzlGSBGyeRNsBQoa
# dfBEyPsuiB2Wl1ETvgBqN6v4u9+Bx96lbjUz+S7dJVIwDuvEmXJbrfMhJ9M7iF0h
# +oKsZGoF+FkD3HNgosNmhejQWOC9/cwnmTKTtOP8rR6ehNTIY+WxHkU4HQ4g
