# CIS 7.29 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.29"
        Title       = "NSG AzureCloud tag as inbound source"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $bad = @($nsgs | Where-Object {
            ...
            $_.sourceAddressPrefix -match '^AzureCloud'
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName) — source=$($_.sourceAddressPrefix)" }); Description="$($bad.Count) NSG rule(s) use AzureCloud service tag as source. This encompasses all Azure datacenter IPs — a very large attack surface." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBX1VC74R3ciaYR
# 0gKPG+UgQhrFBC8MMqJm+1DTpaKup6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEICySCVzvss4QYvj6b4YaQ8YqmD3HCV+qG81iDlU7
# VC3xMA0GCSqGSIb3DQEBAQUABIIBgCkmTvzjQ3QAqkYjnJKSaGHS0ITXNKwlALZN
# gfdSQU6csMhNimHCBgohkuDrDkBk5MobGhkZR0Fbn5oTOK+ERLwChfSEn8b55Q3K
# 0AuUTVtwJiiPFDhb0zNuajSfsj5j6OcsnMG57B/V2oe9hKQyyZEvkc4EBcIMLDng
# XUeAOa8Wpa7htWcoqOq/za2QnPX+0orAuuy1WPwdpTXnCvwD9KyQ96UlXvJXclGI
# ofKHes70Zkhya/sETfgK6dQeus4ZOHTP8tzA0NnevpwZzy3QKXksP6JQGVhKNbIJ
# fIW2s2eINiwvxTXlt6UQuXqPbulRApzzh/0OSPOw0fFYExRhVdnnLelDBd5ZoQho
# thgjsl/4lmbRDZKosQJQFrQS1U3K+NqNc9/7xmM+BRnE1DYZ3YXnHD04AsEV6hL7
# 9pcAOauMKJKhbjq3Yee88A69/EPNQZ989uj1VV3lNDqwN/1YssP87q4+kKyP0HJe
# wZYMw6TAEFumcpMwZyNyqt0XAIkopKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDJaFn2Hjz8yzMzHKpRCad67oJtIGrf8MmH1dEK1e6G/gIGagxE2Ztj
# GBMyMDI2MDUyMzAyMDkzOC4wNzlaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMDkzOFowLwYJKoZI
# hvcNAQkEMSIEICayvXwUBFqNgJs8xKNVp0RmD7GjCUm/6WjVPDrpO021MIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCvTCdBs4BFYwJjeVU218+1d5rUITHYgvxbprRqAgGdE3jL
# mhqdXD2hWcVHIvQ2BH+yFhvYebao69jQzHOXICx5Wir62s0RW6idrXHmWtS4KV7v
# g9ZqDUi7ivZhkeE5EZYG2GhJTNmnJg4EcqjD7w3Jnqpm2R9qS4ZbHuOqP1yAPg/2
# tD15aXjKUN3vmQEZC3uJJgaRb9A+4Uq1ZA5gQq/RjfedfDCk1D0HcopoVzAAn/LL
# eG0B3SU7WzyWWSbanrsA8c4BzEPR19qL5nmZMDU3V51FusTi3i/unx8jKbSgb4Lt
# DcPH5MCBI5h4spkSh7cOhxvB/bM+QPZClmxCqEBone/BNbD/+/i2I9iUjUQ3HEzx
# rsgLO246oGu1KWtDHIhFVgwZx9KOK3TFJs2N2s9GI/aOHkBvSr52hdM0YcJI3KxO
# 8PwkhVED22usDuPUL0BOFIONYDlngdLa3aEVdEmMawGBXvWdun9+2eP6EUdQW1Dr
# ms7rUemY1LUlS4h8YupOh0Xi9NkDoAA/a0dpN2B6XQJfyKSoO6JECcBFg0ZPsJlM
# JQUP//M28/TY7KIlZkKXs5ue9vVCEXO2yDiGRR4Xkv0FJcLMKJrEXxvHvUp3OPZk
# 2xqh6NgW0Gqa8XKdwOK+3NQ6yRmWFKpSMbLsXTiV3VhkyohPZukdDSqh6xUisg==
