# CIS 7.30 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.30"
        Title       = "NSG non-RFC1918 external source on inbound"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $bad = @($nsgs | Where-Object {
            ...
            $_.ruleDirection -eq 'Inbound' -and $_.ruleAccess -eq 'Allow' -and $_.ruleKind -eq 'Custom' -and
            $_.sourceAddressPrefix -match '^\d+\.\d+\.\d+\.\d+' -and
            $_.sourceAddressPrefix -notin $Script:InternetSources -and
            -not (Test-PrivateAddress $_.sourceAddressPrefix)
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName) — source=$($_.sourceAddressPrefix) (external IP)" }); Description="$($bad.Count) NSG rule(s) allow inbound from specific external (non-RFC1918) IP addresses. Verify these are expected and documented." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDtEC15dEErfLw2
# vyakvLCVmiQJlgVLoJ6mgdeo+k82/6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIPtDc4rNwrCSwK/Mru53wZTtmTVKkhOmS0G4UtjV
# RDBAMA0GCSqGSIb3DQEBAQUABIIBgGO7m2zrcAxewiKfoMPs4oGJ/f+YR1k0Xepu
# 6/sTKSDOyeRbMI7DVFwHLtmBUknUBIxBBkkZeL9lsWYPFdXd9o4thjB31gccKo8y
# QXtRhK9G7hbMEfYFviD8iGQdC1bce1KE5KJe1BtO2rlf5g/VlQI39OJpXzB1YHAH
# fLSTb6HXyI9FLpCGgKA5ykSXOEykwLEtURhZz2pvIrat/n/sFUx4W287xOFaQcOy
# AdkQ4yOZohexUlTxlP+1mf/VlUMB0G1LiyeNB5V7cT6lA720MNaJUJ++h4LqNPf2
# YENPcsZQNqIrK7Zq9T3znCrQ6Sam4CmMIrDQYVBDrM2uwW9yM57KQ7c9K17/3m7i
# YXqcib5daRBvec97DNm2RfQg8A+57AlaaFwx6Ar9JlOa8Q+yJg9cBS5rRB0LbxVu
# Q2O+mvdl8es5FTfdi3SOVp02HXkTl/1l6uotS/o7PSLI/KD0d80NTRsWexzEiG4v
# 5HLJfyCQ7OYs/l+nRJtHlsIIvZ4/4qGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDnii0tjCTKw5md7PW9S2KADKZt5aujl3OyFdrwoG3BdAIGagxE2Zvp
# GBMyMDI2MDUyMzAyMTAwNS43OTNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTAwNVowLwYJKoZI
# hvcNAQkEMSIEIADdPj4HaAWJiKJDHwl/la8jKCos+coww2KYmNyfnuLBMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgA2yrtqJbqRESGrXSh1qxbF4vlK1n7yJEDWCmsjfr/i79QS
# ywGDtHsjBLke+uJoeZi74JGz4ca1ytnFstI5gwk7wbkx/RqKgN1zErZvK2YIN5ZM
# g8lvlI87/yG1isv91tV00LjiTHBWYh26TdHaqnjLVejnuA3x0dDt9OIRwbkHWIUS
# rPyAmbKCSxCIrdV8+VEyGqVWfiKdPaY2o++Ov7r1NVgnFQGhiPg2Apa5AhGKnYXT
# VQfIhXgcomA2np7GUvBd9EbYnjwDyZLU5hjdY8wCcoGKV4KS9kkBTTlBuBuSt2+o
# B17ejQaCfgH7iH9Ombon4UBvMeXogYkdbteMrHS/KjkTUAXgnfT8AP7vX4npQ6W5
# YPatB/xkb2w8LXk0TXH1xPUNIcXUQDsYuuv2iyl9tKwjJJFt5cj6pEWNrJ63+CMd
# 0GPcubr4jevp9TdEnPT1NoSuajs0bPbWvFQUQicQ4Af3rBvgZ7Zs0E96L+iVgcJt
# peNQjRm4li4MzbHw9Kt4ecALtfIPqafMBoL3pyNuYAIp3zFjEOvIynwhc/NuwhgK
# MEnP5TjXgyaHXoUx80IUiOemnW32uzC//2BFQuvMo/yPQWoBuwRiFXFVIH3YZtuq
# zc8Qw8d9v13J0M/s8XnD8F/eqJxocahLrHLGFZgsYpMJEA2h9Md75QdLGsFRWg==
