# CIS 7.34 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.34"
        Title       = "NSG allow rules with lower priority than deny"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        if ($null -eq $nsgs -or @($nsgs).Count -eq 0) { return @() }
        $nsgNames = @($nsgs | Where-Object { $_.ruleKind -eq 'Custom' } | ForEach-Object { $_.nsgName } | Select-Object -Unique)
        $misordered = @()
        foreach ($name in $nsgNames) {
            $rules = @($nsgs | Where-Object { $_.nsgName -eq $name -and $_.ruleKind -eq 'Custom' } | Sort-Object { [int]$_.rulePriority })
            $lastDenyPriority = @{}
            foreach ($r in $rules) {
                $dir = $r.ruleDirection
                if ($r.ruleAccess -eq 'Deny') {
                    if (-not $lastDenyPriority.ContainsKey($dir) -or [int]$r.rulePriority -lt $lastDenyPriority[$dir]) {
                        $lastDenyPriority[$dir] = [int]$r.rulePriority
                    }
                }
            }
            foreach ($r in $rules) {
                if ($r.ruleAccess -eq 'Allow' -and $lastDenyPriority.ContainsKey($r.ruleDirection)) {
                    if ([int]$r.rulePriority -gt $lastDenyPriority[$r.ruleDirection]) {
                        $misordered += "${name}: $($r.ruleName) (allow@$($r.rulePriority) after deny@$($lastDenyPriority[$r.ruleDirection]))"
                    }
                }
            }
        }
        if ($misordered.Count -gt 0) {
            return @(@{ AffectedResources=$misordered; Description="$($misordered.Count) NSG allow rule(s) have higher priority numbers (lower precedence) than deny rules in the same direction. These allow rules will never be evaluated — they are shadowed by the deny." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA8WbHWZdk3vLbt
# /dipbDt2xMftYnEGIH9sJLDkg8PwbKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBVp7PECBixxE3Igit6nn+0H4bxxXOTTUdJqPDKH
# GfyxMA0GCSqGSIb3DQEBAQUABIIBgFGqosplIQVpJgc0QqaFmpLc6sONjWcQT0ge
# LqmxWn+i8mSKszqXVBEmWDFV9aWDA7+smZkOL/guAwKKlty7cCLxmkXhWvzH8xl+
# /MAfmlxtTP/AZX+NOxZKl4ZP4w/W+VgDWXH+XMN+DqAHy7faSRTu+MQrPP8bBZ4P
# F83GbZp6abinePy5d/c6gJ26uMFp8hvci9p3sDVRfk1UJscizlo4bnOZcC+25YdL
# EKbbKhf/YVcLxmDM4hpWRC50n8iUYIzQyWUrvJn/PDc7xjCobNqpMYhOo+f6FUaq
# 1Ea31IjdG/IaPPYCt70EySvvn8srXOFLmrt878I9xNu8m23TY7/CATBC57Iv7GAe
# c1PZwHeABd1gJ6zBCvB5XJBVeDKtj4GmhgUMBOyojF+9/uDXM5O7s5iEDOiVJ0zY
# 13QWoYWusIn4dUQacYz5j/ft5eHKmvfHXMUV0mTjeCjc9vKTYINioXiKDjii7vRd
# Qrpz5QYAjjXpPqTpqr2SDGghxDrZaqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCBB8BBUxJmqwaJT/u9KBXgMApkfxWiu30Wxka71e4RGogIGagxE2Zx1
# GBMyMDI2MDUyMzAyMTA1NS41MDZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTA1NVowLwYJKoZI
# hvcNAQkEMSIEICyc17Kt6EdAxoieHLAJ8WJyYbxuzQRqR2s1DwUlqr3YMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBDk1vsi7w/14xI/SBM2J6+qywMz5GHXkR5FNXFnUuHyp4S
# +pCU2MAqQE5Wk2BMxgE0SGvO6tyTab35pqXKZGoXKm8DGdh0as6KFxCD4HZ3BXHC
# 1DYXej7OdXdVLUgrhN1DM0lYTnKOH2S67vJkZ50MxCDXq3aKc9QE4V+QtWod8eX5
# +PeArK/mUT0+u2ioLn1HYjXBI7Il0PU+66NgkR8irgNkaBkhKLaiKc6+D/tO5PmB
# Mr569FPgFk8L+RYp+WSEudGCLmjc/77JTOobVrhdz2FVZFm0tBG29mkWt1OmMkJZ
# jg5E+NmTI5CoJ1rcTr6QL28gHkU8W6CGtNs44IqwJlg79pQ23axBw3KBVjhCNcvH
# 295g5Mifp9XufdtbZ1KZ74XtNYZUXoXyX9OIfzooT3r9R6xWbGmP17m434BJlbdw
# 9tquWJHeZ0hVzLHihupwS3Bj/evUtThQERFOLsNQJQJvtSGyda0ERd3Kq060ojaf
# rrV1X49osFW+VWYSnKN2kk7vu5CrRrJAjnCdvRha/cAlxmBgPCXpY9LE7ilc+C04
# svWf0AwJfN9e8e2w5jdMTjcdl47tDU+fADwlP69dOF5k5t5Id1NEI6x/RMfp+yuv
# IX1iSrySj6kPHj2OPEp/mX760PskL8t4UBWlI+sbPKQIawBJbGACkrzmYrMH5Q==
