# CIS 7.8 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.8"
        Title       = "VNet flow log retention >= 90 days"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $flowLogs = $D.FlowLogs
        if (-not $flowLogs) { return @() }
        $vnetFlows = @($flowLogs | Where-Object { $_.targetResourceId -match 'virtualNetworks' })
        $bad = @($vnetFlows | Where-Object { -not $_.retentionEnabled -or $_.retentionDays -lt 90 })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.flowLogName) — $($_.retentionDays)d" } | Select-Object -First 500); Description="$($bad.Count) VNet flow log(s) with retention below 90 days." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDWH7krtqpp/tAM
# rId1bq8NAizP22ykxgb1fxvq2kigqKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEINcZMdwqJJ21Kta691NHwSnly52NbRF8JbEi+lUC
# Ykl7MA0GCSqGSIb3DQEBAQUABIIBgJolTqK+oyh1a2YhXlNwZZeOsL2l/A75F5P2
# ZaHT+H+9f4pWI0pTwmzL+ZJWgA85fce7/nBcgLt4rr/pzL5JMFfVJynnY3W+xku7
# sT+D/fhaqlt0GJfQgU7V7dteolYTCOemlZI93F9MS0XaTli/sKvjGH4En+/HCl4b
# SWwayRNCsHqmEZdxMfNgvyhCdClj6vJU7Qhi6yBM/h/flZOlhrQCg5xrcUt7D+qP
# pxwkig0vbtDvYsvSse5Y5tTHzOx7Go266FJZKE5rax3Ga2etpvFnVfzQ/uWn0gS3
# oWGl0Z0gLopCiyO8AdOWpat+jFiET0QAAXwtWJEuoNJPpyUUVTiDGdIfTG0Q9v5j
# 8aAT6UXWa20xjCXrfGiNN4QD88UtXqO8CZ+Kk5KKIjPEbDpTt/1DOKusFyfoz4Ah
# +eHmKdFANX1TgiydZT234mMaVld1RjRQAw3RmB7fhRNaZf4RXbfcQwZGAU/U1C6T
# /NXwezJXNR6TFFQ6Z90+oP+hL0TS6qGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDyf5txNqQem0MNPNCCzjBoURAXCOfWZ+eN9gu+ORHpmwIGagxE2Z0K
# GBMyMDI2MDUyMzAyMTIxOC44NjJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTIxOFowLwYJKoZI
# hvcNAQkEMSIEIBW04dL2XtO8BH6h3OZTBMqr+fQEOWeD/vf48ILiceMDMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCf/G5KzTQtzoRYKlHlptWOD3cXvHy2IedFuUev+6I4YrFz
# mALH42GhIBpyErD3/QWFjp9GrpgtIuIV8rMDz4hmPP/YlpJ9Avl+BWCLcpDAS97i
# Qef7F3lI6viFIABJdXvv/qsyyPt5b2CmUpwqjBIxvgisQuMxFuuCjWxjkKnfr+fN
# 5WshQ77EbiXM0klBfvmUEBgJ2tPfuQvl1+MbtM+JkwSH+bhB7sClbINEaK9d9/fl
# oBKDNQSQlQCy/x6YmAWue58Bnw48oaKLC/dtnv7fG5zI4I0xNhj/NnTFIPsLjFiU
# J9GvDPBRngjJZCNVAxI5Nt7qoQWPFui8uqf1jU01gyxCgiRPNujy4y97byQHmj0z
# Gz/EYqlexekpZsp+Dsw6HLi8L6/rCCNcuTSgCu1RvP3BrugELh1ZrmHMTMgg5v1i
# o5B8zmloYgjcvlj+rNc08hmvFjV++boVOgAXOwpE24+eButEg6wqmZndt7FR8no8
# 5QIuZohaSWF3UUuRyh4K2ywMfWleCxcAajXu8X+Alqn8Rn3eriPijlgiLtdGUjtR
# dm9mFRwNo7wgVFLw9oG++nJ2O+WFovw3GniuWxgmqa/aNpP+OH9ghPlAUQGi/C3n
# VRCyMqZ9aGThHC78NFy/Yytn25DooxANA37cjgQHtQb4kupSImM6RMjKs8b94w==
