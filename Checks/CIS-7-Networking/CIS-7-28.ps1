# CIS 7.28 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.28"
        Title       = "NSG wildcard protocol on inbound allow"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $bad = @($nsgs | Where-Object {
            ...
            $_.PSObject.Properties['ruleProtocol'] -and $_.ruleProtocol -eq '*'
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { "$($_.nsgName): $($_.ruleName) — protocol=* (TCP+UDP+ICMP+all)" }); Description="$($bad.Count) NSG rule(s) use wildcard protocol (*). Specify TCP or UDP explicitly — wildcard includes ICMP and all other protocols." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCkfsvA31+Wzkjo
# HhnY7sC2TlEpEhcHcbWJZmau3coJAKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIIluvUPxG0288iH7g7q3ldLj4TSq4kadMSGD8bFz
# HwM+MA0GCSqGSIb3DQEBAQUABIIBgH5qBJMw/hgJ29H8pUfLQKPbbF9SSeOGgfGq
# w6y8HrDa6MuAqWq32RYwIb/RvQ+/wbU3/BZCjyMf3P+/AUcQ2GQrM4OyZyivONO1
# ZcqOVHhtj0V54frStNZRJY7M/BobaUvJAlDjn/brh7cNHu+lPFtTue8OdtYNTsaJ
# ypiH6sizLRvFYUqGo0/HrMIwR0J+5/0zAtXwlP2vZ7GWg+WNVx3LgEPWB/ZRjAuN
# d/UdEWcEcWAvabNIB/I22Uz/juxzC3mwaH+uK6c2BUcHSKmwA/MvnJbfGijWzr1h
# ZrjpHYN/7gBqPgSIxqhLPIX4x0QGA5e3OJAiZio0VCm6QHwzIrkVSddXcc3W7bpg
# gjri/3SgJvkY5DF7VxvqCtOv8q9TPxNwa7bhVvKKeZ6dI+ygFgImA6s5y21dZAtn
# WshA9WRwSXx3GfxHc0huCJOONtX1L3avBpypdK4+/BUv8wW9exPy2PqutlOq3PtJ
# e3EFWqehTNgKfhhe+y/GaJEemUIz+6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDjgo6Yc88kDaZdUeCYPA8pGnH5WKKONajZgHt5WxTOZwIGagxE11s8
# GBMyMDI2MDUyMjE0NTMyNy4yNjRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NTMyN1owLwYJKoZI
# hvcNAQkEMSIEIIbKkz2A4crHV+IkSdouUj1CHMn3HdrI4pm2PY4xDLtGMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBiRlSDL7zXCqE1jja+PpOrM63KBcPXAE7mqO3AT3syRmgn
# w2CjqF9m5EtvUG83GoG5qsSVW2A5BkToeK1wx1AAedq4N/qMg4oBxsi7l4iU1wNM
# nnRz8nZjCFshG64+xafV+Pa7WtdwdCe2YsuS5SC+zYPzPxPnWb9Ser7nHkREJSct
# Nnxk/IMqSInZvZ4JJzDHJUQVaa/eup8KxkwemODF6T1JKkJnMEp0CN0Zd1LH6PBr
# Z4u4jYG7vVL/buCZ/Byxja3j2wZH2hJS0vWOjPatW1E71vhg327B0mZh+zRzhPht
# 9JJ7ridDIoXi9TPSHSwIoeqyGnCBb9JML78i7J8Xac8NR25NiU+3E1ou8bgoZS81
# 6bK69tEjPJcR5KdFhRYxHaKjWZg1bp50hKgVjsXbb/6e7Xrdkvmdny8IRLxHDzHG
# 1WR0QJf2xYdT+cuUGiXU+8IpnnIMWtydIC87XJMcY0uLp76EY0yDbV71eHs6+Gc0
# p9ZFksVfMfx5K3+7Itsv5TqXVFQk8e8zsBp/p9mrftdZCrv8oc4a9TO86ujjY8DZ
# ybXZvKMQwS0e+d7/IPaQlyNxOGyvUqwU1fnJRzenKiI/EApfqRgHhJbZ6qjDGUU/
# /RlgknrKOxq2us90CWbFxd5ReEsLFmIdkn4J/B2pud07caMwzAEqWeYDcz5iMg==
