# CIS 7.21 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.21"
        Title       = "Load Balancer SKU is not Basic"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $lbs = $D.LoadBalancers
        if ($null -eq $lbs -or @($lbs).Count -eq 0) { return @() }
        $basic = @($lbs | Where-Object { $_.PSObject.Properties['skuName'] -and $_.skuName -eq 'Basic' })
        if ($basic.Count -gt 0) {
            return @(@{ AffectedResources=@($basic | ForEach-Object { if ($_.PSObject.Properties['lbName']) { $_.lbName } else { $_.name } } | Select-Object -First 100); Description="$($basic.Count) Load Balancer(s) using Basic SKU. Basic LBs lack zone redundancy and SLA." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDEAoChTVPV1o7Q
# n02Rp3tvEhSrUO6DjlDnABMq586HEKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIPf/fAZthsAtjz3CvX0uDPLuKKib8/jNTiE3N8qa
# uvxyMA0GCSqGSIb3DQEBAQUABIIBgBoR+azrz+YXGkpWi5y7BzH3ZLc1HclzB8Xt
# Y099HUk2GPdbIOzmtBzU20YxBR7rflJFZKdbimWz91sEEPq0gV5HJrs2rZDdUhEa
# 9Vp3sS3f4GucrHz5Jkz2AettDDHBQqV1U0MUkA8uNocefVAlKRs4rEvHsO3iizfy
# G9Z2KJAofd5nCja5krvXdA7FkLgaL0h8GG2WvXvFC1s8OcYOyb8UlccpDipK1iPZ
# e0NSPYmbyDQ5OmmO1zsLCoDndkE4UwCJ2uUeP78ZbnLh+IAhJFkrt8kEEB6z3GUv
# pqUV2lmPZxlHUOomZoA4t63/G2QKdtj44Fprin7ulO3jxHkK14WWzXaUt16iYNrT
# midqA+lOi2Kv4MHpUXVrZ/dnwxY9iRYyQUgKRQSrju9gq2uO1XTRyHEf0MFImPoU
# cbc5J/0RVmwbNhi7a76S8uzjaDbhBQHbdSVGrNlJ9mH/US9pJLkMiessadV4TRts
# /6p+mRekdd8JJRW8ukbOD1qwnL6D/6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDXwfjO6IMWeEHNu+8hr4ocTdyTT8uyhaJ2L+fMtP1pSwIGagxE11li
# GBMyMDI2MDUyMjE0NTE0Ny43NDJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NTE0N1owLwYJKoZI
# hvcNAQkEMSIEIJDNxdhhRBk93fXzWyHmYx2QguT5w2cWAbxV66y/FGnhMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBPCgIpjUYmDaYawtW0eUdHnnh8eIivg9xIPP4y/lmyXNA+
# qKM2S4bN4eEIHeSKSi/rHXkzvVZwhR0GVaLpQPBxcxMu9cHB53Ial8oiQlAB9Iy+
# zp+vA/o3NFbCR4hAg9tSwrSyqNzHzzXn4jMLueGFbVpous1JxwbVxr22zzxu/Wnn
# C5ZXN7DJ8F5v0VdmOB1NV4NKJN3N5y3KLsG0vBjWOrI5io2neCVcebrSLxeZUb5j
# WVnnX5rjoPVsNxAOCi2R6OtH/Hzcqbuk03T19ALeSlSlK8Dti/lXfopjNfSZZqbG
# DszPq0TMOnxvcGfE1WcTCsejAHmn78+gH49ceEfpUpilAaspbSIV1Qr1zmClRsoU
# yKeBGjcA/9Twm3F6ISFKfhkndL9RjDYohh3JTcJWLIfOLJ12Rmsf2rArEtF5MWu8
# XvDhi0NAbyicg1x0jkuthL7Dus5Xi5jZpJh9aqfch9FGkxS+0Utq8u8wTAkYYR0/
# 2VDNhYImM/76INGwZomohXRItmwOXT5AAY6rUPQGh9R7yKys6m6JNuNwFofghE8E
# stkRS1/VdThYrtvxO2LJ7F0w6sxBewHocR4H+MciObxUyskGDi5RZXRChZNrRXBH
# +DDFD3JA8Fj04tNNxLQM8gyq1Mqt3HNyvCeKMVrDDQTI/grzWozI3soaqXDTzQ==
