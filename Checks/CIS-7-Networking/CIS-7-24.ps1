# CIS 7.24 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.24"
        Title       = "NSG database ports from internet"
        Severity    = "Critical"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $dbPorts = @(1433, 3306, 5432, 27017, 6379, 11211, 9042, 1521, 5984)
        $bad = @($inet | Where-Object { Test-PortMatch $_.destPortRange $dbPorts })
        if ($bad.Count -gt 0) {
            $details = @($bad | ForEach-Object {
                $rule = $_
                $matched = @($dbPorts | Where-Object { Test-PortMatch $rule.destPortRange @($_) }) | ForEach-Object { if ($Script:PortLabels[$_]) { "$($Script:PortLabels[$_])/$_" } else { $_ } }
                "$($rule.nsgName): $($rule.ruleName) — $($matched -join ', ')"
            })
            return @(@{ AffectedResources=$details; Description="$($bad.Count) NSG rule(s) expose database ports to the internet. Databases should never be internet-facing." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBxom/XmN1lR2kH
# J3sfBzRNr6fCpYBBChzNwavEQfSLB6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIPb9PFFeM+fv27IESRTg8Yo7uZwBL+NdtvmdDwT/
# hETaMA0GCSqGSIb3DQEBAQUABIIBgJ93vl0wLOr7qAmpZFp1ESXGQTeZNh9vZz1G
# Fz5cJPsUira2B5iJxY9lgmlFW2qvPeO0RmLn9X3XPD1DDym2UfA7QvFGH+gCgDqK
# Nps4Ip4+x+nyj5zwUk+m+SyszEQ0rRztcJwoGFcbueF+HcmceyFOSN832zGhkABN
# kSfwe7zpEMWSQkzQUed4am76k0KgaBfUZcZC6VLSO9Cd11tQrSsk06TzPeIaCGXm
# tf2P0hOoime3QcPmx22BG0Ck2vr4H7c14tOpHwA1LRQycthU3h0K0l8mp7IlZLjP
# LTQtC34CYcY4bna1WbFVdSGDfwxE4xptG9xE5XZSh+qMrZqHVXHixm5MOeVY9fVY
# 4Vg+rZO3GH2PthtSjPGNrObr2rG6rqTJk9w+F29rF1WFf7bpBBlXLOAqHjWKg3/f
# s1O08RSE3rDUUQfe7/mPu5d1pypgBuxJqyriJzi6gD4Bs6ZVzX5sGzo9Fqkzb97J
# Jy/5Q/hsRRUJZsxe0izDfbXycDCe9aGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCCEn2odGjmG0w0aBm8sBnTuRnJoQTNeDej+vZm9FUQNvgIGaeiBPK4H
# GBIyMDI2MDUyMjE0NTIzMC4wN1owBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIyMTQ1MjMwWjAvBgkqhkiG
# 9w0BCQQxIgQgJ8uwRwegXaMGvBR8CXuHMz0/hYKhtGL1arWhB0wLbXwwgbkGCyqG
...
# DQEBAQUABIICADkk5L0joVBO4jcrJ8MtZ2XkT7fS5Dg1wDAF2xlgqcq45x7wM51z
# Qe0V/b/D4Lqv8kQ+VJo6EUOhUsouXByrYYcOfVcgb+cnRQLT1+wipnXUUcJbs6eq
# LsmezeiSsOgyzV7DFVdG65zGNEFXqInbOmFbtEqn4ItUOT9pJol1VS67wyyMOhWa
# u5rJjl9JhxFDV3/KZVOz29SBBLgJooSr31lGdGSQ73RGI/evZ7StCHGVMlJ41+XQ
# a1cnJOAMBRAUJfgH1m/PAgYx9DmvS1vRMktrVrImm4cu+y6awF3/ffVJ4Q++OvPv
# ksjBtUcTfg6sHsFCU3GB9scS4pFMy/BRJiNxlaXniojQBhtS3y2iu2i8WTwZVnfE
# hz0SIa51vEJYEHCuPf8FO3q3oruQ2uWHnC0bZ3P9sOADSpaU9/hws6/4FEMJXqDK
# w3y6w9jO/ZXW3vSW9DP3m0/lp2xQoaPrXEYg6qeOzhGu8WFCYSRZyI/aVhqjfiFy
# 3Ft5piKxMcDbzOWz+XSPKyB4G/IT7bBKfryksv78cbt5caDl5s7dJR/WhJ7zdjA2
# W0RLxL5Kn2OoVZmhd26ovm6sst6wlPfFI71CrMMBNc+QpiJmIHSNKji7zBL5jHl+
# MvixVPxQ0IfQUcjOa+OghmGvE2cjkHBF8QY6rPXHqyXnMvkibXomyogq
