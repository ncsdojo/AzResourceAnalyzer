# CIS 7.26 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.26"
        Title       = "NSG cleartext protocols from internet"
        Severity    = "High"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $clearPorts = @(21, 69, 80, 110, 143, 161, 162, 389)
        $bad = @($inet | Where-Object { Test-PortMatch $_.destPortRange $clearPorts })
        if ($bad.Count -gt 0) {
            $details = @($bad | ForEach-Object {
                $rule = $_
                $matched = @($clearPorts | Where-Object { Test-PortMatch $rule.destPortRange @($_) }) | ForEach-Object { if ($Script:PortLabels[$_]) { "$($Script:PortLabels[$_])/$_" } else { $_ } }
                "$($rule.nsgName): $($rule.ruleName) — $($matched -join ', ')"
            })
            return @(@{ AffectedResources=$details; Description="$($bad.Count) NSG rule(s) expose cleartext protocols (FTP/TFTP/HTTP/POP3/IMAP/SNMP/LDAP) to the internet. All transmit credentials or data unencrypted." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCHq1LAJx/mzg1n
# EU3qikIcSGxE6XOVjXIAs7NgTaa0iaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIF3ReivvlYx6t7ie8hdDaJNJa4V+nS6o39dRo1bL
# lzviMA0GCSqGSIb3DQEBAQUABIIBgJtPVm+XQRrnECwE4AgPDpUPtxSFU2X4DeEI
# L6gjW+gNstLRZHsUDc1loASoe7g0k3zaK8aZqJ8plKaeoHYBycrSUJL51q30mGVS
# VwSZ3MjjOwonzxLsxacg2JnHlX/ybykZ71Y/E3NSVSJUDIPyy7VirkJsnKUVQq2b
# rzuBl0tN2eq/Q12DhBXnCalQFXpwxzNXnPn9dQuP65fRBDpy8F0AnuVTn6t+gazX
# gUh3jdGZLFijqf2qs8nFNH6ThC/2EIY1xNM4ktTECAfJ6+XJMSB55l1z0edKqIPX
# B4ypdPRqLV0c6iorCrAN9LWaqEmCejNsqiurl0gxS2mYg6Shqs3gdSTVxydD5EVh
# UyPngYk8DehXRsiI+UlGw0wvJkoPdbw31uh86rFRuN1frOWineUhQF5z1jEbEfjg
# bClYlXxJe2g9bDU0030w0qISay2pi/YLnMbMwwUi95QD1fX07NeUTuO9RHIzNhxd
# G0ECQK8unAqY+T6DrgCXGtEw3OmMM6GCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCBriex4E8t9O8KrJp/ZvYi5h3WVMbrJW2Y20RDCn/byLAIGaeiBPK8C
# GBMyMDI2MDUyMjE0NTI1OC41NzRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE0NTI1OFowLwYJKoZI
# hvcNAQkEMSIEIKHKZ5vJrcPMEgPbxfUlc3AEyLYkkzCmUTr+tOUgVAIpMIG5Bgsq
...
# 9w0BAQEFAASCAgBZFXY/cl4qGB/OGNGf3Mg5zAqg7f+lLZRaOyVg0K6ZCc4ryEma
# /sOErCwm1qelIad0PUIvwjqB4q7cOu0m4VYzaGuafn2XWtPvisJ/wcp2EzIFFMhJ
# 5blBgoPotFIf6WFIQ+7/tCjia0RiYO6v8N3A3iF2YZxNSD6CpEYar1qsN+q0ehO6
# ibFiYpUFTZkr8eY8sHWKMBo35u6KeLjg45yAQQpdfW+m1uDrR3EwuLjDtIEsYj8L
# ptreUlanUf4kjQsrJP26nxaXzKqRFanG2SAuCCF7nhSXJDbn8xIppTFDrrM/Kgps
# rdJvhVWexcOBRGXqqN3EV993lssXCv27U5+OPSq3ayKW9VGqWwYWo5deJGilUDvW
# 9lXsnntVOxIIKb55HzdXOEz29MpDtAwgRofac/9PPtLurBxI7cvBWivtmx2df5rZ
# C6BEOWxEyxJe/2skiO0GTvt9OON4+RdjbR0zRd2RIsuVfoQEAeqgyyD3L2uxhAKr
# LbEAzQSh6z5GWuy4hC2GSqUqah65McQ2KRjR3A0uSVx+biWOUzxbOk2D7v/Szltv
# zFumnvsnh9YPvOqoOodjsF5yVd6p4VFgv+ZEshuNtGIzGv4sMxlo/2qhHejXOqJl
# +dbe6IYG3ErpWXtAZP+MzICX5Jq7CABGqOVbzQlLstT6w/5tIICXJ14H5Q==
