# CIS 7.32 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "7.32"
        Title       = "NSG outbound risky ports"
        Severity    = "Medium"
        Automated   = $true
        Check       = {
            param($D)
    $nsgs = $D.NSGs
        ...
        $riskyOutbound = @(25, 53, 6660, 6661, 6662, 6663, 6664, 6665, 6666, 6667, 6668, 6669, 4444)
        $bad = @($nsgs | Where-Object {
            $_.PSObject.Properties['ruleDirection'] -and
            $_.ruleDirection -eq 'Outbound' -and $_.ruleAccess -eq 'Allow' -and $_.ruleKind -eq 'Custom' -and
            ($_.PSObject.Properties['destAddressPrefix'] -and $_.destAddressPrefix -in $Script:InternetSources) -and
            $_.destPortRange -ne '*' -and (Test-PortMatch $_.destPortRange $riskyOutbound)
        })
        if ($bad.Count -gt 0) {
            $details = @($bad | ForEach-Object {
                $rule = $_
                $matched = @($riskyOutbound | Where-Object { Test-PortMatch $rule.destPortRange @($_) }) | ForEach-Object { if ($Script:PortLabels[$_]) { "$($Script:PortLabels[$_])/$_" } else { $_ } }
                "$($rule.nsgName): $($rule.ruleName) — outbound $($matched -join ', ')"
            })
            return @(@{ AffectedResources=$details; Description="$($bad.Count) NSG rule(s) allow outbound to risky ports (SMTP/DNS/IRC/C2) on the internet. SMTP enables spam relay, DNS enables tunneling, IRC is a classic C2 channel." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBxeZaJw87yJjhe
# W+gXWtAR2cqIdfIP3ldrdQGxLECf9qCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEID4TVvkkvHo57388O4wEKHA7jvCu53vg/mAifQd1
# 0MYoMA0GCSqGSIb3DQEBAQUABIIBgJB1WeNOwfuTWyA2IY1PtOb81et8iOBrUFxP
# 4jd0+cXGi29fBY4M9KKg6Ydsm00GbDGfzBc3HQp3xju5s8L1aJAoBjzRfmnUPiPr
# 1IGPa2gHsDBgaJULFBmqjE7mw8I4m5g4r+BlwhyMc5+vjWlQ3uh2CNtbZDqB2Ui0
# RkednUJSkE7ngCd6TeNwtDZvgca0ZAkpfQ82ilhuJa7zVrNGiYYnOK+RVHeJkPUH
# kay9+w94Yr61zpOFWeR15CRy3D6E+D7wNBTxBUc6GjUlliHGvscqd4x3JICIiFcA
# K+JHIwCX1BrUKzBbILIBiubaLPtQZSEr9/F37wd9oxbO2IO2aW9O8QJRyiNE8q/6
# v87Lwd5zrudKOD7a3xfP9ejznVYdUk1i1zvv5Xdiyzyr5ESNHLbANHv3C2sTiFFI
# GiuBpFn/0nInKs+tJpfI8LlaqEipiSNlZAeXPu27uTvBoTFndHl/qF82sEm5He7x
# VoIavEtfxF1tZLALVNbm2moEgvA8V6GCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCDYZxgnYp9BTjS6Q7g5EfRzV5FjdniOQjiiWI0n3+kgAIGagxE2Zwg
# GBMyMDI2MDUyMzAyMTAzMC41MjRaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyMTAzMFowLwYJKoZI
# hvcNAQkEMSIEIDay35X3ix0LhgPvLhkKtAEpmpAi7yHsk6n7OYOinmRbMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCqMKKlQZqqSFctgVT3JXVIov2eX+72iYPGZWub+895yp0J
# y+NHT3Ae6T9t8wdZYPLBtCbJNPHTPFDwaCg2R2I1u/WKII83UaXOU0jyquX/h56u
# evrsMLG7UA786+5cBBB1i3PD3yrRjO0JAWxL+zyVdjy8GVsad6HBeXwfHQsM1V/y
# 4cvths0V++At7VeWexXG9trQyZZ90ZwpppXcu0/oO3sdYOywWADSAa33TI6WxoJZ
# 1PAJdO2B98xplc8oK4fqSyQDSvxn2i8qUL2UegO1gH4yGYnc/iIAjuIEoOoyVA2m
# lvfkrh/vuPqwGQSuv4dJJnSqwSPq/Vt+3iSTsEsNIV9w0IHnyQnWUAirU8HnWkp9
# UKbbjX4D5VBvzijWDvyN4TEq1gj8cXOmJ/yL5sojQPQKdzYLgyrHW+vevvVLRkGO
# 49tt0uy59ob9M73Hqoo0SCim7ubLw1btpiiTv1tfgL3szJCvrGYuw/808tg8t6ty
# O4f+po0YDzJjGNUWoMYT7L6MjtE3NMDVmigGy5yD00+Z6m1pkNrSjhlJW0H2cazH
# 4CH7PDXt4VffwSD+3Apm3gNSZ+Wb50qxCSRYYpKynPQdFCASucC/zelhgGCEwRR6
# Lw/naGZsUffN66XqZorWH+SdkTTQ3ddT16CT/ShIV10CKUWVURSkiki1mlVmqQ==
