# CIS 9.3.13 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.13"
        Title       = "Infrastructure encryption enabled for storage"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object {
            $_.PSObject.Properties['infrastructureEncryption'] -and -not $_.infrastructureEncryption
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) without infrastructure (double) encryption." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDALfmeF3L49hYN
# in1b+WNH1q8zjwE2T+wNxDXmafy6dKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIHQ7QKi1Tp6OJWR7hJV5ukYcaNiGs0JInM7IHmF1
# Tcz7MA0GCSqGSIb3DQEBAQUABIIBgBnzO5YJB0fsQxeTa5eHcUMd78w4VRSVarSz
# tMNw96C3t/ZGgRA4xw9UsgMkwOJrEeoHn0KYE2ZQ+oFd8SvnG2XI9LJljnIBDc++
# L2lqX2V5Wrvnlxcs+Y2wlHWvLon1rOe8TE0G1E/8pMhAaqMRp2uF76haX290VQPg
# b00rB4RiCdG9oJNSulkeO0w4jhtx0KbXNpik55hmgaDYeoccljupvv/Pg8Zbu4Sl
# Av+deJO47x/vXW+jyP4hL7tzh6w7Axa9OBugilXdc7hgC2wD+qecrZH15evJOKte
# zKuLSVHX1cI12BVWNqtFQne+/ViBa0poCKqnSVCz891b4ObSNwpQuxnQyOxmlMdX
# 7B9aKyb/+uI7k1zpdDZZjYJE5FX9bMQREkOAeRHIKLdKuXlLgzkO92a6EEt9fgjD
# zBTJp9Copn6vypTyOvS0hjVhk1mY2HG3FKsCkJoitj91dW/SwntD7nHHi8MDhwv2
# p1qPLH09v0r+2bn6W5yQfEbak2FWpKGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCAQYBBdl5qA1CXjiDq3BBYUn1hXoKelGEa7mIXW//qBaQIGaeiBPwOb
# GBIyMDI2MDUyMzAyNDEyMC4yMlowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDI0MTIwWjAvBgkqhkiG
# 9w0BCQQxIgQgV6y8EXSvqu3yYWaeSbeGLfsBfx4CwexEJBR+9kjNi+0wgbkGCyqG
...
# DQEBAQUABIICAGM12TTmY+Z7pddICkvTRN+fbzyExrYoZou0iXji/XtMsZIyC6Kn
# ji/3tYkT6ZDS02Uq0LeTfhBislRo5sioZDCP/VHeuatgwkHqVKgkCxVi6RvkHZlZ
# WosIqfVx5DlKbO8shhaxcbmMIkjX+a7oFW6wsylkpnzEr4xJ/rt/Uo2nU6bSTvSa
# 9gFkH61v67Cs3Lbv6mZUMQNNXVCqe7GJ/lQFqP6RtXcYfwcmqJaiWCVDO3uSUbH/
# q7yx+Wvmy9Atgi896+BhjHyBdp/E/+RGwxZk9Pwne2HtN/gBBOAwaK71QlNoaRbV
# zTNa0BXy5y7o0koMQ7d+i4LSPUcn/1QYL1ENXMmlzvVmlzY/Kzns7ZW1ysYOIhP/
# ylAhVDJ51Hi1ndRvdB0Ls0MxkIx6igc8SPBiRztBe+iaXGE1+5mZuMqGNP+AK3Rk
# DlP4L2smeA9c5d58OpmPsXlLbv80YphknQdSFlB5ltFzcTUcAhJutLGDXFXRCrMy
# SwygXnZ5dMDOPGx3o+PNmgFEx9Xfc2xj7pjHJnwt7sg3dYt68a/OPanlduM2jFsJ
# zzXlGJG1KnB4Zd1kO9xhUd7b0j4N0W0E4DNu8K4kkNAe9aWjtGHdBshPRcShwG41
# Hr6SmrobDS0V6ST8biNYqky5VCxCh/ANRMzYBJptuTkSmXyv9ttXOVei
