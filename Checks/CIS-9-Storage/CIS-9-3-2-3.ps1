# CIS 9.3.2.3 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.2.3"
        Title       = "Default network access deny"
        Severity    = "High"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object { $_.PSObject.Properties['networkDefaultAction'] -and $_.networkDefaultAction -ne 'Deny' })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) with default network access not set to Deny." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBOf9KyME0OLkqm
# AyH0BKOmXyIwh8GRIe0pjziwJSxskaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIA8EUwS64VQ1Tf7cNhxLS/2CgzBFuOtV2EAfKY19
# a5coMA0GCSqGSIb3DQEBAQUABIIBgJztLZSlL71Y86H9+CQh7u0ymiUg4c83keRC
# Gs5Wf/153TWdJANtySsMpkMl7m5QIM65rXLOSg/sytEwCxpw1udLn2sIxZfptCLd
# kyHCdQ0CxJjgqQHCiTSeFPav+7iv8v35FkUK7L4MOe90/W5h85u2qquqfNR6UxYz
# uWoOh1Si/qXt5Ut2D5vAdx5hDLcXrwEpENT7ytHpjRNe+MLS9A6PpeS6b+f+GZFi
# 9bY17IOpUQfpNAgXBIUHvnlPG3AIsmtkox0iJfA5sqV2SmJgUAhzGrTu/2svSjer
# c5cX/IUg9rJYW6yCk1BnrcYdjlhdKy/He1K57PdKca5pUBQJRjBBT+FEeBVuJ7vm
# PtTAwySZQdtGHtRzpW4HmAELMK+l3D/qxJdw/NGpHaX6++0HTrAs5AL0cSOUsbHH
# Fkmdz/d5Y7pn7SNi1YmEFRpHgvdSkowpTXlsK9AYSiy5oD3pJg7C9Dimv9JJQ9ui
# Yqd6nYQDJGTRoMNwKzXvuQ4PbvDIrKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCDXd7CH6IgRop75sx+OEqG5Xg/3erPnQ4BHm/wDiCDRJAIGagxE2au0
# GBMyMDI2MDUyMzAyNDIxMC4zODlaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyNDIxMFowLwYJKoZI
# hvcNAQkEMSIEIBgQ/+ZypOEwLMYavO+5+F0usdKFCIfZE8dXNzq8quDLMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgA+/ubbrnnRiA3/MNLmKYGQGDqk3g9ao1fLUlwTdFRcjWVy
# uOevJzc9TeI8H0hsM+e63JW3tcTfznULeQfkXAlzF7c4+6Dn1/ffgZuZh/KtpZml
# ayW4rBFXbybPRhEt9eA/Yo50Lsja8KxdSwLHcHUMkbPG7afBRfo7QGlUUY6aHL+1
# +Guglh0inPBW0tNU0r0GU5VpswhECSErHG4a6lEn+G9MxOZWeN/qnY6nZM35yM6X
# Rn0yM3ZLxN0ArY1mXJl5k+9n7Rg6TN8RmNlb+xE66eoj9j4o4Am3/t9EzyAXRkEc
# uM+e4vBJIITgtaFt6Y8+CqLJrBF1gBGoGXy2VZ0SlBRYVsV/OIJZkQiqC/RB+Q6x
# 5NiPNL0WK64aXv8Bk0/Ke1YkZcEYcZ8TOzsZ6JTF87y8SNja2pNHN+v3kE9M33P9
# UYs8JPZXXxllTmVoZfAoqUcUshIvT0Y4AzG1lYOAaTQyN34s5DcRUUmfKEzDCPII
# bcxSWVyYnGVip8u0qHBed11/wSNXJ3QBuBIulNijW/XswORPcUgOYY/8QuCLkpvR
# wBy0JB3qNJxPGKm7aG0x/sZxtrWFY0j88YXsemkjlXxYPrRFb7s0gHA6W29BsTwd
# g2abuxyppTk5S8QFwOLTi3fi4xUFtEu+ehON09PWbpho9aXtWl70wk2RAdMqaw==
