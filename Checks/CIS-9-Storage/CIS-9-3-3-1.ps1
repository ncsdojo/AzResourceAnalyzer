# CIS 9.3.3.1 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.3.1"
        Title       = "Default to Entra auth in portal"
        Severity    = "Low"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object { $_.PSObject.Properties['defaultOAuth'] -and -not $_.defaultOAuth })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) not defaulting to Entra ID authentication." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCYKJDAl8C3+9BP
# EW+CwkOZ+Tkjo8SuykJFtaN7npRsMKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMoEpIMcOQvOfIOtZZhYjp7OVfHDdiFrhtxdRP2t
# GvpOMA0GCSqGSIb3DQEBAQUABIIBgE1xATIb23lIUaXnCOdKwtvMLp8z5vjKtuja
# gkpsn1ctx0nxtzt4tgadw63OqbdtCzyDtrqKNkNs1S9z/yjMp9CK9ptFkS/mc+Cr
# 4P+mQSmWlqgvwolRPajHXQXFTWLnht0JJoOjbJKzJis7mmjAvjm4TVTT2gBvcFMu
# 7r+w6AdGw7Mgl9NdHs6na3TPqOfdg36YT+TCboC7mzEGoumkOK4HhRfS3HIQHrlu
# ll69zMBqm1OykA8HpP2iqocpg7UHd5vz7tniqrrqRsDD+GC/9cO4iD5SA4uBmpwU
# TpPDcfwUO2FeEqjKQnrxIvbDAy6zMFG7Iob+nwATE3SH0hGqnXoSibZhpzL9PKi3
# u6FHHPFUXC+HFBstg6rTv0XY7sqHgGyIvvCSvVpIDahqMgGA9sBSFLOw84ScYNrq
# 0t020uEJFdSGndOHPOYlQdz3E1mOaqoWmAa2kGQ0QM2gL19UerfTaTKc1kDK2Bay
# hMrs271PUxFbTPX15bVV5kvA3x3uGKGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCDYfZ1KUkpwebm6h6NfHKy7OGgvM8J3N2Vyabf+H1CUCwIGaeiBPwSG
# GBIyMDI2MDUyMzAyNDIyNy4wMlowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDI0MjI3WjAvBgkqhkiG
# 9w0BCQQxIgQgZJEFCjs/FykC7ahv4As1hegHlAQ1lczSvlNnLn+LiO4wgbkGCyqG
...
# DQEBAQUABIICAEKq/xbZjWQZ7dHQsDe3oFvBynD+CllsgKwEUfXJlGhyqsK5yIyv
# IX054SZDyoXfKqjT8YJWjQofGrNAiczCRnQC0TRYglQKlXR+Zo67MFYJIhvXhYAd
# tC3iUPkZvfZgZg7KPqyoR5tJ4nElzJuUiXG+n9AaOEDhjPIzGMxjyJl85tsyQg4D
# Cr/c55Rzku+h8t5OiyFbLNF4CFPggRHOkAvDXkQB4aTgeMK7MAlB6YS4RIHI9lWg
# kzu5VjjZpbP0ZV/+LNttaNo2jSHEdi6RAEqq3Qjo9bL947Lfj/mTOqz3q8RfkIf3
# 4g23DAK1TUxF7rpRAigpXYFt2d/8A561mHdd2cSk4JPpkproVj5UPTVwlKJFf5it
# zpaWrGEQyhTWpMwztG6FCT1Wg7PmKH8lb4a8mDxk+uRyDp1+K6X9NSKuaD6IBenQ
# oKTEw7nJEd3OUt2h8Ne19ULSxh1DqijO3+dLMkuBEOfwAlZoKu2D37zc/4LHMqIA
# QDbfeB8ETE3fNI1bkDfLIvBgq8bTR/VX59IUf17CznzT1Drsl0wJIhD9AVqYWk8H
# SKD0uXxwLqdDjMJNi48TazMcD5wgKPppXYVYhwvtrYzw3tT+HR8/sEkUh61sgS31
# JtTucInB+wepk/HiJjakjhzaiow/AxvEbe25zsloXhOi4KU9tAFZUQQK
