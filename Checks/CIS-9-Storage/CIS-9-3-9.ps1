# CIS 9.3.9 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.9"
        Title       = "Delete locks on storage accounts"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
            param($D)
    $locks = $D.ResourceLocks
        $sa = $D.StorageAccounts
        if (-not $locks -or -not $sa) { return @() }
        $deleteLocks = @($locks | Where-Object { $_.lockLevel -eq 'CanNotDelete' })
        $lockedScopes = @($deleteLocks | ForEach-Object { $_.lockScope })
        $unlocked = @($sa | Where-Object { $saId = $_.resourceId; -not ($lockedScopes | Where-Object { $saId -match [regex]::Escape($_) -or $_ -match [regex]::Escape($saId) }) })
        if ($unlocked.Count -gt 0 -and $sa.Count -gt 0) {
            return @(@{ AffectedResources=@($unlocked | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($unlocked.Count) storage account(s) without CanNotDelete locks." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDY1Q1qTNA8kQeI
# Xg0iKI/GBiv/aPZM2KJ4A84ld87zgKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIJMIYQHoR6akRr75wGKZNpYp0OwWCa/e8GYfP2ke
# veOYMA0GCSqGSIb3DQEBAQUABIIBgJpHFnDQzeRJK3hxppj3iLM8v8Vb5SERY8Uw
# gX6EGC5gV0sB/ExvWM7UrdcwOnyUFoPPD1A+jxLDvtHAC3YqCRM5FFelNxgQPHXe
# Vd+aa53gAtplWHQUnsEManC+HvPZLTgeMUsz3IiK27RzsApOj1NFlHD+b8dRWvm5
# XdJzmP82AO+STXI4enfwdMbbD3bRfOEHlM3/kmMxMw0vmJNRq7BuZ1SoVaDZeaxn
# Ba3cFlMYgHTBUlbt5nAjwjmS39jny8UI8ULr13uC4FmTVFl+lJIyzVUeOhkDcyYB
# g7yJ69X3gPPYBnrGeHMzMEIqbDBWg7JDT6MjaJh/ALS62XPX+eKsE+dPTXZ4xTtl
# hKuTKfTVbZl3xS+cbL1Wst3pVZIvjp01ACfq/orcmUBKulgx5aK7ohmUFJh+PweA
# ejPhZT0AXbZo4O3KT19t8qepvxRFlKJ2OPcytLWHL1w73mvUgg/GH2CLmcTBYdts
# lV307c7AUGtyroh5b04/CbLEyH163KGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCDCg8UCjTGnDhk7M3dgP/RRAj7EbW4JpAHva+peT17IAwIGaeiBPNjq
# GBMyMDI2MDUyMjE1MjYyNC44MjlaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MjYyNFowLwYJKoZI
# hvcNAQkEMSIEILCHfnkcdTgu1URyGMHpOg85gxaUhBaAfR7CY8rqBWTxMIG5Bgsq
...
# 9w0BAQEFAASCAgAqVqX/bhdSCdzNhzJbN3q3RWvP6ghOcz2St2sfb9jf+jiEazyy
# FVMz1nwCZ+r1UqhkIFsZrhS4drMGUCbQKGpR/oPTSek8ikbi0NWhsd7vMUQqqstW
# ys8h5PaFkQXZxSYCw2kxohwo3qXgRrgbyKa6MNLZ1OZcluDfuwwHETjwai0LavG4
# gpPkyDO+/yxflJZdjEzzAYHYZUuu9maEhlFZ5AxSP7auf8YjRKTavr5E0bSE63Nr
# H4mUXt/Ro1ZR8rcRl612X1XQctcGOWlNOFV5j/PyTxqhwPi26LFG6TMZTTuyVzbQ
# 8pOT6xXUs+FL3O7UhGE0FA63sxcgsl0s9+BFA8PzIwKp6fMKVmc1n6AnTu1VxeaF
# pich/y0QwAha93SSqnzQg9luLNuNfQtQl11m5WQ1YB7Ckgie+8Gx5IRu3yMQM+7t
# ztl6KjkYD9w5szJ74+H6e+tRylf2icfQa+sCsWDGDQc1brIBOAn7bn+7Y7blHzNq
# LqRAdJfDhWielgH8sQm/+Qew1KSr9OhLCeKZHB+9h6+4d8SxjkSsPzbe/AGHPqNY
# zClTR41iv8TLCcuryfIdD3x6C4Joj6QvmX9E1Gb18GGe87AuvB10aO9DLE9jKU9V
# Z+iZM2IG8o058fQ3Un/2bGmuj9rvHPvYVz5VDMs5Mz6ToOhp+y6rCJLZrQ==
