# CIS 9.3.8 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.8"
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object { $_.PSObject.Properties['allowBlobPublicAccess'] -and $_.allowBlobPublicAccess })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) with anonymous blob access enabled." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCN1EvJbqzVnMxf
# LV9xzXzgk0UKEZRUu4MmXytyy9gfHaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEICXGq++K5HYRwyY1c2ycHmK/bNpG+0AS2uEtDpUa
# WxcpMA0GCSqGSIb3DQEBAQUABIIBgIgebB3OG/9IDborj2Ae59kA4+EIUYyQ7s41
# PaTg/LVkQKZF5pUI58uEPUAhzvWNjrRS2mpR32ap1K3ZPIONknHmVDZAtcsiWdOF
# TbKxD4BspR6zw+OULMEjIIsctVyV/VgyuTGiQy2/JqRVEX3wEvdQVSH/9ljBsIZ3
# 06bB7UYkgVdHBEfliOFIBAJ6OA/+me8iUO9kkhuu/J5HrW9ThzALfqUU1eH3Hc9y
# ivDcrpyxLfPN+FdhqCf8UIhpdp7g1GgbrOxj6BWavZmL1SasERgRojC2PPKW50wc
# NRAAoEPn9WDpuepn80SZ8IlHK/8Wd686kmgJwM6m6Rjl7TqMjTmODhY/U+7HAYtd
# ZhvT6YkUxffXdBLmpqP9E3jG2XpvcPBJKEOYaGEmzSujHTKDCYs8RS3Y7TCHPNcM
# nduNBJczDaj9fa3dMhA98qzXM33jY5aY7as27v1nv2b0fqX9P0cUyW13tuttip3h
# O+Yj+BBFu6Ea9SQHOSrRXRWgW3JrwqGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCBwITkKTpEDll+/03aIp6S7vJ5xUvScwZ6jiti442A0BwIGaeiBPNh4
# GBMyMDI2MDUyMjE1MjYwOS4xMzVaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MjYwOVowLwYJKoZI
# hvcNAQkEMSIEIFAj3OW8FuBxGslJs7LCTbkFKaE1RV3JEq28DQpWc/ZbMIG5Bgsq
...
# 9w0BAQEFAASCAgCgtgU24Fxz+I+LNsRiaOjnHn44YIa0JQTLlSHyzlkgqGoAeMsq
# Ys0qhdtFMo3cUqYdYp39wxpqfOb/SLgAkN6aumA/LaBUTmFo5K9kaplwDenW/M8R
# gzm+F8FI0mqwTova6+EujOviP/aldIcEeFcsJorFX4Yvj9ET4CCNgysiRDgqQFdw
# U7/vD6z3UWeq9vVWIznT+onpckt5kchmStkwbhsxeAHMbtwqu1gHZOE+FezGuR+1
# I9DKqdczIINTAg1k0eYj1tysxshg09gFfSLWAEanUotAtnp1aSMRpaA9rgJrNVyY
# LXPY5bYv4WQXHIrELn6iiQgDKfDP9sOGqUKM99GVnPYBV0kcIPKLh4yuSyMmEM19
# NIARNw/QZswmeeLKjUraqVHYmakF3e+z5po3Dp6IVpDoryzhRMU+r3FRicHwNf74
# 5bshlboGHwZtNhPKfUqx5JrYMa3LVsJtWKB9gA7JtewxyE52+6e0AIanY+z2qVii
# QQ7rY2YnRu2shFDtfMBiuySK1MRHyKYjkYFrRVCOKp6HNOPMjkD4J66+WGOnhl/L
# EWLSPtZ/Q9s//jp2kOeyUnNC86VYETmSOpk7ORly+n76fr1xsO4ttokCKl0FNBjk
# e5JER7w0rBDtMqryAD00gYDqE9FwaKDZ+7NDyUJDaDMPn3G7yGzQmYW07Q==
