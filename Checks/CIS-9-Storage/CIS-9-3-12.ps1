# CIS 9.3.12 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.12"
        Title       = "Customer-managed keys for storage encryption"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object {
            (-not $_.PSObject.Properties['encryptionKeySource'] -or $_.encryptionKeySource -eq 'Microsoft.Storage') -and
            (-not $_.PSObject.Properties['encryptionKeySource'] -or $_.encryptionKeySource -ne 'Microsoft.Keyvault')
        })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) using platform-managed keys instead of customer-managed keys (CMK)." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBKaqZhHBXi46jj
# kwvv6ICHN/wE8p0SO9rbbPJffYZFq6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIBCr72KKEx0lRgrifZoUjD+OWMAR3Gmit+caYAdB
# TlwkMA0GCSqGSIb3DQEBAQUABIIBgBfdv+I8C4ZIq+KJreeq+to/8eHh2JedrfM7
# GipcTZT3jHoN58wSPL6Jhv6c578H/u4/qdZSVgY7Uq9Nq62DZsWaiB2btz7zS0RO
# 9U4fehdehsQ/RpiDI5Sfi8FHe7ylDiLNPM1AcdQo75+dfRkGlxhOmvi6WZMrcwbZ
# GPBK2J4PLfaCNpmxxVOVo/aM9o9iuYBn2AmOmGS1ig98gKmqDsYcnpIZuggWFSds
# mmZiAv14aUwoAxUkhK01AvsThZQosHSQMuE+hXa7pktEIsq9kwAwwDXxV8omcXk8
# +VHScIQT8kE7MYDxZXwNOKjZTOuGuM8y9CDQK7NVtBtAsQGI93uR8cJ6WGaNlBom
# 7svbxQ5j6cSxqCGfso90IjsH4KAA0Wc3eiC5CknQxfOkHFURwBsR0bzxmaoYyFS+
# HBngH5b0sY6DBY8gys/Q3xmEKPC4IURL+9OE2MtG9CPcus4+P9Gbk7N3nUWNdWXC
# isi3r8o9N4PPyoycZ47OvUzPrEqpraGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCCWQlMRu2CLGZbXx4kaKOt4jVSe0YEI3JBlSWJShmvJUgIGaeiBPNPr
# GBMyMDI2MDUyMjE1MjMzMC4xNTZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MjMzMFowLwYJKoZI
# hvcNAQkEMSIEICYuETgIWbo/BrU2qH2cDPT5NRWRclnIdsqq9tzLuDK2MIG5Bgsq
...
# 9w0BAQEFAASCAgC0arlQ8ctf+xgp+oieKHNG6etAZHBCoQTcdLZPRK8vJAQdrQIz
# ElZ/sumpaIx3UuSDYIwCB4SaAHkMIXuNikrBMHw0i/waTwuCBy+UyF0oN/Z+VCAu
# 41w61MRN66rvgtdP/PuPfDCjBDmNIo4dhkeQSBGENzNBT72suIRSNkSwnOLcB+Lx
# RYHcW9zGm4eaZuWHKpqOdTzWmDJLEZEHlVYT00aMt2iLXwRAzpH1S5t3PtlNnFOR
# xEkIJ2yriD6SRzFAfeYz5m31POxQHvKRpTg59RHV0d4ZwLDrRs61ldecfbxqtmpJ
# PuooM6YD/Gx7DBjxh2M42gWf/YKTz3o0iaLQtYFtN0E8AVQQJhSLntCRy8jOCxnj
# B7z/99k1amyHWorhZ9DlsK9qao5Grx80lrJL9NYdDNIFejdnTSEu2ygr6FECUQ5y
# ayI2huPqLc7s9cqy6zYmraAS0cPy1OJketWcYhkkwHwNMF2hqCgK6+I1KARCdPgx
# sm9QzhvP5NpHJ3+FgVND8kIh2gE3XoD/ZCUuWMmgt9x+sA7v+qyAjPC+wQSZAo9l
# dTdowjfOX3CP8nyJXL5kjMJ94PPmp2YW1qkXXZ3ybdvD50UDQpjKygavpDRuO9ez
# Gbx2frx0JZO44UtneJkjuxFWzc9q+RRSQV2QfNJfRiP7zB7LV/qGBUd3kg==
