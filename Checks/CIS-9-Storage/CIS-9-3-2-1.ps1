# CIS 9.3.2.1 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.2.1"
        Title       = "Private endpoints for storage"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object { -not $_.privateEndpointConnections -or $_.privateEndpointConnections -eq '[]' })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) without private endpoints." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAjSkJFU6l/kYSu
# EEUhoFjrfgDClEB7xHlV0FxncFOGaqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIB6wyjrrQ9clj1HmNCKZ6uOoOWEUcK24lfI+ZrDX
# E3WNMA0GCSqGSIb3DQEBAQUABIIBgJ9rwcuYZPDvYd8/R0Is+NU2MunB7d0c0jST
# C+HY4/GpAd7utB9NKaBKN1JJGmNep2dv10LbF8gfhPusgNw5QqU0Mpu0J2lIYuAt
# bnJdBQRVwcgXxOaoTgzQ7JoX6EjkyO6IiZoOnWAP8JrBfVbnT8wwGo/FZSZwqPbX
# UMPnhoqtZraRKg8izIL5Cgb6pfNV+T2fJkR9IwPot0HBVyCk/KzUkLx0Tm1hS4lm
# 31WdXDEkpkYpIVuNO3SBtDoFvE6NC0o4h1c+drQ10cVtaFY04V+K/Kget294pSez
# 3j4H+uXtULIW4CEn2OMdINMPXro6Q1AjU4zhOSvRjo+LbEnBdF02es2jCy8ahUE2
# abPbHm0qbUgiuajiQGghg7Hudxv31XvjXhjrDDstQf8oa1uGeosHWjgaTdJXuM4R
# jsIIdgpJae86Gvzaondz1IHV3ZAIinVy/u4Q0yOpHeX6IkgKYbm4TzX6PqgE3QTS
# 1XN8vXdZJfgwL2+5rIaSWgZRyhiCB6GCGBMwghgPBgorBgEEAYI3AwMBMYIX/zCC
...
# AwQCAQUABCCSiehlvOr93lLWlf31TOfoR8Arx3nEzRxDe1XvVoEWLgIGagxE2auf
# GBIyMDI2MDUyMzAyNDEzNi42OVowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDI0MTM2WjAvBgkqhkiG
# 9w0BCQQxIgQg2A+84DffSeE7UK5UIDryTbkvXiZK9Ky+2j8hYs5mlhEwgbkGCyqG
...
# SIb3DQEBAQUABIICAJhIktLQP40KQFimNpaE5ljESaQXnPqsNIqhK31muLsqD9ST
# qV0eGayivDG1r/zmUs7E4c8ZHVLeQNeBLsZuy6AgeKOF5n/9XsKKm3F79EVPVsp4
# qqh7HK+DFDPHR/6h04PVG6TxkNu+Toq8daH+DPLTd1/Dq0R/0XDjYReekgjbpKYk
# Nuj3v+Bn3ngq7+RUP0bIPWwSdqU8i1K5bsbbrgSHthbpHnyagwX8aAs7Vz/DyfDn
# L+iMFFsmsMoq773QNt+PPFeVA2t8kpOzwPpcz29Kz42kadpPtrigee/lGt2TNmqH
# fhZY9WVxL39B2JFSdQ/ydkGXsIWovzyVSdAiIDqnOmR7h5zT/BZPxdubqCADVz1W
# 2BnqFSYxwx097UNM6b58qlP284nZnONFP3jLUjHwYa/f4HrEn5+qAHO1FfBzVl20
# 4sqvLGefdpZZCKica0p7pEp9tlmagR+hYpvL5ONfPWQgFt7P8AJ5X4q9+sEymI3b
# z+vhSnTFUMs7oUHyZpLJvjAkbAaJaJSOqJohFjc/Ur7bu098i57ZzKbQHp1AvzvB
# iaTn3eThC9ddevJY3MnnoyUxag7WhW+47n/CNhAcrHFaHFUIKF3PRJQf8WIj+IeW
# X8LBbwe9bnc1O+y0ocp2O132wVSIH1GvctlavFNxGjInWtt1EycJw/DXONQT
