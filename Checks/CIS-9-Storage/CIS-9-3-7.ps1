# CIS 9.3.7 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.7"
        Title       = "Cross-tenant replication disabled"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object { $_.PSObject.Properties['allowCrossTenantReplication'] -and $_.allowCrossTenantReplication })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) with cross-tenant replication enabled." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAtPqLxj2fQVw6N
# fUnUO6zJuBMO4qvtA3BZT51ThsckhqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIGzXg9B+9M6gWSBI7beVZ+yxnlmOOcuW7ibJsdUU
# NpO5MA0GCSqGSIb3DQEBAQUABIIBgAo4lK/0nPFDH8Qr1R+HLeFU+F/Jeix22dX7
# 4oSwwMyOJVqLHEGf1432qsYPzWoZawEyQI4bYTaLtbU0yv4nTbZa8azMnKQJYFVI
# tPq+AGxyp/sxGaepnF9f/Q7trq93hz5sVx3LuFbanK7eMO8BPiHIVx58v79YpGGI
# AOhqUZv4emVP7TsxhJ21AObpG/jWY7ukz3HY87l6dZA+7XGjyDCTAErlj0jReXKK
# oG9doiJe6WZhpm+6mW2e8utc2wYmsdefoBjT++xMoOAUmyU3oKLSazm7CI5jVDNy
# FWeTQ7wQ5sRa8nBh1OEQYOnCCUjtMZbg0/SbMXc+Qj2V5DI+9/LELY5Yj2iHUudL
# dg74VE3j99nY1DJDFWE1I0bSGfK6X6fEso6T0gHE+7HWrSeHKYN9lZ3OBBh9nPUC
# 6tPKaNYBbglBcxG+FOjW3+MENNUYoCzbZk7woF/IUDMHdZLm6EDpMWbKGzSPX/2W
# tbLlm//Dg2oSlI1+H/aozHMmDcjFlqGCGBMwghgPBgorBgEEAYI3AwMBMYIX/zCC
...
# AwQCAQUABCCB9hx575NcNwFbWtW6ziq1naj6cpH1wnGZ/5p/SvkXcwIGagxE14bC
# GBIyMDI2MDUyMjE1MjU1My43MVowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIyMTUyNTUzWjAvBgkqhkiG
# 9w0BCQQxIgQgyHTRRy//et/EE/dk9wKzmoe5Gx8KK1EgfFYrpfExif4wgbkGCyqG
...
# SIb3DQEBAQUABIICACZ3u9JCNsDMFs9uhFHjdwRT6ibTMWmKuDCdsPM5KPrfPp27
# f1PT2tPWn+q9zomeZDoD4Ky3VPOP0Njs6VEwkuZveXn6aZiUuQZkJz7iyUH/6cQx
# OYDN5+z9JkDg3lVg6fJXida7RUCKmmP684Q6GdS/BTYEgdbS5CPJ25OgK3agqhCt
# aInLgcX/JcMBBoOg3k7X5gqOh9JEawNDLM/+4t1Fl1uhgx4e8WE1vspHzx7hHnjL
# MlK30r3qAHW2fh4ZBSKBJtzMmHh+YojnSiVwiopoOzHYbM/05jzQa/9LA69kCTG2
# J/BZQejXfGfVkUvneiLPU/NkWFE45MfJoVb06zZdxo23u8tAujTeAkAreI0rlDNz
# wwmO7qGYmZjglYPwml7EIEDkE56V0v9hrTxwQfSrOQxgxMnAwAmjmnmqf3p/MabZ
# 0Ab3gul3olmSo1EwYfsuyKxPR933OYmCdEShyzZrYSPHZTgKxcvBMguq3H74Jy3N
# axd2YlQzFM5LAM7s5KQLn+lr2f8caiZcoYoCXnx+UJcpfEVak6ZyLUO/5+59xiGw
# eSkQjtwHIGghxlHPlUBVHhaTCTVnvTfswi62Z9KKcnbG14SmJ1ufuckDHAznnNs/
# p9Qq6mvedM27ExJ9/fxEpf2ddKMSkb+rh6IFZnVULo6cA0q6N7F2PErrTg53
