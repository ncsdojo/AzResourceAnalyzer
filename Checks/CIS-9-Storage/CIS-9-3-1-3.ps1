# CIS 9.3.1.3 — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        CIS         = "9.3.1.3"
        Title       = "Shared key access disabled"
        Severity    = "Medium"
        Automated   = $true
        Recommendation = ""
        Check       = {
    ...
    $sa = $D.StorageAccounts
        if (-not $sa) { return @() }
        $bad = @($sa | Where-Object { $_.PSObject.Properties['allowSharedKeyAccess'] -and $_.allowSharedKeyAccess -ne $false })
        if ($bad.Count -gt 0) {
            return @(@{ AffectedResources=@($bad | ForEach-Object { $_.storageAccountName } | Select-Object -First 500); Description="$($bad.Count) storage account(s) with shared key access enabled." })
        }
        return @()
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA8COE5pZ8VmBgr
# GjpkRQOZ9C66giP9kifwxeExOQ7ie6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIG+72lYWkX/hlI+yo7v3uUCPjcavAIVNTjH9yGvk
# ek3VMA0GCSqGSIb3DQEBAQUABIIBgAHEm40X/Bq22ac+UMjwhKiKsLqxMjpFaJnH
# ho0pPeTpMNdvH8J4sXGg1mxkl8EZw3ud00DVL69/bm9xD8pGp3FyI8sRaAocS8Vs
# KRAA7cOUZY+4R8hdNKrTZPTtdI9rSLinAlmALI2Wgv7a0/LTZhV+640eMk1GCkXd
# 6E5SZ6aYll2j9WHZLz6QEmh7cpbJsSZAs3XnezS1o2qc8vi73oMF/e07gzQxyir0
# 0/kzfnujNzEJ2ddX3/7kuLjNGvEOdKGHDG/Sn2PQYoOScNXzyxEfD5IceUbms5Br
# dvfUPrw9Uqx8tXgsryFxkyrHL6PPEWBieSZykJOpvXNYda2FwdvBcJAw64G4RQlQ
# NPG7FfS0mJJ4z28xTyAoF45jNAAPIrSVGB1dw5mu6RSA7C1d/il7jmC9g0zBZhyb
# W+8XAuE2Oy9sI8CHsyHTdFoMezb73SbzXPNCGf73pd6Xtsx9uOx9/CiMoaxay4WZ
# sqqH5wRZZzPRqogiACctErUJKlJFGqGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCA/ZU8cJSmJtVc5u1xvTLGlDLN+a4VG4sGV3bXFdinumwIGaeiBPNNG
# GBMyMDI2MDUyMjE1MjI0MS4wMTZaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MjI0MVowLwYJKoZI
# hvcNAQkEMSIEINKx0dNzboICfQjI48a9qX6uwypp5UJf3AzFbCrzk3c/MIG5Bgsq
...
# 9w0BAQEFAASCAgA0MeUgCXkYA4NWV5+oViWTHoRi+LEGP+mPIj0m2vxo8W4Rpul2
# zjVwtWmSNfE/2DztjeHJDa3b6K/b3gWlTa6lUfJ6STMn4SyCF5zHcFkAI2YA76rz
# Qvqi6KqQaYPGWm3ZN1AvHmP6fuDnfUYzEFbxkU1t0iv3PiQm35ibqd9wKB/a+ppZ
# b6yaoicEwfIvjCWuKSgIFS9QP1DGSK8FGwjL2iVxNWjCKOli5MCeobo6ATPb9Rk+
# qTui/aF/4VuI8POLCkQLE8nr8wRVivd9GemudLv+/HkB3NmhNbcI3Rslr8tO0yqO
# tfgIBoe0WUKe6Kp9yB7ILRPE3Zd9dK29favg5j1yAdI7QgXJ4c0yDDcmxC11PBj9
# EVInHXA+OA84cGQAMZJhhoRMyga2VfF+u52c9f8EDpQBsGAtN2BK9G7xmhOdauOR
# 6jKrjppMeL0UpDFvqJor8Wn/d1DV2oJAo72Py8/GnnYGzbGdWwtOQu/NmyVbn4uQ
# 3obtVVBUC9RvNv8rJgjGbIaieFGHr7Rnhbq5hiHvhDNYv5KLBi1/XOCX5vHo19u6
# 8rdlljflv+nSeiPOXplSJFhgeHXD0JFDwnvm9QO2VTGIGPOQsLCFI7Lj5CIhdGVr
# mvjXfJXhen5p4aW/3RSkpEIOMpWhneudQbm6sh6me6BAmM5/UOjHU9aRgg==
