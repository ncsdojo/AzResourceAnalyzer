# RouteFilter collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "RouteFilter"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "RouteFilters"
        Label       = "route filter(s)"
        UniqueField = "filterName"
        Query       = @"
resources
| where type =~ 'microsoft.network/routefilters'
| extend ruleCount = coalesce(array_length(properties.rules), 0)
| extend _rules = iff(ruleCount > 0, properties.rules, dynamic([null]))
| mv-expand rule = _rules
| extend filterRuleName = iff(isnotnull(rule), tostring(rule.name), ''),
         ruleAccess     = iff(isnotnull(rule), tostring(rule.properties.access), ''),
         ruleType       = iff(isnotnull(rule), tostring(rule.properties.routeFilterRuleType), ''),
         communities    = iff(isnotnull(rule), rule.properties.communities, dynamic(null))
| project subscriptionId,
          resourceGroup,
          routeFilterName = name,
          routeFilterId   = id,
          location,
          ruleCount,
          filterRuleName,
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDvNY1Wnowkb13i
# qIjcRDhU/zxQ+RqBknHdZwmdofWMtaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIICCJHkCUq8ev4CM66euaspN+xrsamqCBGcQS29M
# QcjHMA0GCSqGSIb3DQEBAQUABIIBgJ2XmXn1kVoHjuSe9igHRGsGFMEVizGHNBPe
# jz0qPDv4mQtIOuTNQxUBWR6bvHZighB5JbbwUS+SthWza0NfgyGJ+vk7k+oRyOHj
# drRsGKNS9dT5OYq6zQOmaqUcfMl1cNTP+75elpD1fdbQIQd2plT8vijx2fNLmeXP
# xtCEFIsX5RLbKVQBnGSQyOy3Y0hkOflkrIGeK+aP5Q5VEikuXA2aeXPR7n6bMMYB
# VL6MN3IT9x4z49W5dAevdpsLda8MyoQJ62hyLSPLWkQZDIraszXqpJXLaakVU2w1
# 3mNritYaM39pwiBlhDoOFEuMjkCZ8+64pBN/7ZhLq0Ctl4rp1M8i99SZZk4Q7tvR
# 7MMFOe0f4l7Pw8M8UwHpK6fbC3gxjaSR2YywBltSad75d/972CcWQlwJHWyNdB5l
# XQ115Et/ioVEkpFe++fR0ahfgfgVC7XSdRLeEB9LKmReDnQSYpuW/8I+7z30v65r
# Tj3OBIWhq5vfJguKLfy/62Vt9wyHFKGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCBpyir2nTGdlMuYlEt6LZ7uW3uD4EWZTSZIPB2Ekx8FJwIGaeiBPOgk
# GBMyMDI2MDUyMjE1NDIwOC42OThaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1NDIwOFowLwYJKoZI
# hvcNAQkEMSIEIIQqVXMrPtYh7JVLavZEyf8fzuhHyhlTJf5GsnvnSl2qMIG5Bgsq
...
# 9w0BAQEFAASCAgAfmzisMMFkMp1kwopJqxlbkbyJS8Cles31JjUsTVv1vhZdPZjw
# uduOjxIPb6Wc1iOuCBrkiTpSKwDXqoLrdFJrjBmHSvo9ZUw6YsoByfLsj/VqpMom
# o5l2c5mOAUT1S0OESrtI0aqe0jgwETOVGt95w9jfIrG2DxLs+7RYqqFn/Te4b+It
# XhVcHTn3qbrSGHDd06FXU2Qs4DsnsTSLCEUK4lwbsW2nfejNcA0PQZMlIM0jnxd7
# kxUHj3g5QHe/g8QPzrIed36mK1LkdH57+miKmj4H+63zRCjLbt8bixvTx4sqBDyT
# EOB7PVeIFjQFK6vpEZCiwGmwf0JLCGcxtNqK7WfLRXaiUq9BFCN4gKSG1sqmsaji
# LiuORj249ACGbg26utpIHtjYBJlJugA+SJI4Xp3PWy0v/AKJpTxs2mXuQt3OavSa
# w8bJQhFU3iJPIbpeSslZKgwLz+U+QkPAjf5xJQIhHePDnFvxzeXYNSXHvvVzdAb7
# ZHv8jBwDLzBPwe+0RDArfkXZSbJcKi+QljFK0IVl70Xa5zAXFhILBb7KGUzzu0zu
# ATnajzoaSPB44zqZv3/iRFz3c18QqWxvnq182hkrDp1o+w1XEHfk4fCnVmcuk6DR
# 930RluOXk3WFStCnRsu9Az4xOZOkpWKOdXyXXlaV5rtAweGC5RZGeLN8/g==
