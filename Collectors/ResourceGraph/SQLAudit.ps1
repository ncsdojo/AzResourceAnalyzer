# SQLAudit collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "SQLAudit"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "SQLAuditSettings"
        Label       = "SQL audit setting(s)"
        UniqueField = "serverName"
        Query       = @"
resources
| where type =~ 'microsoft.sql/servers/auditingsettings'
| extend serverName       = tostring(split(id,'/')[8]),
         state            = tostring(properties.state),
         retentionDays    = toint(properties.retentionDays),
         storageEndpoint  = tostring(properties.storageEndpoint),
         isAzureMonitor   = tobool(properties.isAzureMonitorTargetEnabled),
         isDevOps         = tobool(properties.isDevopsAuditEnabled),
         isManagedIdentity = tobool(properties.isManagedIdentityInUse)
| project subscriptionId, resourceGroup, settingName=name, settingId=id,
          serverName, state, retentionDays, storageEndpoint,
          isAzureMonitor, isDevOps, isManagedIdentity
"@
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC0HXUGhSvvC/cO
# Q8Z/gr7eBE5webDc0t3DduRW1NZak6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIH+VwTZlkXSNFEbI77m7a4DNmYK1+JK3p0giCKkQ
# ZSVlMA0GCSqGSIb3DQEBAQUABIIBgHuNAsIEeGSwIrIa9JRjEELsX/9yWgztzK3s
# am21sZ2tiXc5KfVubkT6oT+QmFx0QrT2wzhnZvbx63d54RE/6Jwv9uC3xAQnYMPf
# /kR2Zw+QKNKNnwIYYj/djC1H+z4ufcVYBQAQKJufLLrXjfkFuoxPb2lMoR4lPebb
# PLd+LvKyL9zYTKgrEtfqcA3KCrW0loRP0GzlnIy8uzvfuRQF+ueog3ZSNqIyByrG
# WgymxZT7c8TwTBFFm194r95RZvvluBvTnE5PFbQasldsMJLztoB+RI0CkyMTtdFb
# +Je2vOYo1tCv44CvHmMXHn3cfT+IP1hNaXo1AN/4vnWs+OOSueSdDHlQC+hEfizM
# 4KNlAMMqlHRFIpyqeM/5V+VNu2XQ8Lg3/THGTO/Z0eolUhSKtGyoQc7tSC4dyVZt
# aDhoSM3ZBU+ir2nV+Phq5YGGwGfuO/tF0/Gm3ui3TBLyWqE8P1O0j6sfhIvs2zLN
# 1EbBZQpISJmELUzhmfqa1+Qt0rrCMaGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCA0c2HVvtFaNKT3e+t/eZ+5eBCCitoGgFSpwclV6bPXzgIGaeiBPwmP
# GBIyMDI2MDUyMzAzMDE0OC4yNFowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDMwMTQ4WjAvBgkqhkiG
# 9w0BCQQxIgQgL24xs7nDpbrO+dSEFqVzHlzkDD9Uz0Vr4W7yXsuMte8wgbkGCyqG
...
# DQEBAQUABIICAJg62ulLrXX5FjbWToNLSMCG4EP79KPk2mCp32qyyv2cusLEDcIj
# bJPMSQgNG8kCFGcay2R8DsQlgl4VZNcayx7X5X7F27I9iKsVWTz+HiO+1hGht6eP
# kq+9OtydJTB65EoIsCX48OOu4YC5yQjTsuXJNtdCeJyeDdpTOttSeo5vd2XgYq9g
# sQjeaZHLZnXK0oMp3PoE83y+2IJgxACe8l4hAqf+rEF40E5sriTYVbHbjEHbvSbD
# dTsVlF9rNCNCv39JdzV5P+95QS1WrD5ZamdnYjnUpxtyyNB+nSpBLnEE+nMeE4aJ
# YT6ln6Y/55ff1CZucTHlxh8MYrVk5QJS4IsjdT23F0R9Gidgkfe+iI9Dy2aGPkD7
# fmi8wEegFA5pOSlekPuqFogl87X+WznuKX6KlOGdhROGwLhcvXOdze6xa/81o6Tr
# xgLozB8gX6rCxF0zIkv3yal+op4p3GQ9mSMnwzsjjGHVs3Licum4glpDKK5NKyWG
# PQS6htiltHcTqGZ0+7vOXXqqBpdaJoy8K/VclVzzTZkTg7YCllWRrSAzUvNoNDBY
# Qo6xNnCUWYVe7hXymrRFH+wKeDUiMBBv8SMhKQX+kyr9s/44k5PuJD7/GNDZLpVs
# KE9kv513K7hXX+2hGpj94rhwCSvwZHL73YwILfbULTt92wx8YA0F6ALC
