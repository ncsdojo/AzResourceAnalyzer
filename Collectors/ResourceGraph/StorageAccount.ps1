# StorageAccount collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "StorageAccount"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "StorageAccounts"
        Label       = "storage account(s)"
        UniqueField = "storageAccountName"
        Query       = @"
resources
...
| extend storageKind                 = tostring(kind),
         ...
         allowCrossTenantReplication = tobool(properties.allowCrossTenantReplication),
         defaultOAuth                = tobool(properties.defaultToOAuthAuthentication),
         ...
         largeFileShares             = tostring(properties.largeFileSharesState),
         networkDefaultAction        = tostring(properties.networkAcls.defaultAction),
         networkBypass               = tostring(properties.networkAcls.bypass),
         networkVirtualNetworkRules  = properties.networkAcls.virtualNetworkRules,
         networkIpRules              = properties.networkAcls.ipRules,
         ...
         infrastructureEncryption    = tobool(properties.encryption.requireInfrastructureEncryption),
         keyVaultProperties          = properties.encryption.keyvaultproperties,
         primaryBlobEndpoint         = tostring(properties.primaryEndpoints.blob),
         primaryFileEndpoint         = tostring(properties.primaryEndpoints.file),
         primaryTableEndpoint        = tostring(properties.primaryEndpoints.table),
         primaryQueueEndpoint        = tostring(properties.primaryEndpoints.queue),
         primaryDfsEndpoint          = tostring(properties.primaryEndpoints.dfs),
         publicNetworkAccess         = tostring(properties.publicNetworkAccess),
         routingChoice               = tostring(properties.routingPreference.routingChoice),
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDNw6msEeDlc+cX
# 0mVSDUbVEJ2j1uNSF/fh4OzxJbvhxKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIM31InoAv13cDFUXjmnywXTUQJGsuNq0gnTOjDRk
# lKmMMA0GCSqGSIb3DQEBAQUABIIBgASbypyog+ZUWf0n+jggQK4kbbiSlI7/QIEe
# Y3gyt5vthDdPk0q9Zkx8QlCXKQCZ/iVLz+Lkn+TBTKnE3vlKQPl61B1Y6O9QOlD5
# lyGO6oApn+DIoqylDAURciK5sdzI8BOAUiKrx25PZ/pMKb3r4V1fDQqUGVQClmT0
# 7ZjiZfXzoXdXujqYhxedW0nqhg4/9IyElWCaWyHueIdCPXNxldyoa+g+Mpc3fpQw
# 5f3sq7l07nVjHtEBGijQF4q2i2zABIF48B37aTJxv78aS6gh/HGaSP/3Ym20ZNOA
# 0OmjNcAB90pVy8D6+HQVpFZpecQO8sCH7WnUe62TVzlGPB+vAYyg6pDqzUNy+Q48
# aiPthlY5w1ZgCJug1DNBtScG1037TqgTbYKgBPhYydhXYvmYVce2AaPWzlN5mffl
# S+thPoJQDUc2rucE3PaYaQYDDitQUMSGkLtDqFM9hRKoPKIrNDJ/Ycc4TP/plxtg
# wf+xIpkdvPoFAx6vuMqvcisR3Ux3iKGCGBAwghgMBgorBgEEAYI3AwMBMYIX/DCC
...
# AwQCAQUABCCL/Tt7wqEUOc9dmcVah+9ZF3ghG/kCFozsZwr24ApdpQIGaeiBPwnc
# GBIyMDI2MDUyMzAzMDQwMi4wNFowBIACAfSggeGkgd4wgdsxCzAJBgNVBAYTAlVT
...
# hkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjYwNTIzMDMwNDAyWjAvBgkqhkiG
# 9w0BCQQxIgQgBEUOslCMfcxeWdHshSDhPw3ctfLOk3DvviWAl3sgyLAwgbkGCyqG
...
# DQEBAQUABIICAJonR7m7MiZytqwYs1SHZa8ZEfz/DTo4KXB6xAF9XbH/DcKuS5mI
# Fv7dGLxPDVDKUD7hme4bQw0vnNyQ/0H2bNFot8yC9LKNovw2YA9I87qIoJPPPM/E
# HtxEjIlyuaUfu6VVsyJ4I3RY9z7EcS26/qb2fOvzOyYuUxmTRzMWqVEE3ISNo5nj
# xJjpJ0B3GHo043KG9D3pwQqIw9fWJI08TGU21EKmvaLFd+iBiyORiQE3LDSXBK90
# DrBAhhMmOKQdMFrKNGxA937Da14W2A53FM5uFJxpTtZ4pq/5zvfIFScsHg8Ci9oe
# 6CD//X8iQCWIlqmyh+uE4nIPwGQ8RciIlzUuR8SKxwSHd0P1xu7hsx+af2ttcdoc
# jOaEyxj+wdmc1mxhssKef8g3MNn02lJe5dc6xBKAu3gqwJXKea5tFclo7m5MHRX4
# Ik6t+X2bmkrMWWykD/jebOJ/oek8PCD2kLa8ciqBFta+WRdZoGQMwkETquiz9JiA
# IHUUp7X8/CtoXgY3tRoviPcQj9Us/5n/qvzY7i9iTx+RtiAkz19iQnF7A1zR9x4+
# 7jR7y4Q1+DlxK3VNdgGNAXRijS9RY7X8U/V3wJxhuXK9LdpX47k0L/G0S4hAGxny
# hJWeffeA2Max491BoyiLozVep8cxhvCC5dPOas+hkPk9agxU5B1OpTU3
