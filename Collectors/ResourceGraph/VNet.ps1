# VNet collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "VNet"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "VNets"
        Label       = "virtual network(s)"
        UniqueField = "vnetName"
        Query       = @"
resources
...
| extend addressSpaces       = tostring(properties.addressSpace.addressPrefixes),
         ...
         flowTimeoutMinutes   = toint(properties.flowTimeoutInMinutes),
         encryptionEnabled    = tobool(properties.encryption.enabled),
         encryptionEnforcement = tostring(properties.encryption.enforcement),
         subnetCount          = coalesce(array_length(properties.subnets), 0),
         peeringCount         = coalesce(array_length(properties.virtualNetworkPeerings), 0),
         provisioningState    = tostring(properties.provisioningState)
| extend _subnets = iff(subnetCount > 0, properties.subnets, dynamic([null]))
| mv-expand subnet = _subnets
| extend subnetName          = iff(isnotnull(subnet), tostring(subnet.name), ''),
         subnetPrefix        = iff(isnotnull(subnet), tostring(subnet.properties.addressPrefix), ''),
         subnetNsgId         = iff(isnotnull(subnet), tostring(subnet.properties.networkSecurityGroup.id), ''),
         subnetRouteTableId  = iff(isnotnull(subnet), tostring(subnet.properties.routeTable.id), ''),
         subnetNatGatewayId  = iff(isnotnull(subnet), tostring(subnet.properties.natGateway.id), ''),
         subnetServiceEndpoints = iff(isnotnull(subnet), subnet.properties.serviceEndpoints, dynamic(null)),
         subnetDelegations   = iff(isnotnull(subnet), subnet.properties.delegations, dynamic(null)),
         subnetPrivateEndpointNetworkPolicies = iff(isnotnull(subnet), tostring(subnet.properties.privateEndpointNetworkPolicies), ''),
         subnetPrivateLinkServiceNetworkPolicies = iff(isnotnull(subnet), tostring(subnet.properties.privateLinkServiceNetworkPolicies), '')
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCpUuaeIMbnjoct
# vbZadnUa6da07mKWmkl6pGkaWyO6o6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIFLussgWytPET16QGwQ/vWSlDHoIyiV3bKJzmft9
# iA+aMA0GCSqGSIb3DQEBAQUABIIBgAZO+AyyD3NkkWoJTJWrh/4UhHPq0NXpNTTC
# npKBgKxDiALqz68dS7XyqOTawonFzzAKOQiNzxy2h+IaM5ZFrNOTf+5j//Lc9pe9
# ZYcfVgN0n22/9Em8vw5NkFajq5NsoDcypC1E1VbTKYXFIchRHdcwtnhO2FPvxsNd
# 56NeCTI+OCkc37o6ToQByV6kIvOyNfuYwQZnun6QdSXdfoYxU5weFeYSYyRiUCyM
# CDoh2vtwYOAA9s8MMqm//U6b2yHWcnzckb+9DfbrUX8Djs6dABrtv/r2cQtt7rQl
