# SQLDatabase collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "SQLDatabase"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "SQLDatabases"
        Label       = "SQL database(s)"
        UniqueField = "databaseName"
        Query       = @"
resources
...
| extend serverName                  = tostring(split(id, '/')[8]),
         ...
         requestedServiceObjective   = tostring(properties.requestedServiceObjectiveName),
         defaultSecondaryLocation    = tostring(properties.defaultSecondaryLocation),
         catalogCollation            = tostring(properties.catalogCollation),
         ...
         currentBackupRedundancy     = tostring(properties.currentBackupStorageRedundancy),
         requestedBackupRedundancy   = tostring(properties.requestedBackupStorageRedundancy),
         isLedgerOn                  = tobool(properties.isLedgerOn),
         isInfraEncryptionEnabled    = tobool(properties.isInfraEncryptionEnabled),
         elasticPoolId               = tostring(properties.elasticPoolId),
         failoverGroupId             = tostring(properties.failoverGroupId),
         maintenanceConfigId         = tostring(properties.maintenanceConfigurationId),
         autoPauseDelay              = toint(properties.autoPauseDelay),
         minCapacity                 = tostring(properties.minCapacity),
         pausedDate                  = tostring(properties.pausedDate),
         resumedDate                 = tostring(properties.resumedDate),
...
| project subscriptionId,
          ...
          readScale,
          haReplicaCount,
          currentBackupRedundancy,
          requestedBackupRedundancy,
          isLedgerOn,
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCSJK8l/VMfDDOe
# hbIaJmPlrYU1ihjkP5VDaZ3j97lGyqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEID4YKmts5ls/xt45KR6KhAEO9UIEUId0HKH2d3CU
# bKD9MA0GCSqGSIb3DQEBAQUABIIBgChNz/DXK62ZRBHhmwloE6C8R6H4XwzN/c4V
# xJv5yyK7vtlboNuK7AnnoMwGBk2hEsw8ewqZAZ3H6bxtwOoRzz2BC4otRdRBsV7K
# 1Yy9U+5pvS1MG0Gid3uCDNpeHd+idzXwaE6fXOit4Vhe3Kz/RwYoyvSNU9SapaTA
# pT2irO4NrmL9T7+WPWbVdWVnxDrATXYHuHgG86o6wWst8Czh7fMk0vEEC6oZ4Uvf
# hqK3cHXoVzCwHqZrdwwwWjswix2AEwQsiDPv8Ftt2ftCLj5Bl/1QhAjoJb3ukvvq
# kLkFQlnoBunQooO4wg6YKVGW4GUVDEENdjgL5HK+48NX5HgWH7AXMH3ckK2BwS10
# TYT+RgCI1J5QDBtmjiYdxDYK/F065rLKc3ch8bwdm7F31k+fN9JKcGUsd0Na3JNC
# eH1pR2lWhP1qDrNpZacWla5V9ZBavWUs+w9JyjnA/a+8e8zmRHtGsKraefGwJfWy
# AdmCzLIi/bX+kStAgjJARZMGeEqTuaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCC+ZJ6RTSF0yzPM72ssO+rWw+2ltcf8jVxvR9TPI0wDPQIGagxE2bF2
# GBMyMDI2MDUyMzAzMDIwNS4wODJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAzMDIwNVowLwYJKoZI
# hvcNAQkEMSIEIIP47pv7MGAXs7DkpvKJJN/r4k4oFDtVpqvgRjuHpB6rMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAfbKT2rhq6OPuqQRj/9w2yexDIKFwAkTLAzkYhRcdCFKIs
# XGdpZpGhZX+NTGnsd/odbbj6Jxa/uOcscGjE4eyiS1hPvdQei+cJlwWh6KvxS32q
# Cxv4YTxFO0x16Nrt1doZrauv/96BpVxS4WGYB/ibaV1XOwg259xavquFxzutpHh2
# bifALyYjIS60yhURbC3hmXL3PCktlFt20rvVMva03eXm+WxI2G66LBNUdjYfjbKG
# wIez3LwZvZ19TFvX72J0NI9k2zjKTslt+rMJvYiTsQd6dhfAjjqzR8ry1KYXMfn3
# Z4e/JZ57hm1tc2gdz1CYgkngUawka2aZeJtSGY8DCRS8AMKvls5SU0e2JdCzgVrN
# NwvDOTm4CRv6BXZzcXlPT0kEr+5HzAk8DP554CnKIR3k0TILAqwSkBgmpacuznD4
# ohuhhlQR43gLBfjhci41TsJpvTdARdD8q0u5LmH9viVBa+N0xKo8rcI7FZrtE6To
# F8HOhJG7rWSWrFvlkw2FaXyqYOr9HvtRM8x17DAn3HqX2IhuJCExuQQ/f4n9sPXK
# DPVQA/ZGWTNU2ONkAR1uNFtbqy+zHKtfqJXxHx4IqfSAG9UYVKYE5xcyEt+GO0KO
# A35r+MQLIQ09rFEBTE9AQYW7opbunenm7iVeeqXSikmoqpGxvB3jjVOAwi6Qtw==
