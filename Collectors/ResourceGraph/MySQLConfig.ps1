# MySQLConfig collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "MySQLConfig"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "MySQLConfigs"
        Label       = "MySQL server(s) with config"
        UniqueField = "serverName"
        Query       = @"
resources
| where type =~ 'microsoft.dbformysql/flexibleservers/configurations'
| where name in ('require_secure_transport','tls_version','audit_log_enabled',
                  'audit_log_events','slow_query_log','log_slow_admin_statements',
                  'ssl_min_protocol_version')
| extend serverName  = tostring(split(id,'/')[8]),
         configValue = tostring(properties.value),
         configSource = tostring(properties.source),
         defaultValue = tostring(properties.defaultValue),
         dataType     = tostring(properties.dataType)
| project subscriptionId, resourceGroup, configName=name, configId=id,
          serverName, configValue, configSource, defaultValue, dataType
"@
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBLNkaxmJKZ5krM
# Ov/30v/TVec4KjfUxe9fb2vPigDuC6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIAIcSxhvd/gX6SSj2mQpEdWAydyh2LpElzzTf4ny
# G8LpMA0GCSqGSIb3DQEBAQUABIIBgGXN0pTBO0mtOIFf0jtIcmPeO4kcS+HuO1yZ
# D7ZAssSED8xQeMkikojnfiX1jYpSU3fDiLXmUUVJllZjgZSyvUMTD3lErQdcG71p
# OCB6OhgIX20LjPQx07MxJBEK16nOHV80YTZIRPLopJm+xYLSIWu4siqQav5ZjlVW
# mwV9T5HZXPIl4CI1ufDcZgeCfaXmOiA8/KFv2NSud0iS66uLorFX4ZHW6CyPxJA/
# zxGVvNtxt33eY8/Pnth26GFj5V5MdJ7zbm8CTmm1P2cD2F95TVpLnM9WfTn5/5Rc
# w+ZE9MxWeLwoKhB4Zz+XgL2/Wu32lwMcJ1TArKh4UEz+vW394d6Aosrl6oP0YdjH
# kf/4ZZ8BGeVGS0xnyJ3mE4wkWmiqBmkuojJE2wKbhswPmFFD/Qp+4N5nt9TzOOC7
# 7Qt3sgkziheD8roKxdiyac6TyaTb1BWk9jFcqHXkfuMhktT+kp6eo3P5QBjLj6x+
# xNsHj2dsr92zUZpmfCh/QGAwSHKodqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCAeeMe9dfdTumQ2iufShwwxDDzHyU3ZLrefU76LaS+dZwIGagxE2bBu
# GBMyMDI2MDUyMzAyNTczOC42MzhaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyNTczOFowLwYJKoZI
# hvcNAQkEMSIEIDlf+kgMh7wZUiEaRxzbKSPw6/KDOHP4UM3rH6uSzc07MIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAa9H5YFbAfGRjbWnnfMHoNEEgR4PJk4hjw/LWz1QkUCys2
# Nzm9pRre3dwOT8GhVkcTRPohMHHtXe3pHttQBJNinANJbyOIa0rSLrXxZJy5gnMh
# K7MVCMom7U021wXTy8LsDvxLIhdV2RiQMbGB/gzriVhaJ+Hwek5h75CnKC6KeCqs
# RgZ6MXKWfYRYzTUpqKYQOhcRR1p7qOGD4ciVqhsmfRNLCaDmiGeyunRJPbho+ae1
# ylJ7QoyLsPUD6X8FfIuf2ml7Xap+nIIeU98e1K1Zw2r5h+xlzVFBt8ueyr0I5zcD
# 6VeL0/mJHVrn1x0mr1MtpdpN5PfVXtOCtYnxlnwACZpJw+dCgAZoxUvExT6GOOB+
# WpW2KyLgf3nW6PvWAh/FeNiwJff2znivPBWdjcUGHI6IQuz+3LFTcls6DeCtZQFA
# QlTOJ+RSbuWHcn0M/RC615cLExQQJrGLWRYPeBvzvdu6rhLW8jhbHE00wMDQIR/O
# vRe4xB3qu8c5IvpzhJNWGGHMdqpm9Zjkxq46VGPzUI3kz2SUIZid/6eN+/+5zAhN
# WBdzqTKQLbFTenDlhbOXAMA+QhoDuvM6d9DcpKkP26XNt7DPq2uJKsT7BwPYAhGs
# ji2ATGiFyeIDx5X2jU48f9hfGDzUbzTIM4561KApq2IlHEsdfmnYrrVuxB5zkA==
