# LoadBalancer collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "LoadBalancer"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "LoadBalancers"
        Label       = "load balancer(s)"
        UniqueField = "lbName"
        Query       = @"
resources
...
| extend skuName                = tostring(sku.name),
         ...
         frontendCount          = coalesce(array_length(properties.frontendIPConfigurations), 0),
         backendPoolCount       = coalesce(array_length(properties.backendAddressPools), 0),
         loadBalancingRuleCount = coalesce(array_length(properties.loadBalancingRules), 0),
         probeCount             = coalesce(array_length(properties.probes), 0),
         inboundNatRuleCount    = coalesce(array_length(properties.inboundNatRules), 0),
         inboundNatPoolCount    = coalesce(array_length(properties.inboundNatPools), 0),
         outboundRuleCount      = coalesce(array_length(properties.outboundRules), 0),
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCYMwLodOYFInuA
# KZ7APIDz5IzE4prKiowseUImIR1e9KCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIInucEexa3LfgFBT0mWfXCfHyt+sWgTpBXKYKWtR
# ubwlMA0GCSqGSIb3DQEBAQUABIIBgAa3U0nxYIcbp6pXFfjnRiV0Cn5VY+Y/gGhO
# A3CrT90te509kfvca8y76SuEH8t9tNVi8tfmfR91fEg+9EdwvoZB5k0QK7WE+fAC
# ONGkx7ZUQd+7qTRCd/ZW/lMEqk3fNJmNZGnCeOK33OTK+O6sNakoOB4u4OjI3GMk
# m5b4Nscti2vrU4PMvCnfBHceB+2AK7Q5FevBhjOUdc30oWyM8ocGfeRRBAi+FVzt
# 2abs/uzj/+N4CKlaOD9A0svMkvHaTH2gP8DIXeSHOgxxWINmWiROgclOziOpauIJ
# 3Yx5ZHa87mYqV7pl29YrbUG9tMKoTSV4j03vXSgcmTNd3s6kOsQBUV73etZNyanL
# 8qRyClAjvcG/bE0qwMO7wGEBMsSOxY5xJ0EBZQgUixWBQ5Xi5OFgoxB+Lsq/86aI
# 3fDv9gwKL/GpTwBaXbR9l+3sb/uSw43qrQ+lgB16cPjskcNkfGVRCPsGJiUcxpUp
# H6KcNoapGXAG6mnQup+B8buOo7ucaKGCGBEwghgNBgorBgEEAYI3AwMBMYIX/TCC
...
# AwQCAQUABCAyVzXnBShA4fVc6ABMdnL3bCIxZ+o9NDT+Ja0vUxeyzAIGaeiBPOUD
# GBMyMDI2MDUyMjE1Mzc1OS42MDdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1Mzc1OVowLwYJKoZI
# hvcNAQkEMSIEIPxx6GVPMKRumOrQ1pvuzbl13upVZnlLLMS5moep9+6AMIG5Bgsq
...
# 9w0BAQEFAASCAgCFuGnjOiDsa1mUmrQL1YSj3t2kOu84lBJHQJcQS62ed+xI7nmr
# nkmsn57XO8Sh6HKwe4sT68bbaMuQBGcgRmX50PClt7Y83ZeDGBjdyvl47asUXbUh
# +9vJNjR3jvaovQB4nTsupA1YN3PhHPnNsJQqthce2LfT6IuU+GRomWw/El78nkvq
# 6gb4MHP/1OxpG/ZZlpwODjwRVDoWnTwAAU8pteikmNjFXxq1xQ1nPTLj+H+fwxUK
# zP8ol6l9Zs/g/HtDYbEtoeohRoyn4xohhLvkjRAJVBx+cQy0xVSb+f5RSpTR08Bh
# q68KxW9CW7VhBUtquru/5e0jfAH1LYKjaITguASLnq4udpwdAMHLVNVGXHRjmZjg
# u/v8XEpM9Tim/s8HzjO6WHgiFcs9Qqn8eqG2TtJ/XJpd/xWYYajSAxJ8f+qOxihi
# q0pVOGI8/4dkv5nOVNi36GPyiwzk8cBEK0EgwauVc8GKtwAgeOat8sFQUx6oJt40
# 991ClMFQAqtBz04jPKja1RBmyZ1cVkmDrtrnjk/exNroyGBhHxE8IP6uLwuOHenJ
# vGTmFWrzwd5cDCdSr9ftsyxOhAfuCnaYv2OyGgXva6K5JeUolhQ+l4y7X+b9yLjl
# hlkMhAcrUE0hE19ShqkWbsdMeJxiwbOLQcU8uiiAhp3g6nsDPz8njtjiYw==
