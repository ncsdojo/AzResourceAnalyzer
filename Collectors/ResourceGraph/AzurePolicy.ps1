# AzurePolicy collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "AzurePolicy"
        ApiType     = "ResourceGraph"
        Table       = "policyresources"
        FileName    = "AzurePolicies"
        Label       = "policy assignment(s)"
        UniqueField = "policyAssignmentName"
        Query       = @"
policyresources
...
| extend complianceState       = tostring(properties.complianceState),
         ...
         policyDefinitionRefId = tostring(properties.policyDefinitionReferenceId),
         policySetDefinitionId = tostring(properties.policySetDefinitionId),
         resourceType_         = tostring(properties.resourceType),
         resourceLocation      = tostring(properties.resourceLocation),
         stateWeight = iff(tostring(properties.complianceState)=='NonCompliant', 300,
                       iff(tostring(properties.complianceState)=='Compliant', 200,
                       iff(tostring(properties.complianceState)=='Conflict', 100,
                       iff(tostring(properties.complianceState)=='Exempt', 50, 0))))
| summarize max(stateWeight) by
         resourceId,
         policyAssignmentId,
         policyAssignmentName,
         policyAssignmentScope,
         policyDefinitionId,
         policyDefinitionRefId,
         policySetDefinitionId
| summarize
    nonCompliantCount = sumif(1, max_stateWeight == 300),
    compliantCount    = sumif(1, max_stateWeight == 200),
    conflictCount     = sumif(1, max_stateWeight == 100),
    exemptCount       = sumif(1, max_stateWeight == 50)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBoIbxaMyKp76rM
# dMM0mNh+VY8CSeDuWa8+uK1Sl3B1iKCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMm4LlxJiYveNyx3cxGuo27UcohJQZZ0w+664NRj
# 9FxwMA0GCSqGSIb3DQEBAQUABIIBgEVgQRqZqZ3pqwuITwvk7BCvU+k6GFQDmCmm
# gUT0IQzAfowogJQuUmrc3pTUW8dkjYyayo5ip4HoxBMGfA02w4EJU4VV81T8eCaX
# zLdgBHpVySAv6EbjFW77bLVWAZUSYeLmf//J5vWrv+2jSlFAb4C4s9YZJS2xjw2B
# 5cpajJdCSDS+uzzfzhtRkpweyGzw37O4D7FwYZTx4UmYx7GGgFIrS+d8YcUil2/t
# rMAi514XzHB/XnIMKEq5tpQxjEQEYM4qhSbmD1myrF3TTwh2o0a1f+Okhq5kM2j3
# YdneI1L7Se6X/W4tgoLadOwkrElqa8irjqcej2FE4MhNnFk5Nj7p+UhMSY48RG+4
# UUsTwWVEHhS4qvpa33qmjfz7g7nSSDmteSSql5BAyxtgSMaBjgmFVq60RhFMhJT6
# KPqo/cksXqOQYjzdYplJnddcE1urPlWp/xpFMb80v43GM13BjbF3qVMZIpqsnvmb
# Px3hfVvKgsXDBRDkIumGh8/RRmlV4aGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCClNLLV7x5e2waMPRu2DdqxJFwR7C0FCKvUHI44NXeR+gIGagxE2a7T
# GBMyMDI2MDUyMzAyNTAyOC43NzFaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyNTAyOFowLwYJKoZI
# hvcNAQkEMSIEILcaEAn3XfGMaAEPguqQfxYJtb77WIXZHRJT9m+K2ST7MIG5Bgsq
...
# hkiG9w0BAQEFAASCAgBDVhUZPr+sb2Phaou111s5SwdN9HRCrdd0SVwceBLFzPEF
# 9wcq5U1OoBa82/CpkB+rNfG9qBa/nZs4cdaGAGkAmFkEzBEXYOYzTgfezFQNol/6
# /NXdg5lN5QSHjFGiklmCMDCjA/5w/Iwpn66pYT5d7q+y+K/5CPMYOn8StjL6ex3N
# fF/zjSqE6Isk2fUvL+y5bqczpykZIVtxKEePmWRzJr5EaxnlsIeiDJh4zbAzN6Vo
# cpcEZfr9GXEVdLjDyMawAVSyl9PMA25m5LLbNQewWi7QOOtiq+7+MqgzjtX6QpdM
# IRd+ek5BYpr8ehBoOyOHNfZ1Wn6Ue3aL7vGDNtRP6dNPYKGrfNYZ/npVGU49Ae2r
# HqnQ9TPrzciH9ilo8ElscZWdiQK3ycQBIOssySQnyVmCCQAO7IBD7fXHiWK5LRSo
# Dbr2R1nVNv6PQVKU/bV3oQobEke01Jo/iFYTqfEmeo2G8PrbMKNQYWGE24/wXXg+
# N3U09btnVqgFkvP0bE0joZEzSIji0H3hrrpaRWyaZ5uSfDnI6caUXjfFF6rwu3IZ
# EgmceDF9iedW05iBkLI3dYLGM8qiwSqrzb6M2etcxZFIdj4LTJlHPBTeioyCcu2p
# y85Uxr3VqaF+UHjX7vQEwBtLyZv0GeJiXxNFLoHXRVgpHrNWaNWTRdm0w8JXbQ==
