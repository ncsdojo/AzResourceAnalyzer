# Peering collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "Peering"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "Peerings"
        Label       = "VNet peering(s)"
        UniqueField = "peeringName"
        Query       = @"
resources
| where type =~ 'microsoft.network/virtualnetworks'
| extend peeringCount = coalesce(array_length(properties.virtualNetworkPeerings), 0)
| extend _peerings = iff(peeringCount > 0, properties.virtualNetworkPeerings, dynamic([null]))
| mv-expand peering = _peerings
| extend peeringName            = iff(isnotnull(peering), tostring(peering.name), ''),
         peeringState           = iff(isnotnull(peering), tostring(peering.properties.peeringState), ''),
         peeringSyncLevel       = iff(isnotnull(peering), tostring(peering.properties.peeringSyncLevel), ''),
         remoteVnetId           = iff(isnotnull(peering), tostring(peering.properties.remoteVirtualNetwork.id), ''),
         allowVnetAccess        = iff(isnotnull(peering), tobool(peering.properties.allowVirtualNetworkAccess), dynamic(null)),
         allowForwardedTraffic  = iff(isnotnull(peering), tobool(peering.properties.allowForwardedTraffic), dynamic(null)),
         allowGatewayTransit    = iff(isnotnull(peering), tobool(peering.properties.allowGatewayTransit), dynamic(null)),
         useRemoteGateways      = iff(isnotnull(peering), tobool(peering.properties.useRemoteGateways), dynamic(null)),
         doNotVerifyRemoteGateways = iff(isnotnull(peering), tobool(peering.properties.doNotVerifyRemoteGateways), dynamic(null)),
         remoteAddressSpace     = iff(isnotnull(peering), peering.properties.remoteAddressSpace.addressPrefixes, dynamic(null)),
         peeringProvisioningState = iff(isnotnull(peering), tostring(peering.properties.provisioningState), '')
| project subscriptionId,
          resourceGroup,
          vnetName                = name,
          vnetId                  = id,
          localSubscriptionId     = subscriptionId,
          remoteSubscriptionId    = iff(isnotempty(remoteVnetId), tostring(split(remoteVnetId, '/')[2]), ''),
          location,
          peeringCount,
          peeringName,
          peeringState,
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDrYK9Zlyka+ZlZ
# JHRA51gwd4BY/5xqZtHgLFAbEGt1GaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIDMH0bjrfXDVoLNMtW8sTOWH3fySC1ADCtS/7/VN
# yLQbMA0GCSqGSIb3DQEBAQUABIIBgD4iE6NvkNyqy8GZO9UBaMWSf2m+ApqBeufP
# NGZf+BCQbcmHnvIt2sq+iPD/WpeTINGFVK6Fmrr2pER5OtPuqnUaa44ApIH48Dho
# klr9RSg9gLFypQPpi95X9ESRwpqTfv7rqPOneHS4qWD5soXyl7pdnAUowNqpRyvo
