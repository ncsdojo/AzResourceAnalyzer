# RouteTable collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "RouteTable"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "RouteTables"
        Label       = "route table(s)"
        UniqueField = "routeTableName"
        Query       = @"
resources
| where type =~ 'microsoft.network/routetables'
| extend routeCount = coalesce(array_length(properties.routes), 0)
| extend _routes = iff(routeCount > 0, properties.routes, dynamic([null]))
| mv-expand route = _routes
| extend routeName        = iff(isnotnull(route), tostring(route.name), ''),
         addressPrefix    = iff(isnotnull(route), tostring(route.properties.addressPrefix), ''),
         nextHopType      = iff(isnotnull(route), tostring(route.properties.nextHopType), ''),
         nextHopIpAddress = iff(isnotnull(route), tostring(route.properties.nextHopIpAddress), ''),
         hasBgpOverride   = iff(isnotnull(route), route.properties.hasBgpOverride, dynamic(null))
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDFAsgXsUM75NB5
# n8nlQPFfS5h4joY+sgVbVLj1ah3rEaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIEa+S8zHbWEKQ3uLnRgmMdFRFGxKPCJlKVfzSQ7r
# XlAVMA0GCSqGSIb3DQEBAQUABIIBgFjQGqDE78XcSnH3WGbwYhAeI19zrZvApXah
# WTTYAfoPeyvdZpfNqwQqSCRicBjOgEa0ATlHk7KIRN5+BDGMfeF9UeF2T6iox9EY
# NFapDgmQlbu0+QhT8C6NH+zK2L6zCv6dW0Pb4a/WSEw6JS9nifBhza4bsXMDV4IQ
# k3oSRCR4qkIA+GnWjwSV+pUDrwAs0j3zrZ7nKk5hBdLT9Zn2RQOwdYHcR9YvGs5F
# gaJB00uUk+VzpUQ6y5x3V2Eu+zJ2rt/f+imntPyu9ORgLgZ1kzjwf/DWw5PMOpyP
# SqAMRhVM8/WBS7b00/PgEng60+TVvMkQAmY2trNGUhkQxVTZw5DukTc569ttcR1u
# +3yp6fRly9EMhhCTTsEi9Bw51agwiF7Gqnxp0R6I4DBFtOyPYV7m/DNfbW5oDQoi
# Pndz/uBPs8Dsyg2VJNZsOQ1yrzYX7RjDXNRg3tpm1n0Ca8Y8AP+QYwzPi14FHtpq
# vMyEEV75c/0jSBONtOd3dAeDN7PU9KGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCB35zIr7FUbamuMkzC5zDhO+aIH4ui8QN6gOJpqDhgwVAIGagxE2bE1
# GBMyMDI2MDUyMzAzMDA1Ni44NTdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAzMDA1NlowLwYJKoZI
# hvcNAQkEMSIEIPouEAMyROyqVcC9JQQ4qglQnICa7DcQZja1vWs8xMCTMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCLtqnhEv6Yhd3YjVB6GD9n+0VzbEHwNy7Xzbltm5dYKRdV
# cgJj/p4ctH6D6+KenYn7jVbeS6YCtFK7sRDVtYpWYrkEg6PyYWVT9QbVKq/3Vq8v
# 2onsk+RhpqVLMXjPIkjxpdyIeRwsbxpCx3LwzMdRK7xkGL88pxwgcq6iORBnKn7O
# 7DWsDBuGga69iHz09c905ucUjfQ0m5chW+aWNW0CccBygpWaS1gcnGu1Tr/kDjtu
# SKIZsRaZESxCo7DkvjsqZTx9iZHKnjcGixhAEMqpOzhtfaCxfbAExrvJkWN3tA6O
# rTAxwcPorepeAEgc1IhXalLxxrg1hBuzzWeR8eTL9lhblKShy08R3lLLm42FLfYQ
# vIJ3Zov5Ly3xKjR+ify5XIrR9znU0ryN9nqQ/vBV0GSLzZuvqFLWlvNMGXfH89qZ
# l4gOnMjo1xPL7871I0w4zA7H1DCuhfneSqZeBpRXb1zx/CmwrgH5NgNZBMwuJWyC
# bFgtwsN/lyQmD3jAm0x48nrdcKBr4GBXI+2NevthpZUbnBQgZcDz8pTB4r7Vl7yM
# LyYf7sB6CaIqrbuI1lFbC+fteeLB2KjBcijPy+7/F8ksqAEbZbq+7/NSc1ntZLkx
# Zxwa8uCaYeltZdklcNof0Xvn4RuOW0gd9Rr0Y+1QQJ8ulgOQ37vlxcertm74Dg==
