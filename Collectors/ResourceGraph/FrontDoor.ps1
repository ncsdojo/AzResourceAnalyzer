# FrontDoor collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "FrontDoor"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "FrontDoors"
        Label       = "Front Door profile(s)"
        UniqueField = "name"
        Query       = @"
resources
| where type =~ 'microsoft.cdn/profiles' or type =~ 'microsoft.network/frontdoors'
| extend skuName              = coalesce(tostring(sku.name), ''),
         resourceKind         = coalesce(tostring(kind), ''),
         provisioningState    = coalesce(
             tostring(properties.provisioningState),
             tostring(properties.resourceState), ''),
         frontDoorId          = tostring(properties.frontdoorId),
         friendlyName         = tostring(properties.friendlyName),
         cname                = tostring(properties.cName),
         enabledState         = tostring(properties.enabledState),
         frontendEndpoints    = properties.frontendEndpoints,
         backendPools         = properties.backendPools,
         routingRules         = properties.routingRules,
         healthProbeSettings  = properties.healthProbeSettings,
         loadBalancingSettings = properties.loadBalancingSettings,
         originGroups         = properties.originGroups,
         frontendCount        = coalesce(array_length(properties.frontendEndpoints), 0),
         ...
         originGroupCount     = coalesce(array_length(properties.originGroups), 0)
| project subscriptionId,
          resourceGroup,
          frontDoorName        = name,
          frontDoorResourceId  = id,
          resourceType         = type,
          location,
          skuName,
          resourceKind,
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD0G65v2y93yq95
# 2NKrfVXU+mWztYVQ0x2OWXirqsPTOqCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIJgRJTA//TF0+BrOGwWwV3KaxFRYWIprHGgFr5d5
# h3+AMA0GCSqGSIb3DQEBAQUABIIBgDEaC6fS39uCVRGxL/wRlfFt4PFN2F8gqEGN
# XEkJ+odZlUxdFmw3JL2dBjDmp6ybjV9oxxMPm7NOkb5C8Y5DWWIWNm16QMT4a9fr
# C5rT5/fmhi00HuRjPJ1Q3OQ09nYxHDuLXzGQCck2MBlF3Cwg3v3t40mkfjdFHkHp
# rQ/KJOuS0u07d2/+PKVEyB4XQVq6snkqaY5hATFJphGIuvB6s/84b/mAILjm02uf
# eiJsQ3Dhl5Rdrj/vUHTr+toqaciEv2drQq3Gqx7VHQFQlqQeQsz4dulvssjnkxN8
# 7kYwTLtJl6DhpfwwkanUtAksdVQtagGzrC+QXDZlRkWjfcybv0I0GbiCR3sA2cAG
# XxdlxVDJoAnMnMxbkrwn7v+L4a256k5vt85Sx2d27+6XzgeZw68YtGaU/Iv37u1g
# pLNXFHa4EDvdnAQ+0LCwJQKdu7Uw/WkH9PIHxu/PDuwquAkzU6lTzxGk3RdvFSv0
# jnOiq9m0njRjwWmFH0YKFHrfE3A4GqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCCkkYa08mXILI0lJHq5htYkRwybRaLHuz4FillqbRrwQwIGagxE15LP
# GBMyMDI2MDUyMjE1MzcyNy43NzNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMjE1MzcyN1owLwYJKoZI
# hvcNAQkEMSIEIMFF3v6Ue8RJHniQTUx64MdVXzrodYIKQI/LkypRh6gqMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgCIN2xAm4XymtTJrvYscQQ12DcJMpOp4W5vih1Ph6TDNrzQ
# NoL9P7tl6gI6tr+xdigRgFOa9RazEYZ397clL2+T4BaqOg2fudDFwUt2doLICxs/
# uc/gEHHm9rIeo/8JbgbInWBoTLGyXywBk8CbZJvpyNqwqAJrGyXZ3vD8AL3Ei0aG
# Ek6R7SWewbiNGuRu1c4gWWyOy9HSoKJfCic6H6VX/dUcuUj46QyztXDmHK7MjbDu
# py2i1yhVryE95CiP1/54KanE0ELyI6fnkzDh8ALQs/d7Dlr+uvG3AC2BkaGAbyTK
# Hcf3RJ9KiPilAw85Wcf1h1sG3Vke463iIl1CljGyOdR+SHuDkdwP67zTfgbWonia
# EJwNJRWE19AwQOScHl2y2kkHEmDeWONmvT7GIauJt/AF8Gp9bKJpP06n4qlLLSn+
# VVP25muUuI4UwW/6CTISiNPKQvZwacpgw62jQtorGMqm4fIfJx1N6pUpMMi9huoR
# bwPXhWGEu1grzGkb+/v+APyQStcrHbKPknvsdd+LE9527V5AjOwNYK7F8b/ZSoeq
# WlpntcMm79gSkKlqvcXAfNYPWDnqCyGpQgCzGm84eRyYwCJFWzDjNZosA3KxlorE
# NdLM3+VdGrgZhSxjB3BViBuDVbCiUuv67R+/mVNydoGQG79a5wGQHnbbhBvDZA==
