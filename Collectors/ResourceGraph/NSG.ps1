# NSG collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "NSG"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "NSGs"
        Label       = "network security group(s)"
        UniqueField = "nsgName"
        Query       = @"
resources
| where type =~ 'microsoft.network/networksecuritygroups'
| extend allRules = array_concat(
        coalesce(properties.securityRules, dynamic([])),
        coalesce(properties.defaultSecurityRules, dynamic([])))
| mv-expand rule = allRules
| extend ruleKind              = iif(toint(rule.properties.priority) >= 65000, 'Default', 'Custom'),
         ruleName              = rule.name,
         ruleDirection         = rule.properties.direction,
         rulePriority          = toint(rule.properties.priority),
         ruleAccess            = rule.properties.access,
         ruleProtocol          = rule.properties.protocol,
         ...
         destAddressPrefix     = rule.properties.destinationAddressPrefix,
         destAddressPrefixes   = rule.properties.destinationAddressPrefixes,
         destPortRange         = rule.properties.destinationPortRange,
         destPortRanges        = rule.properties.destinationPortRanges,
         destASGs              = rule.properties.destinationApplicationSecurityGroups,
         ruleDescription       = rule.properties.description
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDJzlOUKi4XFCSQ
# BluMnV4UnaV8K4O7sMjp3rE4eeMuSaCCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIHmhd8OuLyWLDR712e1BEXJkzEZjzlWJK3l2SIGv
# 191hMA0GCSqGSIb3DQEBAQUABIIBgJJZ/RXW1zgYpjOgrVYjYxIvsdpUvZhPIKgb
# bhmHN3N62uao1xdyAd596OS9FjWIqmeceIEKRSO5d3vHBswN1OP+EaHV2hJu1Bea
# yOWxwwtjFnHiawaC5MdjbXSDDCnnc4PscrToL1ikoH1KurDFGjDVeT+CzDvOZJIT
# UlKAAlrdFWO/TRfrYkMy7eS3lBTxX5RNN3qfi6Ztzd2Og/Jp8c9qj4zIalMlfo9W
# flEO2BsS2VhmiO3Mm3eF+X6gNOZlI7Pcjq+CeTpP4TXo5gUrkmMficItHtSvnFWi
# vnEFhal4jNKl7c2abxQGyOx/ZMNLOLXlTajqfbWTlR39/0UN7NVJSq3FdLUq3mvv
# f6OrJbeDyuI/Ir2tMesxgsF0slbY0WEKLvgxF72XzVOrdmlND7mYJdDg71gdW8D2
# G0CC8YbuS/y2cz1t2bE9PVy2c7o52+Si7UKA6tYSBZpyTvnTe6GVoCLw6X+0cCvH
# Xpcd2hL6zd2cl3nYBU5iBQGB6tYKeaGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCB80atlbrQ8r91Wf+0JYdXVIjqqdUrejUg1XCu8UMaFYQIGagxE2bCt
# GBMyMDI2MDUyMzAyNTgyNy4xMjdaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAyNTgyN1owLwYJKoZI
# hvcNAQkEMSIEIHMv5VoChMbUQyedLji/PqZoWilyUfOSTz5n2Qt9rP0+MIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAPOEU5V0OtExN37/B22vu6C2guEl3lrJBpdHI2qOCQCpC1
# lb5WQhW9XsmTFYWMIVsIfKS0ZPvEAdUem7sq59scisTFaWnI8En+7WzCXGmJqrju
# Vx3P/CwLu5KeODDEsXdUc++KQu7f2HtXKgc0guzQRgYvoj6fmAFXBMWVaKLZx5Mo
# bOsCO4F/ILDOWQ7WzwhDkH2rM+GXQELKR14aRh5LccP/j41dO9OtRuJjZBhNPR+0
# bKVXWpa/bbUPBVH5SpV+PECvDZiZKe12TsuZ98UUf8Fo0szJt1wJzK98XpYMtz6G
# AYnE08aDysHKNguqaWXvUZ76gChCGkyyrJkJkiZyJ7bkAfuxbeNJcPPGuC3+j0zS
# rUNsLVQ4NBLE/647nM/b1yhtwKLAvaf+TRaeF1lCVllsoS1vHxLRtC0m3sV9H+Ut
# Yh8PPQqXuujyXEo5o2i74mkpURC93uh6FO5UFETXrSsGD/rjG2GijKZRlTRGnEKa
# bVknv9ws+xfGoEVCRcQTgkalbzST66fEQz/8ZQzgC4bfKRLqB/AWgJ8WhAyAKEhp
# 0O4ayisd7ZuYRHjrfTbHDINk4MqAOKL1a0BzmLQChy9ZmcEvXlcZFlYhYXATJ2Vv
# v7LKAp4txjcEspKPBCNzLi29hPAj5bStaqJ+eNOTI0CPp3Lv5k8ts3RUJ4iGDw==
