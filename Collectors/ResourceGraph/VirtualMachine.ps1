# VirtualMachine collector — Copyright (c) NCS Dojo. All Rights Reserved.
@{
        Name        = "VirtualMachine"
        ApiType     = "ResourceGraph"
        Table       = "resources"
        FileName    = "VirtualMachines"
...
| extend vmSize              = tostring(properties.hardwareProfile.vmSize),
         ...
         osDiskCaching       = tostring(properties.storageProfile.osDisk.caching),
         osDiskStorageType   = tostring(properties.storageProfile.osDisk.managedDisk.storageAccountType),
         ...
         vmScaleSet          = tostring(properties.virtualMachineScaleSet.id),
         proximityGroup      = tostring(properties.proximityPlacementGroup.id),
         priority            = tostring(properties.priority),
         evictionPolicy      = tostring(properties.evictionPolicy),
         billingMaxPrice     = tostring(properties.billingProfile.maxPrice),
         bootDiagEnabled     = tobool(properties.diagnosticsProfile.bootDiagnostics.enabled),
         bootDiagStorageUri  = tostring(properties.diagnosticsProfile.bootDiagnostics.storageUri),
         ...
         secureBootEnabled   = tobool(properties.securityProfile.uefiSettings.secureBootEnabled),
         vtpmEnabled         = tobool(properties.securityProfile.uefiSettings.vTpmEnabled),
         encryptionAtHost    = tobool(properties.securityProfile.encryptionAtHost),
         userData            = isnotempty(properties.userData),
         vmZones             = tostring(zones),
         extendedLocationType = tostring(extendedLocation.type),
         extendedLocationName = tostring(extendedLocation.name),
         managedIdentityType  = tostring(identity.type),
         timeCreated          = tostring(properties.timeCreated),
         dedicatedHostId      = tostring(properties.host.id),
         platformFaultDomain  = toint(properties.platformFaultDomain)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCelDsFZiLtKm8i
# 76wKxMMqWdJvFK+IRiDM7nmD1FpoE6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIEIMXkEY5B/GySGtQmjwthwM2lLxBLoQAJduPxNjP9
# Nv09MA0GCSqGSIb3DQEBAQUABIIBgEEY4CtFlvde1wSWGOBUwV/bdYsVmRPOrm2T
# 8+AA7nUgKk6zI/1q9QauM7ro60j5HdCmRmksQsEwMKwNtONv3JgTnaC2P3lQ0goD
# zXlFkAoKAmi+etyTnn5S/2krYzM5k1NdAI0H7in+f1tQ91kH5mYUvAjhv1cHhJS/
# UhqKTJQcGNGsqZX6Yt69b4Yl+z+hss0y/zWCn82kvy/NDnfODJSYpuQs6vxjv0Tl
# 1N1lfAHCXM3v7yCe4NCPtB2bPpqX6xpHu4uR17RpzLpcCf2y/x/Chyo2fTSg6hvO
# z/O9nu6f7h9BENNSOJ7hz36ziCsCY4wJSWmJk3pWNzGx6Y00r9mdmtKUP80fAbdz
# wavuTts8pXfnPsUVEgMQQyPibeDtoNyAZYybSh6AYj0SJojxvKUg7KSQPF54wDdT
# klosvUj5XcAFmRiAq3AnDIP9NyhYs4Z+uQnlDGXwIYSlz2i/oILIU84Zqj/0x/cI
# 1beGkighNe+5lw7poju6E9WUYwFjIKGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCC
...
# AwQCAQUABCC37wQb+uf/+df8bmj5JfJ/DFMBaKg8XEooJMAeflysDAIGagxE2bHu
# GBMyMDI2MDUyMzAzMDQzNC42MDJaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJV
...
# KoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI2MDUyMzAzMDQzNFowLwYJKoZI
# hvcNAQkEMSIEIFBEwo0IYktttsDHp21rtgGdm23IIA4H9muRAgPkUwJnMIG5Bgsq
...
# hkiG9w0BAQEFAASCAgAAJuheByeJPKVDfwk0LnSbWLgvGHEHZYGxvXbpgjC4JgJN
# sgpwySRdKtM38wOAWOcahnGvd6alUU2hR8SJ3i8JWY7DCCy6oaiWidt/Oh4IGSNe
# p8cNHuuw9sX+bHnyYoiyHhN5+mXT+tNH4OojrAV0McUMpXvvsYYkrVdLbTLi6LYL
# Y3/oHgrerhkaNcbfGB1U1CavF1qqwZVCy6g4J4vNCNPAj/P1Z6BuDHI8QsqG3BkG
# bE51C60qwaP0yUJyTsbzVnApL5vg65jaZp5PTIZKKwUTcZSjDF0em1Hb+ruFM4Sd
# 5AgPi7GC50CGOnTL05gRZF6ZiM9XmmKcagUW0Ptw55DIOhTpTRTXVwRFOJ6aYlb3
# 63J966WKiWjELUCmL+uUyOB6CbLKarG4ihDjx3nY8tF49dUkrnIqeM9lQJ4j/8un
# 7O4vSP6VyFA3DCFkTl/Tfvjfeh3tfMJO4wVOCqWlGNKJrUlX7+Ofj2YbabvGTlCd
# JGa9oYTvBIBa54Qc5bvM446tu/k62IUUT9oIpudpUVT0Ptey7o6aMW2WMeDjUx1y
# 9M0AIqYiUdSM6lZw6mvSYqhBN675GmheEg4eXfzMOMxWQ4LeeEpG3ybjvVRqSEsK
# 8X8NMudgpCi+6uDF6tSndUeOnoyRmJBYaHvnnle4oXJUjSGFerjZ4rLwuIMcAw==
