class ArTokenStore {
    ...
    hidden [hashtable]    $_tokens = @{}  # scope -> @{ secureToken, expiry, clientId }

    static [string] $ArmClientId   = '04b07795-8ddb-461a-bbee-02f9e1bf7b46'
    static [string] $GraphClientId = '14d82eec-204b-4c2f-b7e8-296a70dab67e'
    static [string] $ArmScope      = 'https://management.azure.com/.default'
    static [string] $GraphScope    = 'Policy.Read.All User.Read.All Directory.Read.All UserAuthenticationMethod.Read.All'
    static [string] $VaultScope    = 'https://vault.azure.net/.default'
    static [int]    $TimeoutSeconds = 45

    ArTokenStore([string]$tenantId) { $this._tenantId = $tenantId }

    [void] Authenticate([string]$scope, [string]$clientId) {
        $body = @{
            client_id = $clientId
        ...
        Set-Clipboard -Value $dcResponse.user_code -ErrorAction SilentlyContinue
        ...
        try { Start-Process $dcResponse.verification_uri -ErrorAction Stop } catch {}

        $interval = [math]::Max([int]$dcResponse.interval, 5)
        $maxWait  = [math]::Min($dcResponse.expires_in, [ArTokenStore]::TimeoutSeconds)
        ...
        while ([datetime]::UtcNow -lt $deadline) {
            ...
            try {
                ...
                $this._tokens[$scope] = @{
                    secureToken = ConvertTo-SecureString -String $tr.access_token -AsPlainText -Force
                ...
                $err = $null
                if ($errMsg) { try { $err = $errMsg | ConvertFrom-Json } catch {} }
    ...
    [string] MarshalToken([string]$scope) {
        if (-not $this._tokens.ContainsKey($scope)) {
            throw [System.InvalidOperationException]::new("No token for scope: $scope. Run Connect-ArSession to authenticate.")
        }
        $entry = $this._tokens[$scope]
        if ([datetime]::UtcNow -ge $entry.expiry) {
            throw [System.InvalidOperationException]::new('Access token expired. Run Connect-ArSession to re-authenticate.')
        }
        $bstr = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($entry.secureToken)
        try { return [System.Runtime.InteropServices.Marshal]::PtrToStringBSTR($bstr) }
        finally { [System.Runtime.InteropServices.Marshal]::ZeroFreeBSTR($bstr) }
    }

    [bool] HasScope([string]$scope) {
        return $this._tokens.ContainsKey($scope) -and [datetime]::UtcNow -lt $this._tokens[$scope].expiry
    }

    [void] Dispose() {
        foreach ($entry in $this._tokens.Values) {
            if ($entry.secureToken) { $entry.secureToken.Dispose() }
        }
        $this._tokens.Clear()
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAQbCUajvL0PYwg
# qHyPjkrVhYKpTpBo1aWUZ4MRjkBSy6CCIdwwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQQwLwYJKoZIhvcNAQkEMSIE